...
scripts/train_st_trans_v2.py  ── ST-Trans v2 Training
======================================================
Model: STTransV2 (Uncertainty Weighting + Easy/Hard Split)

CHẠY LẦN ĐẦU:
  python scripts/train_st_trans.py \
...
RESUME (từ best_model.pth hoặc ckpt_epXXXX.pth):
  python scripts/train_st_trans.py \
    --resume runs/st_trans_v2/best_model.pth \
    --dataset_root /kaggle/input/.../tc-ofm \
    --output_dir   runs/st_trans_v2
...
CHECKPOINT LƯU ĐỦ:
  - model_state, optimizer, scheduler   ← resume đúng dynamics
  - epoch, best_ade, patience_cnt       ← resume đúng trạng thái training
  - threshold_curv, threshold_spd       ← resume đúng easy/hard split
  - sigma (UW σ dict)                   ← logging tiện theo dõi
"""
from __future__ import annotations
...
def _save_checkpoint(path, epoch, model, optimizer, scheduler,
                     best_ade, patience_cnt, sigma_d):
    """
    Lưu đầy đủ mọi thứ cần thiết để resume chính xác:
      - model_state       : weights (bao gồm UW σ và step_weights vì là nn.Parameter)
      - optimizer         : Adam momentum/variance → không mất đà khi resume
      - scheduler         : ReduceLROnPlateau num_bad_epochs → không reset LR counter
      - epoch             : epoch hiện tại
      - best_ade          : best val ADE để so sánh tiếp
      - patience_cnt      : đếm số epoch không cải thiện
      - threshold_curv/spd: easy/hard threshold đã tính từ train set
      - sigma             : UW σ dict để log tiện (đã có trong model_state rồi, đây là log)
    """
    torch.save({
        "epoch":          epoch,
        "model_state":    model.state_dict(),
...
def _load_checkpoint(path, model, optimizer, scheduler, device):
    """
    Load checkpoint và restore toàn bộ trạng thái.
    Trả về (start_epoch, best_ade, patience_cnt).
    optimizer và scheduler được restore in-place.
    """
    ...
    # Restore optimizer — giữ lại Adam momentum/variance
    if "optimizer" in ckpt:
        optimizer.load_state_dict(ckpt["optimizer"])
    else:
        print("  ⚠ Checkpoint không có optimizer state → Adam reset (5-10 ep đầu có thể lạ)")

    # Restore scheduler — giữ lại ReduceLROnPlateau num_bad_epochs
    if "scheduler" in ckpt:
        scheduler.load_state_dict(ckpt["scheduler"])
    else:
        print("  ⚠ Checkpoint không có scheduler state → LR counter reset")

    start_epoch  = ckpt.get("epoch", 0) + 1
    best_ade     = ckpt.get("best_ade", float("inf"))
    patience_cnt = ckpt.get("patience_cnt", 0)

    return start_epoch, best_ade, patience_cnt
...
def get_args():
    ...
    p.add_argument("--gpu_num",          default="0")
    p.add_argument("--resume",           default=None,
                   help="Path tới checkpoint (best_model.pth hoặc ckpt_epXXXX.pth)")
    # DataLoader compat
    p.add_argument("--delim",      default=" ")
    p.add_argument("--skip",       default=1,     type=int)
    p.add_argument("--min_ped",    default=1,     type=int)
...
def main(args):
    ...
    # ── Thresholds ────────────────────────────────────────────────────────
    # Luôn compute từ train set trước; nếu resume thì sẽ override bằng
    # giá trị đã lưu trong checkpoint để đảm bảo nhất quán.
    print(f"\n  Computing easy/hard threshold (p{args.threshold_pct:.0f})...")
    t0 = time.perf_counter()
    tc, ts = compute_hard_thresholds(
        train_loader, device, percentile=args.threshold_pct)
    ...
    # ── Optimizer + Scheduler ─────────────────────────────────────────────
    # Tách UW σ params ra để dùng lr nhỏ hơn, tránh sigma oscillate
    uw_param_ids = set(id(p) for p in model.uw.parameters())
    model_params = [p for p in model.parameters()
                    if id(p) not in uw_param_ids and p.requires_grad]
    uw_params    = list(model.uw.parameters())
    ...
    # ── Resume ────────────────────────────────────────────────────────────
    # Đặt SAU khi tạo optimizer + scheduler để có thể restore state của chúng.
    start_epoch  = 0
    best_ade     = float("inf")
    patience_cnt = 0

    if args.resume and os.path.exists(args.resume):
        start_epoch, best_ade, patience_cnt = _load_checkpoint(
            args.resume, model, optimizer, scheduler, device)
        print(f"\n  ↩ Resumed ep{start_epoch}"
              f"  best_ADE={best_ade:.4f} km"
              f"  patience={patience_cnt}/{args.patience}"
              f"  curv_thr={model.threshold_curv:.3f}"
              f"  lr={optimizer.param_groups[0]['lr']:.2e}")
    elif args.resume:
        print(f"  ⚠ Resume path không tồn tại: {args.resume} → bắt đầu từ đầu")
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        if epoch % args.val_freq == 0:
            ...
            if ade < best_ade:
                best_ade     = ade
                patience_cnt = 0
                _save_checkpoint(best_ckpt, epoch, model, optimizer, scheduler,
                                 best_ade, patience_cnt, sigma_d)
                print(f"  ✅ Best ADE = {best_ade:.4f} km  (epoch {epoch})")
            else:
                patience_cnt += args.val_freq
        ...
        # ── Periodic checkpoint (mỗi 50 epoch) ───────────────────────────
        if epoch % 50 == 0 and epoch > 0:
            periodic_path = os.path.join(
                args.output_dir, f"ckpt_ep{epoch:04d}.pth")
            _save_checkpoint(periodic_path, epoch, model, optimizer, scheduler,
                             best_ade, patience_cnt, sigma_d)
