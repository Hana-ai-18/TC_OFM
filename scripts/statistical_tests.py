...
"""
scripts/statistical_tests.py  ── v10
=====================================
Standalone statistical hypothesis testing for TC track forecasting.

FIXES vs v9:
  BUG-5-STAT FIXED (companion to train_flowmatching BUG-5-FAKE fix):
    run_all_tests() now checks for all-NaN or length-1 baseline arrays
    before running comparisons. Comparisons with invalid/missing baseline
    data are SKIPPED with a clear warning rather than producing
    meaningless test statistics.

    The n < 2 guard already existed but only caught alignment failures.
    Added: np.all(np.isnan(b)) guard before calling paired_test().

Kept from v9:
    All paired tests (Wilcoxon, t-test, Cohen's d, Bonferroni, bootstrap CI)
    Normality tests (Shapiro-Wilk)
    PINN sensitivity table generator
    Synthetic standalone self-test

Implements ALL required paired comparisons (if real data provided):
  FM+PINN vs CLIPER
  FM+PINN vs Persistence
  FM+PINN vs LSTM baseline
  FM+PINN vs Diffusion+Reg
...
def _is_valid_baseline(arr: np.ndarray | None, name: str, min_n: int = 2) -> bool:
    """
    BUG-5-STAT FIX: Check whether a baseline array contains real data.
    Returns False (and prints a warning) if:
      - arr is None
      - len(arr) < min_n
      - All values are NaN  ← new check
    """
    if arr is None:
        print(f"  ⚠  Skipping comparison vs {name!r}: array is None")
        return False
    arr = np.asarray(arr, dtype=float)
    if len(arr) < min_n:
        print(f"  ⚠  Skipping comparison vs {name!r}: only {len(arr)} samples")
        return False
    if np.all(np.isnan(arr)):
        print(f"  ⚠  Skipping comparison vs {name!r}: all values are NaN "
              f"(placeholder from missing real-model inference)")
        return False
    return True
...
def normality_tests(
    ...
    for name, arr in named_arrays.items():
        arr = np.asarray(arr, dtype=float)
        # Skip all-NaN placeholders
        valid = arr[~np.isnan(arr)]
        if len(valid) < 3:
            print(f"  ⚠  Normality test skipped for {name!r}: insufficient valid data")
            continue
        s, p = _shapiro(valid)
        rows.append(NormalityRow(
            model        = name,
...
def run_all_tests(
    ...
    fmpinn_ade     : FM+PINN per-sequence ADE (km) — full test set
    cliper_ade     : CLIPER per-sequence ADE
    lstm_ade       : LSTM baseline ADE  (pass np.array([nan]) to skip)
    diffusion_ade  : Diffusion+Reg ADE  (pass np.array([nan]) to skip)
    fmpinn_rec     : FM+PINN ADE — recurvature subset only
    diff_rec       : Diffusion ADE — recurvature subset
    persist_ade    : Persistence ADE (optional)
    out_dir        : output directory
    ...
    # ── 1. Main paired tests ───────────────────────────────────────────────
    # BUG-5-STAT FIX: _is_valid_baseline() guards each comparison against
    # NaN placeholders or missing real-model data before running any test.
    comparisons = [
        ("FM+PINN vs CLIPER",       cliper_ade,    "all"),
        ("FM+PINN vs Persistence",  persist_ade,   "all"),
        ("FM+PINN vs LSTM",         lstm_ade,      "all"),
    ...
    for comp_name, baseline, subset in comparisons:
        # Validate baseline before attempting any test
        if not _is_valid_baseline(baseline, comp_name):
            continue

        a_src = fmpinn_ade if subset == "all" else fmpinn_rec
        if not _is_valid_baseline(a_src, f"FM+PINN ({subset})", min_n=2):
            continue

        a = np.asarray(a_src, dtype=float)
        b = np.asarray(baseline, dtype=float)

        # Drop NaN entries from either side (paired, so drop by index)
        valid_mask = ~(np.isnan(a[:min(len(a), len(b))]) |
                       np.isnan(b[:min(len(a), len(b))]))
        n_aligned = int(valid_mask.sum())
        if n_aligned < 2:
            print(f"  ⚠  Skipping '{comp_name}': only {n_aligned} valid paired samples")
            continue

        a_clean = a[:min(len(a), len(b))][valid_mask]
        b_clean = b[:min(len(a), len(b))][valid_mask]

        result = paired_test(a_clean, b_clean, comp_name, subset, bonf_n, n_boot)
        test_rows.append(asdict(result))

        g = _hedges_g(a_clean, b_clean)
        effect_rows.append(asdict(EffectSizeRow(
            comparison    = comp_name,
            cohen_d       = result.cohen_d,
            interpretation= result.cohen_interp,
        ...
        print(f"  {comp_name:<35}  N={n_aligned:>4}  "
              f"Δ={result.mean_diff_km:+.1f}km  "
              f"d={result.cohen_d:+.2f}  "
              f"p_wil={result.wilcoxon_p:.4f}  "
              f"p_bonf={result.wilcoxon_p_bonf:.4f}  "
    ...
    if test_rows:
        ...
        print("  ⚠  No valid comparisons were completed.")
...
if __name__ == "__main__":
    ...
    if args.errors_dir and os.path.isdir(args.errors_dir):
        def _load(name):
            ...
            if os.path.exists(p):
                return np.load(p)
            print(f"  ⚠  {p} not found — using NaN placeholder (comparison will be skipped)")
            return np.array([float("nan")])   # NaN placeholder, not zeros
