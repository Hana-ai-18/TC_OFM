...
#         print(f"  Epoch {ep:>3} train={avt:.4f} val={avv:.4f} {eps:.0f}s")
...
"""
train_v74.py — TC-FlowMatching v74-FINAL
════════════════════════════════════════════════════════════════════════
FIXES SO VỚI VERSION CŨ:

[FIX-1] swr=0.0 — double prefix bug
  get_loss_breakdown trả về key "sw_sw_ratio" (double prefix "sw_" + "sw_ratio")
  Train script đọc bd.get("sw_ratio") → luôn 0
  Fix: đọc đúng key "sw_sw_ratio" từ breakdown dict
       VÀ fix model để trả về key đúng (không double prefix)

[FIX-2] efrac quá thấp (6-31%) → ADE epoch 0 rất lớn
  easy_thresh=0.25 quá thấp với TC dataset
  Hầu hết samples có delta~0.3-0.5 → classified hard → L_ep active trên ~90% samples
  Fix: dùng adaptive threshold = running median của delta
       → luôn ~50% easy, ~50% hard bất kể dataset/basin distribution
       Fallback: easy_thresh=0.40 nếu không có adaptive

[FIX-3] lw_dpe_fm cũng bị double prefix
  Tương tự FIX-1: key là "lw_lw_dpe_fm" không phải "lw_dpe_fm"
  Fix: đọc đúng key hoặc fix model

NOTE: Cả 3 fixes đều fix ở TRAIN SCRIPT (không cần sửa model file).
      Model get_loss_breakdown trả về dict với double prefix —
      train script phải đọc đúng key.

MONITORING:
  sw_ratio >= 3.0          → step weights đúng hướng
  lw_dpe_fm ∈ [1.5, 3.0]  → DPE/FM balance ổn
  L_hard > L_easy          → hard storms penalize nhiều hơn
  easy_frac ≈ 0.40-0.60    → distribution hợp lý (FIX-2)
  tot ∈ [1.0, 5.0]         → loss scale tốt
"""
from __future__ import annotations
...
# ── [FIX-1+FIX-3] Key reader — đọc đúng key bất kể single/double prefix ──────

def _bd(d, *keys, default=0.):
    """
    Đọc value từ breakdown dict, thử nhiều key variants.
    Model có thể trả về "sw_sw_ratio" (double prefix) hoặc "sw_ratio" (single).
    Hàm này thử tất cả variants → không bao giờ bị 0 do sai key.
    """
    for k in keys:
...
# ── [FIX-2] AdaptiveThreshold — luôn giữ ~50% easy/hard ─────────────────────

class AdaptiveThreshold:
    """
    Tự động điều chỉnh easy_thresh để giữ easy_frac ≈ target_frac.

    Tại sao cần:
    - easy_thresh=0.25 hardcoded → với TC dataset, efrac=6-31% → hầu hết hard
    - Hard mask dominated → L_ep active trên 90% samples → loss rất lớn ep0
    - Adaptive threshold: EMA của median delta → luôn ~50% easy/50% hard

    Update rule: EMA(0.02) của batch median delta
    → stable sau ~50 steps, không oscillate
    → min=0.15, max=0.65 để tránh degenerate cases
    """
    def __init__(self, init_thresh=0.35, target_frac=0.50, ema_alpha=0.02):
        self.thresh     = init_thresh
        self.target     = target_frac
        self.alpha      = ema_alpha
        self._median_ema = init_thresh

    def update(self, delta_tensor: torch.Tensor) -> float:
        """
        delta_tensor: [B] tensor từ _diff_score, values ∈ [0,1]
        Returns: threshold để dùng trong batch này
        """
        with torch.no_grad():
            batch_median = float(delta_tensor.median().item())
        # EMA update
        self._median_ema = (1. - self.alpha) * self._median_ema + self.alpha * batch_median
        # Clamp để tránh degenerate: không để toàn easy hoặc toàn hard
        self.thresh = float(np.clip(self._median_ema, 0.15, 0.65))
        return self.thresh
...
def get_args():
    ...
    p.add_argument("--no_ot",               dest="use_ot", action="store_false")
    p.add_argument("--cfg_guidance_scale",  default=1.5,  type=float)
    p.add_argument("--easy_thresh",         default=0.35, type=float,
                    help="Init easy_thresh cho adaptive threshold (default 0.35, bỏ 0.25 cũ)")
    p.add_argument("--use_adaptive_thresh", default=True, action="store_true",
                    help="Dùng adaptive threshold (default True)")
    p.add_argument("--no_adaptive_thresh",  dest="use_adaptive_thresh", action="store_false")
    p.add_argument("--n_ensemble",          default=50,   type=int)
    p.add_argument("--val_freq",            default=3,    type=int)
    p.add_argument("--val_subset_size",     default=500,  type=int)
    p.add_argument("--fast_ddim",           default=10,   type=int)
...
def main(args):
    ...
    print("=" * 72)
    print(f"  TC-FlowMatching v74-FINAL")
    print(f"  Loss: L_fm + L_dpe(step_w,d=300) + L_disp(norm) + L_ep(hard)")
    print(f"  Split: adaptive threshold (init={args.easy_thresh})")
    print(f"         → auto-adjust để giữ efrac≈50% [FIX-2]")
    print(f"  Training: single phase, patience={args.patience}, min_ep={args.min_ep}")
    print(f"  Inference: K=3 mode clustering at 72h endpoint")
    print(f"  Target: ADE<{TARGETS['ADE']} 72h<{TARGETS['72h']} "
          f"ATE<{TARGETS['ATE']} CTE<{TARGETS['CTE']}")
    ...
    model = TCFlowMatchingV74(
        ...
        sigma_min=args.sigma_min, use_ema=args.use_ema, ema_decay=args.ema_decay,
        use_ot=args.use_ot, cfg_guidance_scale=args.cfg_guidance_scale,
        easy_thresh=args.easy_thresh,   # model dùng init thresh
    ...
    # [FIX-2] Adaptive threshold tracker
    adap = AdaptiveThreshold(
        init_thresh=args.easy_thresh,
        target_frac=0.50,
        ema_alpha=0.02
    ) if args.use_adaptive_thresh else None
    ...
    for ep in range(start, args.num_epochs):
        ...
        for i, batch in enumerate(trl):
            ...
            # [FIX-2] Cập nhật easy_thresh trong model trước mỗi forward pass
            # Cần tính delta trước để update threshold
            # Cách đơn giản nhất: update model.easy_thresh từ adap tracker
            # Model sẽ dùng thresh này trong get_loss_breakdown
            if adap is not None:
                # Tính delta nhanh từ obs để update threshold
                # (không cần full forward pass, chỉ cần delta distribution)
                obs_tmp = bl[0]
                env_tmp = bl[13] if len(bl) > 13 else None
                with torch.no_grad():
                    from Model.flow_matching_model import _diff_score as _ds
                    delta_tmp = _ds(
                        obs_tmp.permute(1, 0, 2),
                        bl[11],
                        env_tmp,
                        obs_tmp.device
                    )
                new_thresh = adap.update(delta_tmp)
                # Update thresh trong model (nếu model có attr easy_thresh)
                m_inner = _unwrap(model)
                if hasattr(m_inner, 'easy_thresh'):
                    m_inner.easy_thresh = new_thresh
            ...
            if i % 20 == 0:
                ...
                # [FIX-1] Đọc đúng key — thử cả double prefix và single prefix
                sw_ratio = _bd(bd,
                    "sw_sw_ratio",   # double prefix (model hiện tại)
                    "sw_ratio",      # single prefix (nếu model đã fix)
                    default=0.)
                sw_24h = _bd(bd, "sw_sw_24h", "sw_24h", default=0.)
                sw_72h = _bd(bd, "sw_sw_72h", "sw_72h", default=0.)

                # [FIX-3] Đọc đúng key cho loss weights
                dpe_fm = _bd(bd,
                    "lw_lw_dpe_fm",  # double prefix (model hiện tại)
                    "lw_dpe_fm",     # single prefix
                    "lw_lw_dpe",     # fallback
                    default=0.)

                # Warnings
                sw_warn  = " [!sw<3]"  if sw_ratio < 3.0   else ""
                tot_warn = " [!tot>10]" if bd["total"].item() > 10. else ""
                ef_warn  = " [!ef<0.3]" if bd.get("easy_frac", 1.) < 0.3 else ""

                # Adaptive threshold display
                cur_thresh = adap.get() if adap else args.easy_thresh
                ...
                print(
                    ...
                    f"  sw24={sw_24h:.2f}"
                    f"  sw72={sw_72h:.2f}"
                    f"  swr={sw_ratio:.2f}{sw_warn}"    # [FIX-1] dùng giá trị đúng
                    f"  dpe/fm={dpe_fm:.2f}"            # [FIX-3]
                    f"  efrac={bd.get('easy_frac', 0):.2f}{ef_warn}"
                    f"  thr={cur_thresh:.2f}"           # [FIX-2] hiện threshold đang dùng
                    f"  d={bd.get('delta_mean', 0):.2f}"
                    f"  lr={lr:.2e}"
                )
        ...
        print(f"  Epoch {ep:>3}  train={avt:.4f}  val={avv:.4f}  {eps:.0f}s"
              ...
              f"  Epoch {ep:>3}  train={avt:.4f}  val={avv:.4f}  {eps:.0f}s")
