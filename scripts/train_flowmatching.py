...
"""
train_v35_resumable.py — v35 CLEAN + 72H FOCUSED

THAY ĐỔI SO VỚI v34fix:
  1. Log in hard_72h và direct_mse thay vì vel/recurv
  2. Composite score: weight 72h tăng từ 0.40 → 0.50
  3. Targets hiển thị rõ trong log
  4. Tất cả fix checkpoint từ v34fix giữ nguyên

CÁCH DÙNG:
  # Train từ đầu:
  python train_v35_resumable.py --output_dir runs/v35 [args...]

  # Resume:
  python train_v35_resumable.py \
    --resume runs/v35/best_model.pth \
    --output_dir runs/v35 [args...]
"""
from __future__ import annotations
...
def _composite_score(result):
    """
    v35: weight 72h tăng lên 0.50 để reflect target quan trọng nhất.
    """
    ade = result.get("ADE", float("inf"))
    h12 = result.get("12h", float("inf"))
    h24 = result.get("24h", float("inf"))
    h48 = result.get("48h", float("inf"))
    h72 = result.get("72h", float("inf"))
    return 0.05 * ade + 0.05 * h12 + 0.10 * h24 + 0.30 * h48 + 0.50 * h72
...
class SWAManager:
    def __init__(self, model, start_epoch=55):
        self.start_epoch = start_epoch
        self.n_averaged  = 0
        self.avg_state   = None
...
def get_args():
    ...
    p.add_argument("--output_dir",  default="runs/v35",       type=str)
    p.add_argument("--metrics_csv", default="metrics.csv",    type=str)
    p.add_argument("--predict_csv", default="predictions.csv", type=str)
    ...
    p.add_argument("--resume_epoch",  default=None, type=int,
                   help="Epoch to start from (default: from checkpoint)")
...
def main(args):
    ...
    print("=" * 70)
    print("  TC-FlowMatching v35  |  CLEAN LOSS + 72h FOCUSED")
    print("  TARGETS: 12h<50  24h<100  48h<200  72h<300 km")
    print("  LOSSES:  hard_72h(4.0) + direct_mse(2.0) + endpoint(6.0)")
    print("           + multi_scale(5.0) + mse_hav(1.5) + auxiliaries")
    print("  REMOVED: velocity_loss, recurvature_loss")
    print("=" * 70)
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            if i % 20 == 0:
                lr = optimizer.param_groups[0]["lr"]
                # v35 log: thay vel/recurv bằng hard_72h/direct_mse
                print(f"  [{epoch:>3}][{i:>3}/{len(train_loader)}]"
                      f"  tot={bd['total'].item():.3f}"
                      f"  fm={bd.get('fm_mse', 0):.3f}"
                      f"  hor={bd.get('mse_hav', 0):.3f}"
                      f"  ms={bd.get('multi_scale', 0):.3f}"
                      f"  end={bd.get('endpoint', 0):.3f}"
                      f"  h72={bd.get('hard_72h', 0):.3f}"
                      f"  dm={bd.get('direct_mse', 0):.3f}"
                      f"  shp={bd.get('shape', 0):.3f}"
                      f"  hd={bd.get('heading', 0):.3f}"
                      f"  str={bd.get('steering', 0):.3f}"
                      f"  σ={bd.get('sigma', 0):.3f}"
