...
# THAY ĐỔI LỚN so với v9-turbo:
#   1. UNet3D  → FNO3DEncoder  (4-8x nhanh hơn, 81×81 → 32×32 → FNO)
#   2. En-LSTM → MambaEncoder  (3x nhanh hơn, parallel selective scan)
#   3. Ước tính thời gian cập nhật dựa trên benchmark thực

# Ước tính thời gian (Kaggle T4, batch=32, workers=2, grad_accum=2):
#   Train batch (v10) : ~0.35s  (vs v9: ~1.3s)
#   Val loss          : ~0.12s/batch (ít thay đổi)
#   evaluate_fast     : ~2s (500 seq, nhanh hơn vì FNO)
#   Tổng/epoch (~481 batches, accum=2 → 240 steps):
#     Train     : 240 × 0.35s  = ~84s
#     Val loss  : 266 × 0.12s  = ~32s
#     Amortised : ~5s/epoch
#     Tổng/epoch: ~120s
#   80 epochs: ~160 min = ~2.7h  ✅✅✅

#   Với grad_accum=1 (debug): ~240s/epoch → 5.3h cho 80 epochs
# """
...
#     # ── FNO hyperparams ───────────────────────────────────────────────────
#     p.add_argument("--fno_modes_h",     default=16,         type=int,
#                    help="FNO spatial frequency modes (h). Max=spatial_down//2")
#     p.add_argument("--fno_modes_t",     default=4,          type=int,
#                    help="FNO temporal frequency modes")
#     p.add_argument("--fno_layers",      default=4,          type=int,
#                    help="Number of FNO layers (4 = recommended)")
#     p.add_argument("--fno_spatial_down",default=32,         type=int,
#                    help="Downsample 81→N before FNO. 32=fast, 40=balance, 81=full")

#     # ── Mamba hyperparams ─────────────────────────────────────────────────
#     p.add_argument("--mamba_d_state",   default=16,         type=int,
#                    help="Mamba SSM state dimension. 16=standard, 32=richer")
...
# # ══════════════════════════════════════════════════════════════════════════════
# #  Helpers (unchanged from v9-turbo)
# # ══════════════════════════════════════════════════════════════════════════════
...
#     print(f"  FNO spatial_down: {args.fno_spatial_down}  modes_h={args.fno_modes_h}  layers={args.fno_layers}")
#     print(f"  Mamba d_state   : {args.mamba_d_state}")
#     print(f"  val_freq        : {args.val_freq}  val_subset: {args.val_subset_size}")
...
#     # Patch FNO hyperparams if non-default requested
#     fno = model.net.spatial_enc
#     if (args.fno_spatial_down != 32 or args.fno_modes_h != 16
...
#     print(f"  params  : {n_params:,}  (v9 was ~2.7M, v10 target <1.2M)")
#     print()
