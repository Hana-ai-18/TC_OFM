...
# # # """
# # # scripts/train_fm.py  ──  TC-FlowMatching v2.9

# # # Giải quyết triệt để 3 nguồn gốc của gap:

# # #   Source 1 - FNO3D encoder overfit:
# # #     freeze_encoder_epochs = 25 (was 10)
# # #     lr_enc_peak = 2e-5 (was 5e-5, train slower)
# # #     enc_weight_decay = 5e-4 (separate, was 1e-4)
# # #     fno3d_dropout = 0.20 (set via model patching after load)

# # #   Source 2 - Speed distribution shift:
# # #     L_accel_speed (in model, ST-Trans style)
# # #     obs_dropout_p = 0.15 (in model, forces robust dynamics learning)

# # #   Source 3 - Multi-basin distribution confusion:
# # #     basin_embed (in model, 6 basins × 32 dim)

# # #   Training protocol:
# # #     num_epochs = 300 (ST-Trans: 1200, was 150)
# # #     weight_decay encoder: 5e-4 (was 1e-4)
# # #     Plateau LR: patience=15ep, factor=0.5
...
# # #     TCFlowMatching, _norm_to_deg, _haversine_deg,
# # #     EMAModel, augment_batch, hard_score_from_obs,
# # #     compute_ensemble_uncertainty, compute_heading_deviation,
# # #     compute_cte_contribution, compute_obs_attribution,
# # # )
...
# # # # ─────────────────────────────────────────────────────────────────────────────
# # # #  FNO3D dropout patching
# # # # ─────────────────────────────────────────────────────────────────────────────

# # # def patch_fno3d_dropout(model, dropout_p: float = 0.20):
# # #     """
# # #     Patch FNO3D encoder dropout after model creation.
# # #     FNO3D default dropout=0.05 is too small for 4M params.
# # #     0.20 forces spatial encoder to learn robust features.
# # #     """
# # #     raw = _unwrap(model)
# # #     n_patched = 0
# # #     for name, module in raw.encoder.spatial_enc.named_modules():
# # #         if isinstance(module, torch.nn.Dropout):
# # #             module.p = dropout_p; n_patched += 1
# # #     print(f"  FNO3D dropout patched: {n_patched} modules → p={dropout_p}")
# # #     return n_patched


# # # # ─────────────────────────────────────────────────────────────────────────────
# # # #  3-group optimizer: velocity / encoder / basin_embed
# # # # ─────────────────────────────────────────────────────────────────────────────

# # # def build_optimizer(model, lr_velocity, lr_encoder, wd_velocity, wd_encoder):
# # #     """
# # #     3 param groups with different weight decay and LR.
    
# # #     velocity: lr=2e-4, wd=1e-4 (main model, standard regularization)
# # #     encoder:  lr=0 initially, wd=5e-4 (aggressive regularization for FNO3D)
# # #     basin:    lr=4e-4, wd=1e-4 (fast learning, simple lookup)
    
# # #     Per-parameter weight decay:
# # #       LayerNorm, bias → no decay (standard practice, decay hurts normalization)
# # #       Everything else → apply decay
# # #     """
# # #     raw = _unwrap(model)

# # #     def _no_decay(name):
# # #         return any(nd in name for nd in ["bias", "LayerNorm", "layer_norm", "ln_", "_ln"])

# # #     vel_wd   = []; vel_nwd = []
# # #     enc_wd   = []; enc_nwd = []
# # #     bas_wd   = []; bas_nwd = []

# # #     for n, p in raw.velocity.named_parameters():
# # #         if not p.requires_grad: continue
# # #         (vel_nwd if _no_decay(n) else vel_wd).append(p)

# # #     for n, p in raw.encoder.named_parameters():
# # #         if "basin_embed" in n:
# # #             (bas_nwd if _no_decay(n) else bas_wd).append(p)
# # #         else:
# # #             (enc_nwd if _no_decay(n) else enc_wd).append(p)

# # #     groups = [
# # #         {"params": vel_wd,  "lr": lr_velocity, "weight_decay": wd_velocity, "name": "vel_wd"},
# # #         {"params": vel_nwd, "lr": lr_velocity, "weight_decay": 0.0,         "name": "vel_nwd"},
# # #         {"params": enc_wd,  "lr": lr_encoder,  "weight_decay": wd_encoder,  "name": "enc_wd"},
# # #         {"params": enc_nwd, "lr": lr_encoder,  "weight_decay": 0.0,         "name": "enc_nwd"},
# # #         {"params": bas_wd,  "lr": lr_velocity * 2, "weight_decay": wd_velocity, "name": "basin_wd"},
# # #         {"params": bas_nwd, "lr": lr_velocity * 2, "weight_decay": 0.0,         "name": "basin_nwd"},
# # #     ]
# # #     # Filter empty groups
# # #     groups = [g for g in groups if len(g["params"]) > 0]
# # #     return optim.AdamW(groups, weight_decay=0.0)  # per-group wd set above

# # # def get_vel_lr(opt):
# # #     for pg in opt.param_groups:
# # #         if pg.get("name") == "vel_wd": return pg["lr"]
# # #     return float("nan")


# # # # ─────────────────────────────────────────────────────────────────────────────
# # # #  Scheduler with Plateau LR
# # # # ─────────────────────────────────────────────────────────────────────────────

# # # class Scheduler:
# # #     """
# # #     Cosine schedule + Plateau LR reduction.
    
# # #     Encoder frozen for freeze_end_ep, then unfrozen with small LR.
# # #     Basin embed always trains (lr = 2× velocity LR).
    
# # #     Plateau: if val ADE no improve plateau_patience epochs → LR × factor.
# # #     Applied to ALL groups proportionally.
# # #     """
# # #     def __init__(self, opt, warmup_epochs, total_epochs,
# # #                  lr_vel, lr_vel_min, freeze_end_ep,
# # #                  lr_enc_peak, encoder_warmup_epochs=5,
# # #                  plateau_patience=15, plateau_factor=0.5, plateau_min_delta=0.5):
# # #         self.opt = opt; self.warmup = warmup_epochs; self.total = total_epochs
# # #         self.lr_vel = lr_vel; self.lr_vel_min = lr_vel_min
# # #         self.freeze_end = freeze_end_ep; self.lr_enc_peak = lr_enc_peak
# # #         self.enc_warmup = encoder_warmup_epochs; self.epoch = 0
# # #         self.plateau_patience = plateau_patience; self.plateau_factor = plateau_factor
# # #         self.plateau_min_delta = plateau_min_delta
# # #         self._plateau_counter = 0; self._best_val = float("inf")
...
# # #     def step(self):
# # #         ep = self.epoch
# # #         lr_vel = (self.lr_vel*(0.1+0.9*ep/max(self.warmup-1,1)) if ep < self.warmup
# # #                   else self._cosine(self.warmup, self.total, self.lr_vel, self.lr_vel_min, ep))
# # #         lr_enc = (0.0 if ep < self.freeze_end
# # #                   else self.lr_enc_peak*(ep-self.freeze_end)/self.enc_warmup
# # #                   if ep < self.freeze_end + self.enc_warmup
# # #                   else self._cosine(self.freeze_end+self.enc_warmup, self.total,
# # #                                     self.lr_enc_peak, self.lr_vel_min, ep))
# # #         # Apply plateau scale (floor at lr_min)
# # #         scale = self._plateau_scale
# # #         lr_vel = max(lr_vel * scale, self.lr_vel_min)
# # #         lr_enc = max(lr_enc * scale, self.lr_vel_min) if lr_enc > 0 else 0.0
# # #         for pg in self.opt.param_groups:
# # #             name = pg.get("name", "")
# # #             if "enc" in name: pg["lr"] = lr_enc
# # #             elif "basin" in name: pg["lr"] = min(lr_vel * 2, 5e-4)
# # #             else: pg["lr"] = lr_vel
# # #         self.epoch += 1
# # #         return lr_vel, lr_enc

# # #     def report_val_ade(self, val_ade, ep, min_ep=30):
# # #         if ep < min_ep: return False
# # #         if val_ade < self._best_val - self.plateau_min_delta:
# # #             self._best_val = val_ade; self._plateau_counter = 0
# # #             return False
# # #         self._plateau_counter += 1
# # #         if self._plateau_counter >= self.plateau_patience:
# # #             self._plateau_scale *= self.plateau_factor
# # #             self._plateau_counter = 0
# # #             new_lr = max(self.lr_vel * self._plateau_scale, self.lr_vel_min)
# # #             print(f"  [Plateau ep{ep}] val stagnant {self.plateau_patience}ep "
# # #                   f"→ LR×{self.plateau_factor} → vel={new_lr:.2e}")
# # #             return True
# # #         return False


# # # # ─────────────────────────────────────────────────────────────────────────────
# # # #  SWA handler — identical to v2.7
# # # # ─────────────────────────────────────────────────────────────────────────────

# # # class SWAHandler:
# # #     def __init__(self, swa_lr=2e-6):
# # #         self.swa_lr=swa_lr; self.active=False; self.start_ep=None
# # #         self.n_updates=0; self.avg_state={}

# # #     def should_activate(self, ade_history, window=3, threshold=1.5):
# # #         return len(ade_history) >= window and (ade_history[-window]-ade_history[-1]) < threshold

# # #     def activate(self, model, opt, ep):
# # #         self.active=True; self.start_ep=ep
# # #         for pg in opt.param_groups: pg["lr"] = self.swa_lr
# # #         m = _unwrap(model)
# # #         self.avg_state = {k: v.detach().clone().float()
...
# # #         self.n_updates = 1
# # #         print(f"  *** SWA @ ep{ep} (lr→{self.swa_lr:.1e}) ***")
...
# # #     def save_avg_state(self, path, epoch, best_score, extra=None):
# # #         payload = {"epoch":epoch,"model":self.avg_state,"best_score":best_score,
# # #                    "is_swa":True,"swa_updates":self.n_updates}
# # #         if extra: payload.update(extra)
# # #         torch.save(payload, path)
...
# # # @torch.no_grad()
# # # def evaluate_hard_val(model, val_loader, device, hard_threshold=0.35, n_ensemble=20, ema=None):
# # #     bk = None
# # #     if ema:
# # #         try: bk = ema.apply_to(model)
# # #         except: pass
# # #     model.eval()
# # #     all_ade, all_ate, all_cte = [], [], []; n_hard = 0
# # #     for batch in val_loader:
# # #         bl = move(list(batch), device); B = bl[0].shape[1]
# # #         h_score = hard_score_from_obs(bl[0][:,:,:2])
# # #         hard_mask = h_score > hard_threshold
# # #         if hard_mask.sum() == 0: continue
# # #         hard_idx = hard_mask.nonzero(as_tuple=True)[0]
...
# # #         for i, item in enumerate(bl_h):
# # #             if torch.is_tensor(item):
# # #                 if item.dim()>=2 and item.shape[1]==B: bl_h[i]=item[:,hard_idx,...]
# # #                 elif item.dim()>=1 and item.shape[0]==B: bl_h[i]=item[hard_idx,...]
# # #         try: pred,_,_ = model.sample(bl_h, num_ensemble=n_ensemble)
# # #         except: continue
# # #         gt=bl_h[1]; T=min(pred.shape[0],gt.shape[0])
# # #         pd=_norm_to_deg(pred[:T]); gd=_norm_to_deg(gt[:T])
# # #         dist=_haversine_deg(pd,gd); ate,cte=_ate_cte(pd,gd)
# # #         all_ade.extend(dist.mean(0).tolist()); n_hard+=len(hard_idx)
# # #         if ate.shape[0]>0: all_ate.extend(ate.abs().mean(0).tolist()); all_cte.extend(cte.abs().mean(0).tolist())
# # #     if bk:
# # #         try: ema.restore(model, bk)
# # #         except: pass
# # #     def _m(lst): return float(np.mean(lst)) if lst else float("nan")
# # #     return {"ADE":_m(all_ade),"ATE":_m(all_ate),"CTE":_m(all_cte),
# # #             "n_hard":n_hard,
# # #             "combined_score":0.6*_m(all_ade)+0.2*_m(all_ate)+0.2*_m(all_cte)}


# # # @torch.no_grad()
# # # def evaluate(model, loader, device, tag="", n_ensemble=20, ema=None,
# # #              ref_targets=None, epoch_for_loss=9999,
# # #              run_xai=False, xai_batch=None) -> Dict:
# # #     bk = None
# # #     if ema:
# # #         try: bk = ema.apply_to(model)
...
# # #     all_ade, all_ate, all_cte = [], [], []
# # #     step_dist = defaultdict(list)
# # #     sum_loss=sum_cfm=sum_accel=0.0; sum_n=0

# # #     for batch in loader:
# # #         bl = move(list(batch), device); gt=bl[1]; B=bl[0].shape[1]
# # #         try:
# # #             bd = model.get_loss_breakdown(bl, epoch=epoch_for_loss)
# # #             if torch.isfinite(bd["total"]):
# # #                 sum_loss+=bd["total"].item()*B; sum_cfm+=bd["l_cfm"]*B
# # #                 sum_accel+=bd.get("l_accel",0.)*B; sum_n+=B
# # #         except: pass
# # #         try: pred,_,_ = model.sample(bl, num_ensemble=n_ensemble)
# # #         except Exception as e: print(f"  sample err:{e}"); continue
# # #         T=min(pred.shape[0],gt.shape[0])
# # #         pd=_norm_to_deg(pred[:T]); gd=_norm_to_deg(gt[:T])
# # #         dist=_haversine_deg(pd,gd); ate,cte=_ate_cte(pd,gd)
# # #         all_ade.extend(dist.mean(0).tolist())
...
# # #     def _m(lst): return float(np.mean(lst)) if lst else float("nan")
# # #     result = {"ADE":_m(all_ade),"ATE":_m(all_ate),"CTE":_m(all_cte),
# # #               "n":len(all_ade), "val_loss":sum_loss/max(sum_n,1),
# # #               "val_cfm_loss":sum_cfm/max(sum_n,1),
# # #               "val_accel_loss":sum_accel/max(sum_n,1),
# # #               "combined_score":0.6*_m(all_ade)+0.2*_m(all_ate)+0.2*_m(all_cte),
# # #               **{f"{h}h":_m(step_dist[h]) for h in [12,24,48,72]}}
# # #     ref = ref_targets or {}
# # #     def _c(v,r): return f"({v-r:+.1f}vs_ST)" if r and np.isfinite(v) and np.isfinite(r) else ""
# # #     print(f"\n  [{tag}] n={result['n']}"
# # #           f"  ADE={result['ADE']:.1f}{_c(result['ADE'],ref.get('ADE'))}"
# # #           f"  ATE={result['ATE']:.1f}{_c(result['ATE'],ref.get('ATE'))}"
# # #           f"  CTE={result['CTE']:.1f}{_c(result['CTE'],ref.get('CTE'))}"
# # #           f"  loss={result['val_loss']:.5f}  accel={result['val_accel_loss']:.5f}")
# # #     print(f"  12h={result['12h']:.1f}  24h={result['24h']:.1f}"
# # #           f"  48h={result['48h']:.1f}  72h={result['72h']:.1f}")
# # #     if run_xai and xai_batch is not None:
# # #         try:
# # #             pred_x,_,_,xai = model.sample(xai_batch, num_ensemble=n_ensemble, return_xai=True)
# # #             sc = xai.get("speed_comparison",{})
# # #             print(f"  [XAI-5] obs={sc.get('obs_speed_mean',float('nan')):.1f}"
# # #                   f"  pred={sc.get('pred_speed_mean',float('nan')):.1f}"
# # #                   f"  ratio={sc.get('speed_ratio',float('nan')):.3f}")
# # #             hd = xai.get("heading_deviation_deg")
# # #             if hd is not None and hd.numel()>0:
# # #                 s72 = min(hd.shape[0]-1, 10)
# # #                 print(f"  [XAI-6] 12h={float(hd[min(1,hd.shape[0]-1)].mean()):.1f}°"
# # #                       f"  72h={float(hd[s72].mean()):.1f}°")
# # #             atc = xai.get("ate_cte_decomp",{})
# # #             print(f"  [XAI-7] ATE={atc.get('ate_abs_mean',float('nan')):.1f}"
# # #                   f"  CTE={atc.get('cte_abs_mean',float('nan')):.1f}")
# # #             print(f"  [XAI-B] basin_ids={xai.get('basin_ids',[][:8])}")
# # #         except Exception as e: print(f"  XAI err:{e}")
# # #     return result


# # # def _save(path, epoch, model, opt, sched, best_score, ema=None, scaler=None, extra=None):
# # #     m = _unwrap(model)
# # #     esd = None
# # #     if ema:
# # #         try: esd = {k:v.cpu().clone() for k,v in ema.shadow.items()}
# # #         except: pass
# # #     payload = {"epoch":epoch,"model":m.state_dict(),"optimizer":opt.state_dict(),
# # #                "scheduler":sched.epoch,"scaler":scaler.state_dict() if scaler else None,
# # #                "best_score":best_score,"best_ade":best_score,"ema":esd}
# # #     if extra: payload.update(extra)
# # #     torch.save(payload, path)
...
# # #     p.add_argument("--step_w_smooth",         default=0.1,   type=float)
# # #     p.add_argument("--obs_dropout_p",         default=0.15,  type=float)
# # #     p.add_argument("--fno3d_dropout",         default=0.20,  type=float,
# # #                    help="FNO3D dropout (patched after load, was 0.05)")
# # #     p.add_argument("--use_ot",                default=True,  action="store_true")
# # #     p.add_argument("--no_ot",                 dest="use_ot", action="store_false")
# # #     p.add_argument("--ot_epsilon",            default=0.05,  type=float)
# # #     p.add_argument("--n_ensemble",            default=20,    type=int)
...
# # #     p.add_argument("--warmup_epochs",         default=5,     type=int)
# # #     p.add_argument("--weight_decay",          default=1e-4,  type=float,
# # #                    help="velocity weight decay")
# # #     p.add_argument("--enc_weight_decay",      default=5e-4,  type=float,
# # #                    help="encoder weight decay (aggressive for FNO3D)")
# # #     p.add_argument("--grad_clip",             default=1.0,   type=float)
# # #     p.add_argument("--use_amp",               action="store_true", default=False)
# # #     p.add_argument("--use_ema",               default=True,  action="store_true")
# # #     p.add_argument("--no_ema",                dest="use_ema",action="store_false")
# # #     # Encoder
# # #     p.add_argument("--freeze_encoder_epochs", default=25,    type=int,
# # #                    help="was 10, now 25 to prevent encoder overfit")
# # #     p.add_argument("--encoder_warmup_epochs", default=5,     type=int)
# # #     p.add_argument("--lr_enc_peak",           default=2e-5,  type=float,
# # #                    help="was 5e-5, now 2e-5 — train encoder slower")
# # #     # Plateau
# # #     p.add_argument("--plateau_patience",      default=15,    type=int)
# # #     p.add_argument("--plateau_factor",        default=0.5,   type=float)
# # #     p.add_argument("--plateau_min_delta",     default=0.5,   type=float)
# # #     # Eval
# # #     p.add_argument("--val_freq",              default=5,     type=int)
# # #     p.add_argument("--patience",              default=40,    type=int)
# # #     p.add_argument("--min_ep",                default=40,    type=int)
# # #     p.add_argument("--hard_val_threshold",    default=0.35,  type=float)
# # #     p.add_argument("--hard_val_freq",         default=10,    type=int)
# # #     # SWA
# # #     p.add_argument("--swa_lr",               default=2e-6,  type=float)
# # #     p.add_argument("--swa_window",           default=3,     type=int)
# # #     p.add_argument("--swa_threshold",        default=1.5,   type=float)
# # #     p.add_argument("--swa_min_ep",           default=80,    type=int)
# # #     # Test
# # #     p.add_argument("--multiscale_test",      default=True,  action="store_true")
# # #     # IO
# # #     p.add_argument("--output_dir",           default="runs/fm_v29")
# # #     p.add_argument("--gpu_num",              default="0")
# # #     p.add_argument("--resume",               default=None)
# # #     p.add_argument("--test_at_end",          action="store_true", default=True)
# # #     p.add_argument("--no_test",              dest="test_at_end", action="store_false")
...
# # #     print("=" * 72)
# # #     print("  TC-FlowMatching v2.9 — Triệt để 3 nguồn gốc gap")
# # #     print(f"  Source1(FNO3D): dropout={args.fno3d_dropout} freeze={args.freeze_encoder_epochs}ep"
# # #           f" enc_wd={args.enc_weight_decay} lr_enc={args.lr_enc_peak:.0e}")
# # #     print(f"  Source2(Speed): L_accel λ={args.lambda_accel} obs_drop={args.obs_dropout_p}")
# # #     print(f"  Source3(Basin): basin_embed 6×32 (in model)")
# # #     print(f"  Protocol: {args.num_epochs}ep plateau={args.plateau_patience}ep"
# # #           f" accum={args.grad_accum}×{args.batch_size}={args.grad_accum*args.batch_size}eff")
# # #     print("=" * 72)
...
# # #     try:    xai_batch = move(list(next(iter(val_loader))), device)
# # #     except: xai_batch = None

# # #     # ── Model ─────────────────────────────────────────────────────────────
# # #     model = TCFlowMatching(
# # #         pred_len=args.pred_len, obs_len=args.obs_len, unet_in_ch=args.unet_in_ch,
# # #         d_cond=args.d_cond, d_model=args.d_model, nhead=args.nhead,
# # #         num_dec_layers=args.num_dec_layers, dim_ff=args.dim_ff, dropout=args.dropout,
# # #         sigma_min=args.sigma_min, sigma_max=args.sigma_max,
# # #         lambda_reg=args.lambda_reg, lambda_accel=args.lambda_accel,
# # #         use_ot=args.use_ot, ot_epsilon=args.ot_epsilon,
# # #         use_ema=args.use_ema, n_ensemble=args.n_ensemble,
# # #         n_inference_steps=args.n_inference_steps, sigma_inference=args.sigma_inference,
# # #         step_w_smooth=args.step_w_smooth, obs_dropout_p=args.obs_dropout_p,
# # #     ).to(device)

# # #     # Patch FNO3D dropout (Source 1 fix)
# # #     patch_fno3d_dropout(model, args.fno3d_dropout)
...
# # #     n_enc = sum(p.numel() for p in raw.encoder.parameters())
# # #     n_vel = sum(p.numel() for p in raw.velocity.parameters())
# # #     print(f"\n  Encoder:{n_enc:,}  Velocity:{n_vel:,}  Total:{n_enc+n_vel:,}")

# # #     # ── Optimizer ─────────────────────────────────────────────────────────
# # #     opt    = build_optimizer(model, lr_velocity=args.lr, lr_encoder=0.0,
# # #                              wd_velocity=args.weight_decay, wd_encoder=args.enc_weight_decay)
# # #     scaler = GradScaler("cuda", enabled=args.use_amp)
# # #     sched  = Scheduler(opt=opt, warmup_epochs=args.warmup_epochs,
...
# # #     swa = SWAHandler(swa_lr=args.swa_lr)
# # #     print(f"  Optimizer groups: {[pg['name'] for pg in opt.param_groups]}")
...
# # #         # Freeze/unfreeze encoder (basin_embed always trains)
# # #         for n, p in raw_m.encoder.named_parameters():
# # #             if "basin_embed" in n: p.requires_grad_(True)
# # #             else: p.requires_grad_(not freeze)

# # #         if rel_ep == 0 and freeze:           print(f"  *** Ep{ep}: encoder frozen (basin_embed active) ***")
# # #         if rel_ep == args.freeze_encoder_epochs: print(f"\n  *** Ep{ep}: encoder unfrozen ***")

# # #         model.train()
# # #         sum_loss=sum_cfm=sum_reg=sum_acc=sum_ade1=0.0; t0_ep=time.perf_counter()
# # #         opt.zero_grad()
...
# # #             with autocast(device_type="cuda", enabled=args.use_amp):
# # #                 bd = model.get_loss_breakdown(bl_aug, epoch=ep)
# # #                 loss_scaled = bd["total"] / args.grad_accum

# # #             scaler.scale(loss_scaled).backward()

# # #             sum_loss+=bd["total"].item(); sum_cfm+=bd["l_cfm"]
# # #             sum_reg+=bd["l_reg"]; sum_acc+=bd.get("l_accel",0.); sum_ade1+=bd["ade_1step"]

# # #             # Gradient accumulation step
# # #             if (i + 1) % args.grad_accum == 0 or (i + 1) == nstep:
# # #                 scaler.unscale_(opt)
# # #                 torch.nn.utils.clip_grad_norm_(model.parameters(), args.grad_clip)
# # #                 # Zero frozen encoder grads (basin_embed still trains)
# # #                 if freeze:
# # #                     for n, p in raw_m.encoder.named_parameters():
# # #                         if "basin_embed" not in n and p.grad is not None:
# # #                             p.grad.zero_()
# # #                 scaler.step(opt); scaler.update(); opt.zero_grad()
# # #                 model.ema_update(); swa.update(model)
...
# # #                       f"  cfm={bd['l_cfm']:.4f}"
# # #                       f"  reg={bd['l_reg']:.4f}"
# # #                       f"  acc={bd.get('l_accel',0.):.4f}(λ={bd.get('lam_accel',0.):.2f})"
# # #                       f"  ade1={bd['ade_1step']:.0f}km"
# # #                       f"  enc={'frozen' if freeze else 'active'}"
# # #                       f"  lr={lr_v:.2e}")
...
# # #             sched.report_val_ade(val_ade, ep, min_ep=args.min_ep)

# # #             if len(val_ade_history) >= 4:
# # #                 trend = float(np.mean(val_ade_history[-2:])) - float(np.mean(val_ade_history[-4:-2]))
# # #                 ts = (f"↑{trend:+.1f}⚠" if trend>5 else f"↓{trend:+.1f}✓" if trend<-5 else f"→{trend:+.1f}")
# # #             else: ts = "—"
# # #             print(f"  val_ADE={val_ade:.1f}  combined={score:.1f}  trend={ts}")

# # #             if not swa.active and ep >= args.swa_min_ep and \
# # #                swa.should_activate(val_ade_history, args.swa_window, args.swa_threshold):
# # #                 swa.activate(model, opt, ep)

# # #             if score < best_score:
# # #                 best_score=score; patience_cnt=0
# # #                 _save(best_ckpt, ep, model, opt, sched, best_score, ema, scaler,
# # #                       extra={"val_ade":r["ADE"],"val_ate":r["ATE"],"val_cte":r["CTE"]})
# # #                 print(f"  ✅ Best! ADE={r['ADE']:.1f} ATE={r['ATE']:.1f} CTE={r['CTE']:.1f}")
# # #             else:
# # #                 if rel_ep >= args.min_ep: patience_cnt += args.val_freq
...
# # #                                      hard_threshold=args.hard_val_threshold,
# # #                                      n_ensemble=args.n_ensemble, ema=ema)
# # #             print(f"  [HVAL] n={r_h['n_hard']} ADE={r_h['ADE']:.1f}"
# # #                   f" ATE={r_h['ATE']:.1f} CTE={r_h['CTE']:.1f}")
# # #             if r_h["combined_score"] < best_hard and r_h["n_hard"] >= 10:
# # #                 best_hard = r_h["combined_score"]
# # #                 _save(hard_best_ckpt, ep, model, opt, sched, best_hard, ema, scaler,
# # #                       extra={"hard_val_ade":r_h["ADE"]})
# # #                 print(f"  💎 Hard-best! ADE={r_h['ADE']:.1f}")
...
# # #             swa.restore_from_backup(model, backup)
# # #             swa_score = r_swa["combined_score"]
# # #             print(f"  [SWA] score={swa_score:.2f} updates={swa.n_updates}")
# # #             if swa_score < best_score:
# # #                 best_score=swa_score; patience_cnt=0
# # #                 swa.save_avg_state(swa_ckpt, ep, best_score,
# # #                                    extra={"val_ade":r_swa["ADE"],"val_ate":r_swa["ATE"],"val_cte":r_swa["CTE"]})
# # #                 import shutil; shutil.copy(swa_ckpt, best_ckpt)
# # #                 print(f"  ✅ SWA best! ADE={r_swa['ADE']:.1f}")

# # #     print(f"\n  Done! best={best_score:.2f}")
# # #     if not args.test_at_end: return

# # #     if not os.path.exists(best_ckpt): print("  No checkpoint."); return
# # #     ck = torch.load(best_ckpt, map_location=device)
# # #     is_swa = ck.get("is_swa", False)
# # #     _unwrap(model).load_state_dict(ck["model"], strict=False)
# # #     if not is_swa and ema and ck.get("ema"):
...
# # #     r_test = evaluate(model, test_loader, device, tag="TEST",
# # #                       n_ensemble=args.n_ensemble, ema=None if is_swa else ema,
# # #                       ref_targets=ST_TRANS_TEST, run_xai=True, xai_batch=xai_batch)

# # #     if args.multiscale_test:
# # #         raw_m = _unwrap(model); raw_m.eval()
# # #         ms_ades, ms_ates, ms_ctes = [], [], []
# # #         bk_ms = (ema.apply_to(model) if (ema and not is_swa) else None)
# # #         with torch.no_grad():
# # #             for batch in test_loader:
# # #                 bl = move(list(batch), device)
# # #                 try: pred,_,_ = raw_m.sample_multiscale(bl)
# # #                 except: continue
# # #                 gt=bl[1]; T=min(pred.shape[0],gt.shape[0])
# # #                 pd=_norm_to_deg(pred[:T]); gd=_norm_to_deg(gt[:T])
# # #                 dist=_haversine_deg(pd,gd); ate,cte=_ate_cte(pd,gd)
# # #                 ms_ades.extend(dist.mean(0).tolist())
# # #                 if ate.shape[0]>0: ms_ates.extend(ate.abs().mean(0).tolist()); ms_ctes.extend(cte.abs().mean(0).tolist())
# # #         if bk_ms: ema.restore(model, bk_ms)
# # #         def _mm(lst): return float(np.mean(lst)) if lst else float("nan")
# # #         print(f"  Multi-scale: ADE={_mm(ms_ades):.1f} ATE={_mm(ms_ates):.1f} CTE={_mm(ms_ctes):.1f}")
...
# # #     val_ate_b = ck.get("val_ate", float("nan"))
# # #     val_cte_b = ck.get("val_cte", float("nan"))
# # #     v21 = {"ADE":229.8,"ATE":214.4,"CTE":71.6}; st = ST_TRANS_TEST
# # #     print("\n" + "="*72)
# # #     print("  v2.9 FINAL RESULTS")
# # #     print("="*72)
# # #     print(f"  {'Metric':<8} {'Val':>8} {'Test':>8} {'Gap':>8} | {'v2.1':>8} {'ST-Trans':>10}")
# # #     print("  " + "─"*58)
# # #     for m_n, val_v, test_v, v21_v, st_v in [
# # #         ("ADE", val_ade_b, r_test["ADE"], v21["ADE"], st["ADE"]),
# # #         ("ATE", val_ate_b, r_test["ATE"], v21["ATE"], st["ATE"]),
# # #         ("CTE", val_cte_b, r_test["CTE"], v21["CTE"], st["CTE"]),
# # #     ]:
# # #         gap  = test_v - val_v if (np.isfinite(test_v) and np.isfinite(val_v)) else float("nan")
# # #         beat = "✅BEAT" if (np.isfinite(test_v) and np.isfinite(st_v) and test_v<st_v) else "❌"
# # #         print(f"  {m_n:<8} {val_v:>8.1f} {test_v:>8.1f} {gap:>+8.1f} | {v21_v:>8.1f} {st_v:>10.1f} {beat}")
# # #     print("="*72)
