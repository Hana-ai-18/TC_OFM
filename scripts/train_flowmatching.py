...
FIXES vs v24:
  ...
  FIX-T25-A  [CRITICAL] freeze_backbone() gọi đúng hàm từ model module
             (không dùng string matching). Giờ thực sự đóng băng
             net.spatial_enc + net.enc_1d + net.env_enc (~12M params).
             Phase-1 chỉ train ShortRangeHead + ctx_fc + transformer.

  FIX-T25-B  [CRITICAL] Sửa thứ tự scheduler.step(): optimizer.step()
             TRƯỚC scheduler.step(). Warmup LR giờ hoạt động đúng.

  FIX-T25-C  [HIGH] n_train_ens tối thiểu = 4 ngay từ epoch 0
             (thay vì 1). CRPS/AFCRPS cần >=2 samples để tính pairwise
             diversity. Giữ progressive schedule nhưng min=4.

  FIX-T25-D  [MEDIUM] val_ade_freq default = 3 (thay vì 1) để giảm
             thời gian eval từ ~5h xuống ~1.7h, dành GPU cho training.

  FIX-T25-E  [MEDIUM] BestModelSaver tolerance: sr_tol=5.0 km (thay vì
             3.0 km) ở phase 1 để tránh early stop sai khi SR-12h còn
             đang học. Reset về 3.0 km khi chuyển sang phase 2.

  FIX-T25-F  [MEDIUM] Phase-1 optimizer chỉ train params có
             requires_grad=True. Kết hợp FIX-T25-A → phase-1 LR=5e-4
             chỉ áp dụng cho ~2.4M trainable params, không phá FNO3D.

  FIX-T25-G  [LOW] Log pinn saturation warning: nếu pinn_val > 9.5
             (gần max_val=10) in cảnh báo. Cho phép monitor FIX-L54.

Kept from v24:
  FIX-T24-A  Two-phase training concept
  FIX-T24-B  Primary metric = SR-12h ADE
  FIX-T24-C  short_range_weight schedule 8→5→3
  FIX-T24-D  initial_sample_sigma=0.03, spread_weight↑
  FIX-T24-E  pinn_warmup=80 ep
  FIX-T24-F  evaluate SR-12h/24h separately
  FIX-T23-6  patience=15
"""
from __future__ import annotations
...
from Model.data.loader_training import data_loader
from Model.flow_matching_model import TCFlowMatching, ShortRangeHead
from Model.flow_matching_model import freeze_backbone, unfreeze_all  # FIX-T25-A
from Model.utils import get_cosine_schedule_with_warmup
from Model.losses import WEIGHTS as _BASE_WEIGHTS
from utils.metrics import (
    TCEvaluator, StepErrorAccumulator,
...
def get_progressive_ens(epoch: int, n_train_ens: int = 6) -> int:
    """
    FIX-T25-C: minimum ensemble = 4 (not 1) from epoch 0.
    Need >= 2 for pairwise CRPS; 4 gives better calibration from start.
    """
    if epoch < 20:
        return min(4, n_train_ens)
    elif epoch < 50:
        return min(max(4, n_train_ens // 2), n_train_ens)
    else:
        return n_train_ens
...
def compute_sr_ade(model: TCFlowMatching,
                   batch_list: list,
                   device) -> tuple[float, float]:
    """Compute 12h and 24h ADE from ShortRangeHead."""
    obs_t   = batch_list[0]
    traj_gt = batch_list[1]
...
def evaluate_fast(model, loader, device, ode_steps, pred_len, fast_ensemble=8):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            # FIX-T25-G: track pinn value
            try:
                from Model.losses import pinn_bve_loss
                pred_deg = pred.clone()
                pred_deg[..., 0] = (pred[..., 0] * 50.0 + 1800.0) / 10.0
                pred_deg[..., 1] = (pred[..., 1] * 50.0) / 10.0
                pv = pinn_bve_loss(pred_deg, bl).item()
                pinn_buf.append(pv)
            except Exception:
                pass
    ...
    # FIX-T25-G: pinn saturation warning
    if pinn_buf:
        pinn_mean = float(np.mean(pinn_buf))
        r["pinn_mean"] = pinn_mean
        if pinn_mean > 9.5:
            print(f"  ⚠️  PINN SATURATING: pinn_mean={pinn_mean:.3f} > 9.5 "
                  f"(max_val=10). Consider reducing pinn weight.")
...
def evaluate_full_val_ade(model, val_loader, device, ode_steps, pred_len,
    ...
    # FIX-T25-G: saturation warning in full eval too
    if pinn_buf and float(np.mean(pinn_buf)) > 9.5:
        print(f"  ⚠️  PINN SATURATING in full val: {np.mean(pinn_buf):.3f}")
...
class BestModelSaver:
    """
    Primary metric = ShortRangeHead 12h ADE.
    FIX-T25-E: sr_tol=5.0 km in phase 1, reset to 3.0 km in phase 2.
    """

    def __init__(self, patience: int = 15, sr_tol: float = 5.0,
                 ade_tol: float = 5.0):
        self.patience      = patience
        self.sr_tol        = sr_tol
        self.ade_tol       = ade_tol
    ...
    def tighten_tolerance(self, new_sr_tol: float = 3.0) -> None:
        """FIX-T25-E: called at phase 2 start to tighten tolerance."""
        self.sr_tol = new_sr_tol
        print(f"  [SAVER] Tolerance tightened: sr_tol → {new_sr_tol} km")
    ...
    def log_subset_sr(self, sr_12h: float, sr_24h: float, epoch: int):
        hit12 = "  🎯 <50km!" if sr_12h < 50.0 else ""
        hit24 = "  🎯 <100km!" if sr_24h < 100.0 else ""
        print(f"  [SUBSET SR ep{epoch}]  12h={sr_12h:.1f}  24h={sr_24h:.1f} km"
              f"  [target <50/<100]{hit12}{hit24}  (monitor only)")
...
def get_args():
    ...
    p.add_argument("--val_loss_freq",    default=1,   type=int)
    p.add_argument("--val_freq",         default=1,   type=int)
    # FIX-T25-D: default val_ade_freq = 3
    p.add_argument("--val_ade_freq",     default=3,   type=int,
                   help="FIX-T25-D: 1→3 to reduce eval overhead")
    p.add_argument("--full_eval_freq",   default=10,  type=int)
    p.add_argument("--val_subset_size",  default=600, type=int)
...
def main(args):
    ...
    print("=" * 70)
    print("  TC-FlowMatching v25  |  ShortRangeHead + OT-CFM + PINN")
    print("  v25 FIXES:")
    print("    FIX-T25-A: freeze_backbone() via module ref (not string match)")
    print("    FIX-T25-B: optimizer.step() BEFORE scheduler.step()")
    print("    FIX-T25-C: n_train_ens >= 4 from epoch 0 (CRPS needs pairs)")
    print("    FIX-T25-D: val_ade_freq=3 (reduce eval overhead)")
    print("    FIX-T25-E: sr_tol=5.0km phase1 → 3.0km phase2")
    print("    FIX-T25-F: phase-1 optimizer only trains trainable params")
    print("    FIX-T25-G: pinn saturation warning (>9.5)")
    print("    FIX-L54:   PINN scale 5e-3, tanh max_val=10 (no saturation)")
    print("    FIX-L55:   sr_loss normalize /100 (correct scale)")
    print("    FIX-L56:   single-sample AFCRPS guard")
    print("    FIX-M28:   freeze correct backbone modules")
    print("    FIX-M29:   step_scale init 0.6 (larger early displacements)")
    print("    FIX-M30:   vel_enc robust for short obs sequences")
    print("    FIX-M31:   all_trajs NOT overridden → correct CRPS/SSR")
    print("    FIX-M32:   ctx_noise per-member for diversity")
    print("=" * 70)
    print(f"  device               : {device}")
    print(f"  phase2_start         : ep {args.phase2_start}")
    print(f"  phase1_lr            : {args.phase1_lr:.2e}")
    ...
    print(f"  ode_steps            : train={ode_train} val={ode_val} test={ode_test}")
    print(f"  patience             : {args.patience}  min_epochs={args.min_epochs}")
    print(f"  val_ade_freq         : {args.val_ade_freq}")
    print()
    ...
    # ── FIX-T25-A: Phase 1 freeze via module references ──────────────────
    freeze_backbone(model)

    # ── FIX-T25-F: optimizer only on trainable params ────────────────────
    ...
    # FIX-T25-E: start with looser tolerance in phase 1
    saver  = BestModelSaver(patience=args.patience, sr_tol=5.0, ade_tol=1.0)
    scaler = GradScaler("cuda", enabled=args.use_amp)
    ...
    last_val_loss = float("inf")
    _phase        = 1
    _prev_ens     = get_progressive_ens(0, args.n_train_ens)
    _lr_ep30_done = False
    _lr_ep60_done = False
    ...
    for epoch in range(args.num_epochs):
        ...
        if epoch == args.phase2_start and _phase == 1:
            _phase = 2
            unfreeze_all(model)
            # FIX-T25-E: tighten tolerance at phase 2 start
            saver.tighten_tolerance(new_sr_tol=3.0)
            # Reset optimizer with all params + phase-2 LR
            optimizer = optim.AdamW(
                model.parameters(),
                lr=args.g_learning_rate,
                weight_decay=args.weight_decay,
        ...
        for i, batch in enumerate(train_loader):
            ...
            if ((i + 1) % args.grad_accum == 0
                ...
                scaler.step(optimizer)
                scaler.update()
                # FIX-T25-B: scheduler.step() AFTER optimizer.step()
                scheduler.step()
                optimizer.zero_grad()
        ...
        print(f"  [FAST ep{epoch} {t_ade_s:.0f}s]"
              ...
              f"{collapse}{hi_sprd}")
        print(f"         ShortRange: SR-12h={sr_12h:.1f} km"
              f"  SR-24h={sr_24h:.1f} km  (monitor only)")

        saver.log_subset_sr(sr_12h, sr_24h, epoch)

        # ── Full val ADE (FIX-T25-D: every val_ade_freq epochs) ──────────
        if epoch % args.val_ade_freq == 0:
            try:
                r_full = evaluate_full_val_ade(
                    model, val_loader, device,
