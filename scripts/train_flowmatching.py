...
# """
# scripts/train_flowmatching.py  ── v16
# ======================================
# FIXES vs v15:

# FIX-V16-1  Early stopping quá aggressive:
#            - patience=30 nhưng val_freq=2 → 60 epoch tolerance.
#            - Tăng patience lên 50 (= 100 epoch tolerance).
#            - BestModelSaver: chỉ tăng counter khi ADE improvement < 5km
#              (không phải bất kỳ improvement nào).
#            - Thêm min_epochs=80 — không được stop trước epoch 80
#              bất kể metrics ra sao.

# FIX-V16-2  Curriculum bật mặc định (--curriculum):
#            Dữ liệu TC ngắn hạn (6-12h) nhiều hơn và dễ học hơn.
#            Curriculum giúp mô hình build foundation trước khi học 72h.
#            Mặc định: curriculum=True, curriculum_start_len=4, end_epoch=40.

# FIX-V16-3  Velocity loss integration:
#            Thêm velocity và accel vào log output và adaptive weighting.
#            velocity_w tăng từ 0.5 → 1.5 song song với pinn_w.
#            Điều này trực tiếp fix ATE >> CTE.

# FIX-V16-4  val_subset_size tăng 400→600 để ADE estimate ổn định hơn.
#            Noisy ADE từ subset nhỏ gây early stop sớm.

# FIX-V16-5  Logging cải thiện: in đủ loss components mỗi 20 batch
#            (thay vì 40) để dễ debug hơn.

# FIX-V16-6  TSS/DTW/BSS NaN: đảm bảo evaluate_full luôn set
#            ev.cliper_ugde trước khi gọi ev.compute().
#            Kiểm tra lại BSS computation — thêm fallback khi LANDFALL_TARGETS rỗng.

# Kept from v15:
#     FIX-V15-1..V15-8 (TSS, DTW, BSS, dual patience, adaptive PINN,
#                       SSR ensemble, LR warmup, val_subset_size)
# """
# from __future__ import annotations
...
# def get_velocity_weight(epoch: int, warmup_epochs: int = 20,
#                         w_start: float = 0.5, w_end: float = 1.5) -> float:
#     """
#     FIX-V16-3: Ramp velocity weight từ 0.5→1.5 trong 20 epoch đầu.
#     Giúp mô hình học speed sớm, trước khi PINN constraint kick in.
#     """
#     if epoch >= warmup_epochs:
#         return w_end
#     alpha = epoch / max(warmup_epochs - 1, 1)
...
#     p.add_argument("--grad_clip",       default=1.0,            type=float)
#     p.add_argument("--grad_accum",      default=2,              type=int)
#     # FIX-V16-1: patience 30→50
#     p.add_argument("--patience",        default=50,             type=int)
#     # FIX-V16-1: min epochs trước khi được phép early stop
...
#     p.add_argument("--val_freq",        default=2,              type=int)
#     p.add_argument("--full_eval_freq",  default=10,             type=int)
#     # FIX-V16-4: val_subset_size 400→600
#     p.add_argument("--val_subset_size", default=600,            type=int)
#     p.add_argument("--output_dir",      default="runs/v16",     type=str)
#     p.add_argument("--save_interval",   default=10,             type=int)
#     p.add_argument("--metrics_csv",     default="metrics.csv",        type=str)
#     p.add_argument("--predict_csv",     default="predictions.csv",    type=str)
#     p.add_argument("--lstm_errors_npy",      default=None, type=str)
...
#     p.add_argument("--threshold",       default=0.002,          type=float)
#     p.add_argument("--other_modal",     default="gph")
#     # FIX-V16-2: curriculum bật mặc định
#     p.add_argument("--curriculum",      default=True,           type=lambda x: x.lower() != 'false')
...
#     p.add_argument("--pinn_w_start",    default=0.01,           type=float)
#     p.add_argument("--pinn_w_end",      default=0.1,            type=float)
#     # FIX-V16-3: velocity weight warmup
...
#                   metrics_csv, tag="", predict_csv=""):
#     """
#     FIX-V16-6: Đảm bảo cliper_ugde được set trước ev.compute().
#     FIX-V15-1..3: TSS/DTW/BSS được tính đúng.
#     """
#     model.eval()

#     cliper_step_errors: list[np.ndarray] = []

#     ev = TCEvaluator(pred_len=pred_len, compute_dtw=True)
#     obs_seqs_01  = []
#     gt_seqs_01   = []
#     pred_seqs_01 = []
#     ens_seqs_01: list[np.ndarray] = []
...
#     # FIX-V16-6: Set cliper_ugde TRƯỚC khi compute() — đây là key fix TSS
#     if cliper_step_errors:
...
#     # BSS computation — FIX-V16-6: thêm try/except chi tiết hơn
#     try:
...
#                     radius_km=LANDFALL_RADIUS_KM,
#                 )
#                 if not math.isnan(bss_v):
#                     bss_vals.append(bss_v)
#             if bss_vals:
#                 dm.bss_mean = float(np.mean(bss_vals))
#                 print(f"  [BSS] mean={dm.bss_mean:.4f} from {len(bss_vals)} targets")
#     except Exception as e:
#         print(f"  ⚠  BSS computation failed: {e}")
...
# class BestModelSaver:
#     """
#     FIX-V16-1: Dual patience với min_epochs guard.
#     - min_epochs: không được early stop trước epoch này.
#     - counter tăng chỉ khi improvement < ade_tol (5 km).
#     """
#     def __init__(self, patience=50, min_delta=0.1, ade_tol=5.0):
#         self.patience       = patience
#         self.min_delta      = min_delta
#         self.ade_tol        = ade_tol   # FIX-V16-1: minimum meaningful improvement
#         self.best_ade       = float("inf")
#         self.best_val_loss  = float("inf")
#         self.counter_ade    = 0
#         self.counter_loss   = 0
...
#     def update_ade(self, ade: float, model, out_dir, epoch, optimizer, tl, vl,
#                    min_epochs: int = 80):
#         """FIX-V16-1: min_epochs guard — không stop trước epoch min_epochs."""
...
#                 val_loss         = vl,
#                 val_ade_km       = ade,
#                 model_version    = "v16-FNO-Mamba-curriculum",
#             ), os.path.join(out_dir, "best_model.pth"))
#             print(f"  ✅ Best ADE {ade:.1f} km  (epoch {epoch})")
#         else:
#             self.counter_ade += 1
#             improvement = self.best_ade - ade
#             print(f"  No ADE improvement {self.counter_ade}/{self.patience}"
#                   f"  (improvement={improvement:.1f} km < tol={self.ade_tol} km)"
#                   f"  | Loss no-improve {self.counter_loss}/{self.patience}")

#         # FIX-V16-1: Chỉ early stop khi ĐÃ QUA min_epochs VÀ cả hai counter hết
#         if epoch >= min_epochs:
#             if self.counter_ade >= self.patience and self.counter_loss >= self.patience:
#                 self.early_stop = True
#                 print(f"  ⛔ Early stopping: both ADE and val_loss plateaued"
#                       f" (epoch {epoch} >= min_epochs {min_epochs})")
#         else:
#             if self.counter_ade >= self.patience and self.counter_loss >= self.patience:
#                 print(f"  ⚠  Would early stop but min_epochs={min_epochs} not reached"
#                       f" (epoch {epoch}). Continuing...")
#                 # Reset counters để tiếp tục training
#                 self.counter_ade  = 0
#                 self.counter_loss = 0
...
#     print("=" * 68)
#     print("  TC-FlowMatching v16  |  FNO3D + Mamba + OT-CFM + PINN")
#     print("  v16 FIXES: early-stop min_epochs, curriculum default ON,")
#     print("             velocity_loss (ATE fix), adaptive vel_weight,")
#     print("             val_subset_size=600, improved TSS/BSS/DTW")
#     print("=" * 68)
#     print(f"  device          : {device}")
#     print(f"  dataset_root    : {args.dataset_root}")
#     print(f"  num_epochs      : {args.num_epochs}")
#     print(f"  patience        : {args.patience} (dual, min_epochs={args.min_epochs})")
#     print(f"  val_subset_size : {args.val_subset_size}")
#     print(f"  curriculum      : {args.curriculum} "
#           f"(start={args.curriculum_start_len}, end_ep={args.curriculum_end_epoch})")
#     print(f"  pinn_warmup     : {args.pinn_warmup_epochs} epochs "
#           f"({args.pinn_w_start}→{args.pinn_w_end})")
#     print(f"  vel_warmup      : {args.vel_warmup_epochs} epochs "
#           f"({args.vel_w_start}→{args.vel_w_end})")
#     print(f"  fast_ensemble   : {args.fast_ensemble}")
#     print(f"  val_ensemble    : {args.val_ensemble}")
#     print(f"  ode_steps       : {args.ode_steps}")
#     print()
...
#         # Adaptive PINN weight (FIX-V15-5)
#         epoch_weights = copy.copy(_BASE_WEIGHTS)
#         epoch_weights["pinn"] = get_pinn_weight(
#             epoch,
#             warmup_epochs = args.pinn_warmup_epochs,
...
#             w_end         = args.pinn_w_end,
#         )
#         # FIX-V16-3: Adaptive velocity weight
#         epoch_weights["velocity"] = get_velocity_weight(
#             epoch,
#             warmup_epochs = args.vel_warmup_epochs,
#             w_start       = args.vel_w_start,
#             w_end         = args.vel_w_end,
#         )
#         _losses_mod.WEIGHTS["pinn"]     = epoch_weights["pinn"]
#         _losses_mod.WEIGHTS["velocity"] = epoch_weights["velocity"]

#         # LR restart at epoch 30 (FIX-V15-8: 3 epoch warmup)
#         if epoch == 30 and not _lr_restart_ep30_done:
#             _lr_restart_ep30_done = True
#             restart_warmup  = steps_per_epoch * 3
#             remaining_steps = steps_per_epoch * (args.num_epochs - 30)
...
#             # FIX-V16-5: Log mỗi 20 batch (thay vì 40) + thêm velocity/accel
#             if i % 20 == 0:
#                 lr = optimizer.param_groups[0]["lr"]
#                 elapsed = time.perf_counter() - t0
#                 print(f"  [{epoch:>3}/{args.num_epochs}][{i:>3}/{len(train_loader)}]"
...
#                       f"  pinn={bd.get('pinn',0):.3f}"
#                       f"  pinn_w={epoch_weights['pinn']:.3f}"
#                       f"  vel_w={epoch_weights['velocity']:.2f}"
#                       f"  ens={current_ens}  cur_len={curr_len}"
#                       f"  lr={lr:.2e}"
#                       f"  t={elapsed:.0f}s")
...
#         # Val loss (FIX-V15-7: chỉ compute khi val_freq triggers)
#         if epoch % args.val_freq == 0:
#             model.eval()
#             val_loss = 0.0
#             t_val = time.perf_counter()
...
#             print(f"  [ADE eval {t_ade_s:.0f}s]"
#                   f"  ADE={m['ADE']:.1f} km  FDE={m['FDE']:.1f} km"
#                   f"  12h={m.get('12h', 0):.0f} km"
#                   f"  24h={m.get('24h', 0):.0f} km"
#                   f"  72h={m.get('72h', 0):.0f} km"
#                   f"  (ens={effective_fast_ens})")
#             saver.update_ade(m["ADE"], model, args.output_dir, epoch,
#                              optimizer, avg_t, last_val_loss,
...
#         all_results.append(ModelResult(
#             model_name   = "FM+PINN-v16-FNO-Mamba-Curriculum",
#             split        = "test",
#             ADE          = dm_test.ade,
#             FDE          = dm_test.fde,
#             ADE_str      = dm_test.ade_str,
...
#         with open(os.path.join(args.output_dir, "test_results.txt"), "w") as fh:
#             fh.write(dm_test.summary())
#             fh.write(f"\n\nmodel_version   : FM+PINN v16\n")
#             fh.write(f"v16_fixes       : early-stop min_epochs, curriculum ON,\n")
#             fh.write(f"                  velocity_loss, adaptive vel_weight,\n")
#             fh.write(f"                  val_subset=600, TSS/BSS/DTW fix\n")
#             fh.write(f"curriculum      : start={args.curriculum_start_len},"
#                      f" end_ep={args.curriculum_end_epoch}\n")
#             fh.write(f"pinn_weight     : adaptive {args.pinn_w_start}→{args.pinn_w_end}\n")
#             fh.write(f"vel_weight      : adaptive {args.vel_w_start}→{args.vel_w_end}\n")
#             fh.write(f"patience        : {args.patience} (dual, min_epochs={args.min_epochs})\n")
#             fh.write(f"val_subset_size : {args.val_subset_size}\n")
#             fh.write(f"ode_steps       : {args.ode_steps}\n")
#             fh.write(f"eval_ensemble   : {final_ens}\n")
#             fh.write(f"train_time_h    : {total_train_h:.2f}\n")
