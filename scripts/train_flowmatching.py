...
def get_args():
    ...
    p.add_argument("--num_workers",     default=2,              type=int) # FIX-T23-9: num_workers=2 (từ 4) để tránh lỗi file handle khi load gph500
    ...
    p.add_argument("--val_loss_freq",   default=1,              type=int) # FIX-T23-2: val_loss_freq=1 để full val loss mỗi epoch, không bỏ epoch nào.
    p.add_argument("--val_freq",        default=1,              type=int) # FIX-T23-2: val_freq=1 để full val ADE mỗi epoch, không bỏ epoch nào.
    p.add_argument("--val_ade_freq",    default=1,              type=int) # FIX-T23-2: val_ade_freq=1 để full val ADE mỗi epoch, không bỏ epoch nào.
    p.add_argument("--full_eval_freq",  default=10,             type=int)
    p.add_argument("--val_subset_size", default=800,            type=int) # FIX-T23-2: val_subset_size=800 để monitor subset ADE mỗi epoch, không bỏ epoch nào.
