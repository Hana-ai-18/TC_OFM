...
def main(args):
    ...
    print("=" * 72)
    print("  TC-FlowMatching v50  |  ATE/72h Breakthrough Edition")
        ...
        for i, batch in enumerate(train_loader):
            ...
            if i % 20 == 0:
                ...
                print(
                    ...
                    f"  vel={bd.get('vel_reg',  0):.4f}"   # [v50] velocity regression
                    f"  ep={bd.get('endpoint',  0):.4f}"   # [v50] 72h endpoint
                    f"  hdg={bd.get('heading',  0):.4f}"   # [v50] heading cosine
                    f"  fm={bd.get('fm_mse',   0):.4f}"
                    f"  σ={bd.get('sigma',     0):.3f}"
                    f"  lr={lr:.2e}"
                )
