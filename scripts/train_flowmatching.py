...
#     main(args)
"""
Model/flow_matching_model_v48.py — ATE-Focused Edition
═══════════════════════════════════════════════════════════════════════════════

CHANGES vs v47:
  [LOSS-REDESIGN] Loại bỏ chồng chéo, loss rõ ràng từng vai trò:
    L_total = L_DPE                   ← position (primary)
            + 0.40 * L_speed_match    ← tốc độ ratio vs GT (main ATE fix, tăng từ 0.20)
            + 0.25 * L_ate_direct     ← NEW: projection lỗi dọc track → đánh thẳng ATE
            + 0.15 * L_cumul_disp     ← NEW: lỗi tích lũy quãng đường → ATE tích lũy
            + 0.01 * L_accel          ← smoothness (giữ nhỏ, chỉ regularize)

  Đã XÓA (chồng nhau):
    - L_MSE : redundant với L_DPE (cùng supervise vị trí, scale khác)
    - L_speed_prior : redundant với L_speed_match (cùng supervise tốc độ)

  [FIX-KINE] avg_speed_enc: encode global speed stats (mean/std/trend/accel)
             → decoder có context tốt hơn để calibrate tốc độ
             → kine_proj: concat(local 256, global 64) → 256

  [FIX-PERSIST] Weighted persistence init (exp weights, gần cuối nặng hơn)
                thay vì chỉ dùng last-step velocity (noisy)

  [FIX-LOG] Log spd_ratio (phải → 1.0) và ate_direct trong training

CHECKPOINT COMPAT (strict=False vs v47):
  Mismatch mới:
    net.avg_speed_enc.*  : NEW
    net.kine_proj.*      : NEW
  Các layer khác load bình thường từ v47.
"""
from __future__ import annotations
...
# Step weights: tăng trọng số bước đầu (12h/24h) và bước cuối (72h)
...
# ══════════════════════════════════════════════════════════════════════════════
#  ATE / CTE decomposition helpers
# ══════════════════════════════════════════════════════════════════════════════

def _compute_ate_cte(pred_deg, gt_deg):
    """
    Decompose error thành ATE (along-track) và CTE (cross-track).

    Args:
        pred_deg : [T, B, 2]  lon/lat degrees
        gt_deg   : [T, B, 2]
    ...
    Logic:
        - GT direction tại mỗi bước → unit vector dọc track
        - Project error vector lên unit vector đó → ATE
        - Project lên vector vuông góc → CTE
    """
    ...
    # GT direction vectors (metric space)
    gt_dir   = gt_deg[1:T] - gt_deg[:T - 1]            # [T-1, B, 2]
    lat_mid  = gt_deg[:T - 1, :, 1]                    # [T-1, B]
    cos_lat  = torch.cos(torch.deg2rad(lat_mid)).clamp(1e-4)

    gt_km    = torch.stack([
        gt_dir[:, :, 0] * cos_lat * DEG2KM,
        gt_dir[:, :, 1] * DEG2KM,
    ], dim=-1)                                          # [T-1, B, 2]
    gt_norm  = gt_km / gt_km.norm(dim=-1, keepdim=True).clamp(1e-4)  # unit vector

    # Error vectors at forecast positions (NOT diff; absolute position error)
    err_deg  = pred_deg[1:T] - gt_deg[1:T]             # [T-1, B, 2]
    err_km   = torch.stack([
        err_deg[:, :, 0] * cos_lat * DEG2KM,
        err_deg[:, :, 1] * DEG2KM,
    ], dim=-1)                                          # [T-1, B, 2]

    ate = (err_km * gt_norm).sum(-1)                   # [T-1, B] signed
    perp = torch.stack([-gt_norm[:, :, 1], gt_norm[:, :, 0]], dim=-1)
    cte = (err_km * perp).sum(-1)                      # [T-1, B] signed

    return ate, cte


def _compute_cumul_disp_loss(pred_deg, gt_deg):
    """
    Lỗi tích lũy quãng đường từ điểm xuất phát.

    ATE tích lũy = model đi nhanh/chậm hơn GT liên tục →
    cumulative displacement sai → loss này fix gốc rễ.

    Normalize by 150 km² (typical cumul error scale).
    Returns scalar.
    """
    T = min(pred_deg.shape[0], gt_deg.shape[0])
    if T < 2:
    ...
    lat      = gt_deg[:T, :, 1]
    cos_lat  = torch.cos(torch.deg2rad(lat)).clamp(1e-4)  # [T, B]

    # Cumulative displacement from step 0
    pred_d   = pred_deg[:T] - pred_deg[0:1]              # [T, B, 2]
    gt_d     = gt_deg[:T]   - gt_deg[0:1]

    pred_km  = torch.stack([
        pred_d[:, :, 0] * cos_lat * DEG2KM,
        pred_d[:, :, 1] * DEG2KM,
    ], dim=-1)                                            # [T, B, 2]
    gt_km    = torch.stack([
        gt_d[:, :, 0] * cos_lat * DEG2KM,
        gt_d[:, :, 1] * DEG2KM,
    ], dim=-1)

    # Step weights: bước sau tích lũy sai nhiều hơn → trọng số cao hơn
    w        = torch.arange(1, T + 1, dtype=pred_deg.dtype,
                            device=pred_deg.device).float()
    w        = (w / w.sum()).view(T, 1, 1)

    err_sq   = ((pred_km - gt_km) ** 2).sum(-1, keepdim=True) * w  # [T, B, 1]
    return err_sq.mean() / (150.0 ** 2)


# ══════════════════════════════════════════════════════════════════════════════
#  Loss — v48 CLEAN (không chồng nhau)
# ══════════════════════════════════════════════════════════════════════════════

def compute_st_trans_loss(pred_deg, gt_deg, epoch=0, speed_stats=None):
    """
    v48 Clean Loss Design:

        L = L_DPE                       [primary: weighted haversine position]
          + 0.40 * L_speed_match        [speed ratio → main ATE fix]
          + 0.25 * L_ate_direct         [direct along-track error]
          + 0.15 * L_cumul_disp         [accumulated displacement error]
          + 0.01 * L_accel              [smoothness regularizer]

    Đã XÓA khỏi v47:
        - L_MSE (chồng L_DPE: cùng supervise vị trí)
        - L_speed_prior (chồng L_speed_match: cùng supervise tốc độ)

    Mỗi loss một vai trò rõ ràng:
        L_DPE         → giảm distance tổng (ADE/72h)
        L_speed_match → calibrate tốc độ vs GT → giảm ATE gốc
        L_ate_direct  → nhắm thẳng ATE metric, không đụng CTE
        L_cumul_disp  → fix ATE tích lũy nhiều bước
        L_accel       → tránh oscillation (không supervise position/speed)
    """
    T = min(pred_deg.shape[0], gt_deg.shape[0])
    if T < 2:
        return {"total": pred_deg.new_zeros(()), "dpe": 0.0,
                "ate_direct": 0.0, "speed_match": 0.0, "cumul": 0.0}
    ...
    pred_speed = _step_speed(pred_deg) if T >= 2 else None
    gt_speed   = _step_speed(gt_deg)   if T >= 2 else None

    # ── L_speed_match: ratio pred/gt → 1.0 ───────────────────────────────────
    # Huber: robust vs outlier tốc độ bão (sudden intensification)
    # Clamp gt_speed ≥ 2 km/h để tránh đứng yên gây NaN
    if pred_speed is not None and gt_speed is not None:
        ratio        = pred_speed / gt_speed.clamp(min=2.0) - 1.0  # [T-1, B]
        l_speed_match = F.huber_loss(ratio,
                                     torch.zeros_like(ratio),
                                     delta=0.5)
        # Thêm hard cap: tránh speed vượt giới hạn vật lý
        l_cap = F.relu(pred_speed - v_hard_cap).pow(2).mean() / (v_hard_cap ** 2)
        l_speed_match = l_speed_match + 0.1 * l_cap
    else:
        l_speed_match = pred_deg.new_zeros(())
        ratio         = None

    # ── L_ate_direct: along-track projection error ───────────────────────────
    # Vai trò: nhắm thẳng ATE metric, không đụng CTE
    # Không chồng L_DPE vì: DPE = f(distance) không phân biệt along/cross
    if T >= 2:
        ate_vec, _ = _compute_ate_cte(pred_deg, gt_deg)   # [T-1, B]
        l_ate_direct = ate_vec.pow(2).mean() / (80.0 ** 2)
    else:
        l_ate_direct = pred_deg.new_zeros(())

    # ── L_cumul_disp: accumulated displacement error ──────────────────────────
    # Vai trò: fix ATE tích lũy (model đi nhanh/chậm hơn GT liên tục)
    # Không chồng L_DPE: DPE supervise từng điểm; cumul supervise tổng quãng đường
    l_cumul = _compute_cumul_disp_loss(pred_deg, gt_deg)

    # ── L_accel: smoothness regularizer ──────────────────────────────────────
    # Vai trò: tránh oscillation, KHÔNG supervise vị trí hay tốc độ
    # Weight nhỏ 0.01 để không lấn át signal chính
    ...
    spd_ratio_val = (_s(ratio.mean()) + 1.0) if ratio is not None else 1.0

    return dict(
        total        = total,
        dpe          = _s(l_dpe),
        speed_match  = _s(l_speed_match),
        ate_direct   = _s(l_ate_direct),
        cumul        = _s(l_cumul),
        accel        = _s(l_accel),
        spd_ratio    = spd_ratio_val,          # monitor: phải → 1.0
        ...
        speed_acc=0.0, accel_b=0.0, decomp=0.0, cons=0.0, mse=0.0,
        hav_km       = _s(dist.mean()) if T > 0 else 0.0,
        h72_km       = _s(_haversine_deg(
            pred_deg[min(11, T - 1)],
            gt_deg[min(11, T - 1)]).mean()) if T > 0 else 0.0,
...
class VelocityField(nn.Module):
    ...
    def __init__(self, pred_len=12, obs_len=8, ctx_dim=256,
        ...
        # obs_len*6 → local temporal kinematics
        self.vel_obs_enc = nn.Sequential(
            nn.Linear(obs_len * 6, 256), nn.GELU(),
            nn.LayerNorm(256),
            nn.Linear(256, 256), nn.GELU(),
        )
        # [v48 NEW] Global speed statistics: mean/std/trend/accel → 64-dim
        # Cho decoder biết bão đang nhanh/chậm/tăng/giảm tốc → giảm ATE
        self.avg_speed_enc = nn.Sequential(
            nn.Linear(4, 64), nn.GELU(),
            nn.Linear(64, 64), nn.GELU(),
        )
        # Project concat(256 + 64) → 256
        self.kine_proj = nn.Linear(256 + 64, 256)

        # ── Steering (u/v500 từ env) ──────────────────────────────────────────
        ...
        # ── Env kinematic encoder với GATED INJECTION ─────────────────────────
        # Gate init=-3 → sigmoid≈0.05 (nearly off ở early training)
    ...
    def _get_kinematic_obs_feat(self, obs_traj):
        """
        obs_traj: [T_obs, B, 2] (normalized)

        Trả về [B, 256] gồm:
          - local : temporal kinematics per step (vel, speed, heading, accel)
          - global: speed statistics (mean, std, trend, accel_mag)
                    → key signal để decoder calibrate tốc độ → giảm ATE
        """
        B     = obs_traj.shape[1]
        T_obs = obs_traj.shape[0]
        ...
        if T_obs >= 2:
            ...
            kine = torch.stack(
                [vel[:, :, 0], vel[:, :, 1],
                 speed_n, heading.sin(), heading.cos(), accel],
                dim=-1)                                         # [T-1, B, 6]

            # ── Global speed statistics (v48 NEW) ──────────────────────────
            spd_mean  = (speed.mean(0) / 20.0).clamp(-3, 3)   # [B]
            spd_std   = (speed.std(0)  / 10.0).clamp(0, 3)    # [B]
            half      = max(speed.shape[0] // 2, 1)
            spd_trend = (
                (speed[-half:].mean(0) - speed[:half].mean(0)) / 10.0
            ).clamp(-3, 3)
            if speed.shape[0] >= 2:
                accel_mag = (
                    (speed[1:] - speed[:-1]).abs().mean(0) / 5.0
                ).clamp(0, 3)
            else:
                accel_mag = obs_traj.new_zeros(B)

            global_feat_in = torch.stack(
                [spd_mean, spd_std, spd_trend, accel_mag], dim=-1)  # [B, 4]
        else:
            kine           = obs_traj.new_zeros(self.obs_len, B, 6)
            global_feat_in = obs_traj.new_zeros(B, 4)
        ...
        local_feat  = self.vel_obs_enc(kine.permute(1, 0, 2).reshape(B, -1))  # [B, 256]
        global_feat = self.avg_speed_enc(global_feat_in)                       # [B, 64]

        return self.kine_proj(torch.cat([local_feat, global_feat], dim=-1))   # [B, 256]
    ...
    def _get_env_kine_feat(self, env_data, B, device):
        """
        Gate init ≈ 0.05 → warm up tự nhiên sau vài epoch.
...
class TCFlowMatching(nn.Module):
    ...
    def _persistence_forecast_rel(self, obs_traj, lp, lm, pred_len):
        """
        v48: Exponentially weighted average velocity thay vì last-step.

        Last-step velocity noisy (đặc biệt khi bão đổi hướng đột ngột).
        Weighted average ổn định hơn → persistence init tốt hơn → ATE thấp hơn.
        """
        B, device = obs_traj.shape[1], obs_traj.device

        if obs_traj.shape[0] >= 2:
            n_steps = obs_traj.shape[0] - 1
            # Exp weights: bước gần cuối quan trọng hơn
            weights = torch.exp(
                torch.arange(n_steps, dtype=torch.float, device=device) * 0.5)
            weights = weights / weights.sum()

            vels   = obs_traj[1:] - obs_traj[:-1]                  # [T-1, B, 2]
            lv_lon = (vels[:, :, 0] * weights.view(-1, 1)).sum(0)  # [B]
            lv_lat = (vels[:, :, 1] * weights.view(-1, 1)).sum(0)  # [B]
        else:
    ...
    def get_loss_breakdown(self, batch_list, step_weight_alpha=0.0, epoch=0):
        ...
        d.update({
            ...
            "kine_gate"  : float(torch.sigmoid(
                               self.net.env_kine_gate.detach())),
        })
        return d
        ...
        with open(csv_path, "a", newline="") as fh:
            ...
            for b in range(B):
                for k in range(T):
                    ...
                    spread = float(((dlat ** 2 + dlon ** 2) ** 0.5).mean()
                                   * DEG2KM)
                    w.writerow({
                        "timestamp"     : ts,
                        "batch_idx"     : b,
