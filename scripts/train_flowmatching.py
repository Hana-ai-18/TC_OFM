...
# # # """
# # # scripts/train_flowmatching_v46.py — ST-Trans loss + FM architecture
# # # ═══════════════════════════════════════════════════════════════════════════════

# # # KEY CHANGES vs v45:
# # #   1. Loss đơn giản: DPE + 0.05*MSE + 0.1*speed + 0.01*accel (như ST-Trans)
# # #   2. Sigma schedule: 0.15 → 0.03 (fast learning phase)
# # #   3. Log đơn giản: dpe, spd, acc, fm_mse
# # #   4. Giảm patience → tránh overtrain
# # #   5. Output dir v46

# # # MONITOR:
# # #   - dpe phải giảm dần: ep0~3.0, ep10~2.0, ep30~1.5
# # #   - 72h phải < 350 ở ep15, < 320 ở ep30, < 300 ở ep50
# # # """
# # # from __future__ import annotations
...
# # #     print("  SIGMA: 0.15→0.03 (fast convergence)")
# # #     print("  TARGETS: 72h<297 | ATE<79.94 | CTE<93.58 | ADE<136.41")
# # #     print("  MONITOR: dpe↓, h72↓ — phải < 350km ở ep15")
# # #     print(f"  EMA decay: {args.ema_decay}")
# # #     print("=" * 72)
...
# # #     print("=" * 72)
# # #     print(f"  TRAINING  ({steps_per_epoch} steps/epoch, start={start_epoch})")
# # #     print(f"  KEY: dpe phải < 2.0 sau ep10, h72 < 350km sau ep15")
# # #     print("=" * 72)
...
# # #                     f"  σ={bd.get('sigma',    0):.3f}"   # current sigma
...
# # """
# # scripts/train_flowmatching_v50.py — ATE/72h Breakthrough Edition
# # ═══════════════════════════════════════════════════════════════════════════════

# # KEY CHANGES vs v46:
# #   1. Loss v50: DPE(v50_weights) + 0.03*MSE + 0.05*speed + 0.01*accel
# #                + 0.15*heading + 0.30*vel_reg + 0.25*endpoint
# #   2. STEP_WEIGHTS_V50: [1,1,1,1.5,2,2.5,3,4,5,6,7,10] — 72h = 10x
# #   3. Sigma schedule: 0.15→0.04 từ ep2 (nhanh hơn v46)
# #   4. Inference v50: speed-sweep ensemble (7 scales × 50 members)
# #   5. Log thêm: vel_reg, endpoint, heading

# # MONITOR:
# #   - vel_reg phải giảm liên tục: ep0~2.0, ep10~0.5, ep20~0.3
# #   - endpoint phải < 1.0 ở ep10
# #   - heading phải < 0.3 ở ep15
# #   - 72h phải < 350 ở ep15, < 300 ở ep30
# #   - ATE phải < 150 ở ep20, < 80 ở ep40

# # NẾU vel_reg không giảm sau ep5: tăng weight 0.30→0.40 trong patch
# # """
# # from __future__ import annotations
...
# # def _beat_report(r: dict) -> str:
# #     parts = []
# #     if np.isfinite(r.get("ADE",      float("inf"))) and r["ADE"]      < 136.41: parts.append(f"ADE✅{r['ADE']:.1f}")
# #     if np.isfinite(r.get("ATE_mean", float("inf"))) and r["ATE_mean"] < 79.94:  parts.append(f"ATE✅{r['ATE_mean']:.1f}")
# #     if np.isfinite(r.get("CTE_mean", float("inf"))) and r["CTE_mean"] < 93.58:  parts.append(f"CTE✅{r['CTE_mean']:.1f}")
# #     if np.isfinite(r.get("72h",      float("inf"))) and r["72h"]      < 297.0:  parts.append(f"72h✅{r['72h']:.1f}")
# #     return "🏆 BEAT: " + " ".join(parts) if parts else ""
...
# #     def ind(v, t): return ("✅" if v < t else "❌") if np.isfinite(v) else ""
...
# #         except Exception as e: print(f"  ⚠  EMA restore failed: {e}")
# #     return r


# # # ══════════════════════════════════════════════════════════════════════════════
# # #  Best Model Saver — giống v46
# # # ══════════════════════════════════════════════════════════════════════════════
...
# #     p.add_argument("--swa_start_epoch", default=60,   type=int)
# #     p.add_argument("--teacher_forcing", action="store_true", default=True)
# #     p.add_argument("--output_dir",  default="runs/v50",        type=str)   # v46→v50
# #     p.add_argument("--metrics_csv", default="metrics_v50.csv", type=str)   # v46→v50
# #     p.add_argument("--predict_csv", default="predictions.csv", type=str)
# #     p.add_argument("--gpu_num",     default="0", type=str)
# #     p.add_argument("--delim",       default=" ")
# #     p.add_argument("--skip",        default=1,   type=int)
...
# #     print("  SIGMA: 0.15→0.04 (từ ep2)")
# #     print("  TARGETS: 72h<297 | ATE<79.94 | CTE<93.58 | ADE<136.41")
# #     print("  MONITOR: vel_reg↓ (<0.5 ở ep10)  endpoint↓ (<1.0 ở ep10)")
# #     print(f"  EMA decay: {args.ema_decay}")
# #     print("=" * 72)
...
# #                     for k, v in state.items():
# #                         if torch.is_tensor(v): state[k] = v.to(device)
# #             except Exception as e: print(f"  ⚠  Optimizer restore failed: {e}")
# #         has_scheduler_state = "scheduler_state" in ckpt
...
# #     print("=" * 72)
# #     print(f"  TRAINING  ({steps_per_epoch} steps/epoch, start={start_epoch})")
# #     print(f"  KEY: vel_reg < 0.5 ở ep10 | endpoint < 1.0 ở ep10 | 72h < 350 ở ep15")
# #     print("=" * 72)
...
# #             if i % 20 == 0:
# #                 lr = optimizer.param_groups[0]["lr"]
# #                 # [v50] Log thêm vel_reg, endpoint, heading
# #                 print(
# #                     f"  [{epoch:>3}][{i:>3}/{len(train_loader)}]"
# #                     f"  tot={bd['total'].item():.3f}"
...
# """
# scripts/train_flowmatching_v52.py
# ═══════════════════════════════════════════════════════════════════════════════

# THAY ĐỔI SO VỚI v50:
#   1. Import TCFlowMatching từ flow_matching_model v52
#   2. Log thêm rollout_ate, rollout_w
#   3. Output dir / metrics csv đổi thành v52
#   4. Print header cập nhật

# MONITOR sau ep16:
#   rollout_ate : phải giảm liên tục (bắt đầu bật từ ep16)
#   ate_mean_km : phải < 130km ở ep18, < 100km ở ep22, < 80km ở ep30
#   72h         : phải < 340km ở ep18
#   fm_mse      : phải < 1.0 (không spike)
# """
# from __future__ import annotations
...
# from Model.data.loader_training import data_loader
# from Model.flow_matching_model import TCFlowMatching   # v52
...
#     p.add_argument("--swa_start_epoch", default=60,   type=int)
#     p.add_argument("--teacher_forcing", action="store_true", default=True)
#     p.add_argument("--output_dir",  default="runs/v52",        type=str)
#     p.add_argument("--metrics_csv", default="metrics_v52.csv", type=str)
#     p.add_argument("--predict_csv", default="predictions.csv", type=str)
#     p.add_argument("--gpu_num",     default="0", type=str)
#     p.add_argument("--delim",       default=" ")
#     p.add_argument("--skip",        default=1,   type=int)
...
#     print("=" * 72)
#     print("  TC-FlowMatching v52  |  ATE Fix Edition")
#     print("  [F1] _signed_ate_loss: normalize km TRƯỚC (fix lệch lat)")
#     print("  [F2] _rollout_ate_loss: DDIM 4-step rollout → fix train/inf gap")
#     print("  LOSS: DPE×0.6 + vel_reg×0.6 + signed_ate×0.4 + rollout_ate×sch")
#     print("        + sph_ate×0.25 + endpoint×0.15 + direct_ep×sch")
#     print("  TARGETS: ATE<79.94 | 72h<297 | 48h<200 | CTE<93.58 | ADE<136.41")
#     print("  MONITOR: rollout_ate↓ (ep16+) | ate_km↓ <130@ep18 <100@ep22")
#     print(f"  EMA decay: {args.ema_decay}")
#     print("=" * 72)
...
#     print("=" * 72)
#     print(f"  TRAINING  ({steps_per_epoch} steps/epoch, start={start_epoch})")
#     print(f"  KEY: rollout_ate bắt đầu ở ep16 | ate_km < 130 ở ep18")
#     print("=" * 72)
...
#             if i % 20 == 0:
#                 lr = optimizer.param_groups[0]["lr"]
#                 rollout_w = bd.get('rollout_w', 0.0)
#                 rollout_s = f"rol={bd.get('rollout_ate',0):.4f}(w={rollout_w:.2f})" \
#                             if rollout_w > 0.01 else "rol=OFF"
#                 print(
#                     f"  [{epoch:>3}][{i:>3}/{len(train_loader)}]"
#                     f"  tot={bd['total'].item():.3f}"
#                     f"  dpe={bd.get('dpe',      0):.3f}"
...
"""
scripts/train_flowmatching_v55.py
═══════════════════════════════════════════════════════════════════════════════

THAY ĐỔI SO VỚI v52:
  1. Import TCFlowMatching từ flow_matching_model v55
  2. Sau khi train xong: chạy per-storm evaluation trên test set
     → In ADE, 72h, ATE, CTE cho từng cơn bão + tổng thể
  3. Output dir / metrics csv đổi thành v55

v55 FIXES:
  [FIX-v55-1] fm_mse × 6.0 (was 4.0)
  [FIX-v55-2] Speed bias 6:1 asymmetry + hard speed floor
  [FIX-v55-3] Adaptive ATE weight: cap 2.0, rate 0.04
  [FIX-v55-4] Direct endpoint: doubled weights + s=10 + ep5 start
  [FIX-v55-5] vel_reg × 2.00 (was 1.20)

TARGETS: ATE<79.94 | CTE<93.58 | ADE<136.41 | 72h<297

MONITOR:
  fm_mse < 0.8 ở ep10
  ATE ep20<120, ep30<100, ep40<85, ep50<80
  72h ep30<330, ep40<315, ep50<300
  vel_reg > 0.10
"""
from __future__ import annotations
...
from Model.data.loader_training import data_loader
from Model.flow_matching_model import TCFlowMatching   # v55
...
# Targets từ paper Faiaz et al. 2026
TARGETS = {
    "ADE" : 136.41,
    "72h" : 297.0,
    "ATE" : 79.94,
...
# ══════════════════════════════════════════════════════════════════════════════
#  Coordinate helpers (dùng cho per-storm eval)
# ══════════════════════════════════════════════════════════════════════════════

def _norm_to_deg_local(arr: torch.Tensor) -> torch.Tensor:
    out = arr.clone()
...
def _haversine_local(p1: torch.Tensor, p2: torch.Tensor) -> torch.Tensor:
...
def _compute_ate_cte_local(pred_deg, gt_deg):
    ...
    dlon_along = lon2 - lon1
    y_along = torch.sin(dlon_along) * torch.cos(lat2)
    x_along = torch.cos(lat1)*torch.sin(lat2) - torch.sin(lat1)*torch.cos(lat2)*torch.cos(dlon_along)
    bear_along = torch.atan2(y_along, x_along)
    dlon_err = lon3 - lon2
    y_err = torch.sin(dlon_err) * torch.cos(lat3)
    x_err = torch.cos(lat2)*torch.sin(lat3) - torch.sin(lat2)*torch.cos(lat3)*torch.cos(dlon_err)
    bear_err = torch.atan2(y_err, x_err)
    total = _haversine_local(pred_deg[1:T], gt_deg[1:T])
    angle = bear_err - bear_along
    ate   = total * torch.cos(angle)
    cte   = total * torch.sin(angle)
    return ate, cte


# ══════════════════════════════════════════════════════════════════════════════
#  Per-storm evaluation
# ══════════════════════════════════════════════════════════════════════════════

_HORIZON_STEPS = {12: 1, 24: 3, 48: 7, 72: 11}


class PerStormAccumulator:
    def __init__(self):
    ...
    def update(self, storm_id, dist, ate, cte):
        d = self.data[storm_id]
        B = dist.shape[1]
        d["dists"].extend(dist.mean(0).tolist())
        d["n_seq"] += B
        for h, s in _HORIZON_STEPS.items():
            if s < dist.shape[0]:
    ...
    def compute(self, storm_id) -> dict:
        d = self.data[storm_id]
        r = {
            "storm_id": storm_id,
            "n_seq"   : d["n_seq"],
    ...
    def compute_overall(self) -> dict:
        ...
        for d in self.data.values():
            all_d.extend(d["dists"]); all_a.extend(d["ates"]); all_c.extend(d["ctes"])
            n += d["n_seq"]
            for h, vals in d["step_dists"].items():
                step_all[h].extend(vals)
        r = {
            "storm_id": "OVERALL", "n_seq": n,
            "ADE": float(np.mean(all_d)) if all_d else float("nan"),
            "ATE": float(np.mean(all_a)) if all_a else float("nan"),
            "CTE": float(np.mean(all_c)) if all_c else float("nan"),
        }
        for h in _HORIZON_STEPS:
            vals = step_all.get(h, [])
            r[f"{h}h"] = float(np.mean(vals)) if vals else float("nan")
...
def _extract_storm_ids(batch) -> Optional[List[str]]:
...
@torch.no_grad()
def evaluate_per_storm(model, test_loader, device, args,
                        ema_obj=None) -> List[dict]:
    """
    Chạy inference trên test_loader, tính metrics per-storm.
    Nếu test loader không có storm_id → gom vào "STORM_ALL".
    """
    # Apply EMA nếu có
    backup = None
    ...
    model.eval()
    acc = PerStormAccumulator()

    for batch in test_loader:
        bl = move(list(batch), device)
        storm_ids = _extract_storm_ids(batch)

        # Inference
        pred, _, _ = model.sample(
            bl,
            num_ensemble    = args.val_ensemble,
            ddim_steps      = args.ode_steps_val,
            importance_weight = True,
        )
        ...
        B   = pred.shape[1]
        pd  = _norm_to_deg_local(pred[:T])
        gd  = _norm_to_deg_local(gt[:T])
        dist = _haversine_local(pd, gd)
        ate, cte = _compute_ate_cte_local(pd, gd)

        if storm_ids and len(storm_ids) == B:
            for b in range(B):
                sid = storm_ids[b]
                acc.update(sid,
                           dist[:, b:b+1],
                           ate[:, b:b+1] if ate is not None else None,
                           cte[:, b:b+1] if cte is not None else None)
        else:
            acc.update("STORM_ALL", dist, ate, cte)
    ...
    # Collect results
    results = []
    storms = sorted([s for s in acc.data if s not in ("STORM_ALL",)])
    if not storms:
        storms = ["STORM_ALL"]

    for sid in storms:
        results.append(acc.compute(sid))

    results.append(acc.compute_overall())
    return results


def _print_storm_table(results: List[dict], model_name: str):
    print(f"\n{'='*100}")
    print(f"  Per-Storm Test Evaluation  |  {model_name}")
    print(f"{'='*100}")
    print(f"  {'Storm':<14} {'N':>5}  "
          f"{'ADE':>8}  {'12h':>8}  {'24h':>8}  {'48h':>8}  {'72h':>8}  "
          f"{'ATE':>8}  {'CTE':>8}")
    print(f"  {'-'*96}")
    ...
    def _v(r, k):
        v = r.get(k, float("nan"))
        t = TARGETS.get(k)
        s = f"{v:>7.1f}" if np.isfinite(v) else "    nan"
        if t and np.isfinite(v):
            s += "✅" if v < t else "❌"
        else:
            s += "  "
        return s
    ...
    for r in results:
        ...
        print(f"{prefix} {r['n_seq']:>5}  "
              f"{_v(r,'ADE')}  {_v(r,'12h')}  {_v(r,'24h')}  "
              f"{_v(r,'48h')}  {_v(r,'72h')}  {_v(r,'ATE')}  {_v(r,'CTE')}")

    overall = results[-1]
    print(f"\n  Targets: ADE<{TARGETS['ADE']} | 72h<{TARGETS['72h']} | "
          f"ATE<{TARGETS['ATE']} | CTE<{TARGETS['CTE']}")

    beats = []
    for k in ["ADE", "72h", "ATE", "CTE", "12h", "24h", "48h"]:
        v = overall.get(k, float("nan"))
        if np.isfinite(v) and v < TARGETS.get(k, float("inf")):
            beats.append(f"{k}={v:.1f}✅")
    if beats:
        print(f"  🏆 BEAT: {' | '.join(beats)}")
    print(f"{'='*100}\n")


def _save_storm_csv(results: List[dict], csv_path: str, model_name: str):
    import csv as _csv
    from datetime import datetime
    ...
    fields = ["timestamp", "model", "storm_id", "n_seq",
              ...
              "beat_ADE", "beat_72h", "beat_ATE", "beat_CTE"]
    with open(csv_path, "w", newline="") as fh:
        w = _csv.DictWriter(fh, fieldnames=fields)
        w.writeheader()
        for r in results:
            def _f(k): return f"{r.get(k, float('nan')):.2f}"
            def _b(k): return "YES" if np.isfinite(r.get(k, float("nan"))) and r.get(k) < TARGETS.get(k, float("inf")) else "NO"
            w.writerow({
                "timestamp": ts, "model": model_name,
                "storm_id": r["storm_id"], "n_seq": r["n_seq"],
                "ADE": _f("ADE"), "12h": _f("12h"), "24h": _f("24h"),
                "48h": _f("48h"), "72h": _f("72h"),
                "ATE": _f("ATE"), "CTE": _f("CTE"),
                "beat_ADE": _b("ADE"), "beat_72h": _b("72h"),
                "beat_ATE": _b("ATE"), "beat_CTE": _b("CTE"),
            })
    print(f"  Per-storm CSV saved: {csv_path}")


# ══════════════════════════════════════════════════════════════════════════════
#  Helpers (giữ nguyên từ v52)
# ══════════════════════════════════════════════════════════════════════════════
...
def get_args():
    ...
    p.add_argument("--use_ema",         action="store_true", default=True)
    p.add_argument("--ema_decay",       default=0.995, type=float)
    p.add_argument("--output_dir",  default="runs/v55",        type=str)
    p.add_argument("--metrics_csv", default="metrics_v55.csv", type=str)
    p.add_argument("--predict_csv", default="predictions.csv", type=str)
    p.add_argument("--gpu_num",     default="0", type=str)
    p.add_argument("--delim",       default=" ")
    p.add_argument("--skip",        default=1,   type=int)
    ...
    p.add_argument("--eval_test_after_train", action="store_true", default=True,
                   help="Chạy per-storm evaluation sau khi train xong")
    p.add_argument("--test_storm_csv",
                   default=None, type=str,
                   help="Path lưu per-storm results (default: output_dir/test_storms.csv)")
    return p.parse_args()
...
def main(args):
    ...
    print("=" * 72)
    print("  TC-FlowMatching v55  |  ATE+72h Breakthrough Edition")
    print("  [v55-1] fm_mse × 6.0  (was 4.0)")
    print("  [v55-2] Speed bias 6:1 + hard floor 85%")
    print("  [v55-3] ATE weight: cap 2.0, rate 0.04/ep")
    print("  [v55-4] Endpoint: doubled weights + s=10 + ep5 start")
    print("  [v55-5] vel_reg × 2.00  (was 1.20)")
    print("  TARGETS: ATE<79.94 | CTE<93.58 | ADE<136.41 | 72h<297")
    print(f"  EMA decay: {args.ema_decay}")
    print("=" * 72)
    ...
    # ══════════════════════════════════════════════════════════════════════════
    #  Per-storm evaluation trên test set
    # ══════════════════════════════════════════════════════════════════════════

    if args.eval_test_after_train:
        print("\n" + "=" * 72)
        print("  POST-TRAINING: Per-Storm Evaluation on Test Set")
        print("=" * 72)
        ...
        try:
            ...
            print("  Skipping per-storm evaluation")
            return
            ...
            if "ema_shadow" in ckpt and ckpt["ema_shadow"] is not None:
                ...
                if ema_obj is not None:
                    for k, v in ckpt["ema_shadow"].items():
                        if k in ema_obj.shadow:
                            ema_obj.shadow[k].copy_(v.to(device))
                    print("  EMA weights loaded for test evaluation")
        else:
            print("  ⚠  best_model.pth not found, using current model weights")

        ema_obj = _get_ema_obj(model)

        # Evaluate without EMA (raw)
        print("\n  [1/2] Evaluating with RAW weights...")
        results_raw = evaluate_per_storm(model, test_loader, device, args, ema_obj=None)
        _print_storm_table(results_raw, "v55-RAW")

        # Evaluate with EMA
        if ema_obj is not None:
            print("\n  [2/2] Evaluating with EMA weights...")
            results_ema = evaluate_per_storm(model, test_loader, device, args, ema_obj=ema_obj)
            _print_storm_table(results_ema, "v55-EMA")

            # Dùng EMA nếu tốt hơn
            raw_overall = results_raw[-1]
            ema_overall = results_ema[-1]
            if (np.isfinite(ema_overall.get("ADE", float("nan"))) and
                ema_overall.get("ADE", float("inf")) < raw_overall.get("ADE", float("inf"))):
                results_final = results_ema
                tag = "EMA"
            else:
        ...
        # Save CSV
        storm_csv = args.test_storm_csv or os.path.join(
            args.output_dir, "test_storm_metrics.csv")
        _save_storm_csv(results_final, storm_csv, f"v55-{tag}")

        # Final summary
        overall = results_final[-1]
        print(f"\n  {'='*60}")
        print(f"  FINAL TEST RESULTS  ({tag} weights)")
        print(f"  {'─'*60}")
        for k in ["ADE", "12h", "24h", "48h", "72h", "ATE", "CTE"]:
            v = overall.get(k, float("nan"))
            t = TARGETS.get(k)
            if np.isfinite(v):
                mark = "✅ BEAT!" if (t and v < t) else f"❌ (target: {t})"
                print(f"  {k:>6}: {v:>8.1f} km  {mark}")
        print(f"  {'='*60}")
