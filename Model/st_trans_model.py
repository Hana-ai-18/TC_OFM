...
#     ROOT CAUSE #4 FIX — σ_dpe_hard drift:
#     Quan sát từ log v2: σ_dpe_hard tăng dần 0.61→0.68 trong khi L_dpe_hard vẫn cao.
#     Nguyên nhân: UW tự tăng σ để giảm contribution của loss lớn → counter-productive.
#     Hiệu ứng: weight(dpe_hard) = exp(-2σ)/2 giảm từ 0.30→0.23 → model "buông" hard DPE.

#     Giải pháp kép:
#       1. CLAMP log_sigma[dpe_hard] ≤ LOG_SIGMA_DPE_MAX → σ ≤ 0.65 → weight ≥ 0.27
#          Giữ minimum effective weight cho metric quan trọng nhất.
#       2. Fixed weight mode cho dpe_hard khi epoch < warmup_uw:
#          Trong 20 epoch đầu, dpe_hard dùng fixed weight=1.0 (không UW).
#          Sau đó UW mới được phép điều chỉnh, nhưng vẫn bị clamp.
#          → Decoder học ổn định trước, sau đó UW fine-tune balance.

#     heading và recurv: KHÔNG clamp — chúng là auxiliary, cho phép drift tự nhiên.
#     """
#     TASKS = ["dpe_hard", "heading", "recurv"]
#     INIT  = {
#         "dpe_hard": -0.5,   # σ≈0.61, weight≈0.30 — khởi đầu cao vì quan trọng
#         "heading":   0.3,   # σ≈1.35, weight≈0.27 — auxiliary direction
#         "recurv":    1.0,   # σ≈2.72, weight≈0.07 — auxiliary classify (nhẹ)
#     }
#     # Clamp log_sigma[dpe_hard] ≤ này → σ ≤ exp(LOG_SIGMA_DPE_MAX) ≈ 0.65
#     # → effective weight = exp(-2*0.65)/2 ≈ 0.27 (không giảm dưới đây)
#     LOG_SIGMA_DPE_MAX = math.log(0.65)

#     def __init__(self, uw_warmup_epochs: int = 20):
#         super().__init__()
#         inits = torch.tensor([self.INIT[t] for t in self.TASKS])
#         self.log_sigma = nn.Parameter(inits)
#         self._idx = {t: i for i, t in enumerate(self.TASKS)}
#         # Epoch counter để biết khi nào UW được phép control dpe_hard
#         self._current_epoch = 0
#         self.uw_warmup_epochs = uw_warmup_epochs

#     def set_epoch(self, epoch: int):
#         """Gọi từ training loop ở đầu mỗi epoch."""
#         self._current_epoch = epoch

#     def weight(self, name: str, loss: torch.Tensor) -> torch.Tensor:
#         idx = self._idx[name]
#         s   = self.log_sigma[idx]

#         if name == "dpe_hard":
#             # Phase 1 (epoch < warmup): fixed weight=1.0, không UW
#             # → σ không được update cho dpe_hard, loss đi thẳng
#             if self._current_epoch < self.uw_warmup_epochs:
#                 return loss   # fixed weight = 1.0

#             # Phase 2 (epoch ≥ warmup): UW được phép, nhưng clamp σ ≤ 0.65
#             # → weight không giảm dưới ~0.27
#             s = s.clamp(max=self.LOG_SIGMA_DPE_MAX)
...
#         self.threshold_spd    = threshold_spd
#         self.dpe_ramp         = dpe_ramp
#         # ROOT CAUSE #2: Curriculum hard gradient params
#         self.curriculum_warmup    = 30    # epoch bắt đầu cho decoder học hard
#         self.curriculum_full      = 80    # epoch blend đạt max
#         self.curriculum_max_blend = 0.4   # max gradient scale từ hard về decoder
#         # Internal epoch counter (sync từ training loop qua set_epoch)
#         self._current_epoch = 0
...
#         # ── UW: 3 tasks cho hard, với dpe_hard clamp (ROOT CAUSE #4 FIX) ──
#         # uw_warmup_epochs=20: 20 epoch đầu dpe_hard dùng fixed weight=1.0
#         # sau đó UW được phép nhưng σ bị clamp ≤ 0.65 → weight ≥ 0.27
#         self.uw = UncertaintyWeighting(uw_warmup_epochs=20)
...
#     def set_epoch(self, epoch: int):
#         """
#         Gọi từ training loop ở đầu mỗi epoch để sync curriculum state.
#         Propagate epoch tới UW (cho UW warmup logic) và lưu nội bộ
#         để tính curriculum_hard_blend.
#         """
#         self._current_epoch = epoch
#         self.uw.set_epoch(epoch)

#     # ── Training loss ─────────────────────────────────────────────────────

#     def get_loss_breakdown(self, batch_list) -> Dict:
#         """
#         ROOT CAUSE #2 FIX — Curriculum Hard Gradient:

#         Vấn đề: lp_h.detach() → decoder KHÔNG BAO GIỜ học từ hard samples.
#         Decoder chỉ tối ưu easy path (~61% data). Hard samples trong inference
#         được gate-corrected nhưng starting point lp_h từ decoder chưa bao giờ
#         thấy recurvature patterns → gate phải "sửa" nhiều → ADE hard cao hơn cần.

#         Giải pháp — Curriculum 3 giai đoạn:
#           Phase 1 (epoch < curv_warmup=30):
#             lp_h.detach() hoàn toàn — gate học độc lập, ổn định
#             L_hard = UW(gate_loss + heading + recurv)
#             → Gate học blend α tốt mà không bị "kéo" bởi decoder gradient

#           Phase 2 (curv_warmup ≤ epoch < curv_full=80):
#             lp_h_input = blend tuyến tính: detach→full
#             blend = (epoch - curv_warmup) / (curv_full - curv_warmup) * max_blend
#             max_blend = 0.4 → 40% gradient từ hard về decoder ở epoch 80
#             L_hard_dec = step_weighted_dpe(lp_h, gt_h) * blend_weight
#             → Decoder dần dần học hard patterns mà không phá vỡ gate

#           Phase 3 (epoch ≥ curv_full=80):
#             blend cố định = max_blend = 0.4
#             L_hard_dec tiếp tục update decoder với 40% weight
#             Gate vẫn chạy với lp_h không fully-detached
#             → Ổn định, không tăng thêm để tránh overwrite easy learning

#         Tại sao max_blend=0.4 chứ không phải 1.0?
#           Easy samples vẫn là supervisor chính cho decoder (correct predictions).
#           Nếu hard gradient chiếm quá nhiều (>50%), decoder có thể drift sang
#           overfitting hard patterns (recurvature rất noisy) → easy_ADE tăng trở lại.
#           0.4 là balance: decoder học hard patterns nhưng easy vẫn dominant.
#         """
#         obs_traj = batch_list[0]   # [T_obs, B, 2]
#         traj_gt  = batch_list[1]   # [T_pred, B, 2]
...
#         # ── EASY: step-weighted DPE, gradient đầy đủ ─────────────────────
#         L_easy = None
#         if n_easy > 0:
#             lp_e = lp[is_easy].permute(1, 0, 2)   # [T, B_e, 2]
#             gt_e = traj_gt[:, is_easy]             # [T, B_e, 2]
...
#         # ── HARD: curriculum gate + decoder gradient ──────────────────────
#         L_hard = None
#         if n_hard > 0:
#             lp_h  = lp[is_hard]           # [B_h, H, 2]
#             gt_h  = traj_gt[:, is_hard]   # [T, B_h, 2]
...
#             # ROOT CAUSE #2: Curriculum blend để decoder học từ hard
#             curv_warmup = getattr(self, 'curriculum_warmup', 30)
#             curv_full   = getattr(self, 'curriculum_full',   80)
#             max_blend   = getattr(self, 'curriculum_max_blend', 0.4)

#             if epoch < curv_warmup:
#                 # Phase 1: hoàn toàn detach — gate học độc lập
#                 blend = 0.0
#             elif epoch < curv_full:
#                 # Phase 2: blend tuyến tính 0 → max_blend
#                 progress = (epoch - curv_warmup) / max(curv_full - curv_warmup, 1)
#                 blend    = progress * max_blend
#             else:
#                 # Phase 3: fixed blend
#                 blend = max_blend

#             # lp_h_gate: input cho gate (detach phần nào theo blend)
#             if blend == 0.0:
#                 lp_h_gate = lp_h.detach()
#             else:
#                 # (1-blend)*detach + blend*original → gradient scale = blend
#                 lp_h_gate = (1.0 - blend) * lp_h.detach() + blend * lp_h

#             # Gate blend: stop_ctx_grad=True bảo vệ encoder khỏi gate gradient
#             final_h, alpha_m = self._gate_blend(
#                 lp_h_gate, ctx_h, steer_h, obs_h,
#                 stop_ctx_grad=True)

#             hl = self._hard_losses(final_h, gt_h, ctx_h, obs_h)

#             # UW weighting cho hard losses (dpe_hard có clamp từ Phase 2)
#             L_hard = (
#                 self.uw.weight("dpe_hard", hl["dpe"])
#                 + self.uw.weight("heading", hl["heading"])
#                 + self.uw.weight("recurv",  hl["recurv"])
