...
class TCFlowMatching(nn.Module):
    ...
    def get_loss_breakdown(self, batch_list, epoch: int = 0, **kwargs) -> Dict:
        ...
        # clamp(min=-3): ngăn log_sigma → -∞ (suy biến ngược, total âm).
        # log_sigma=-3 → sigma=e^{-3}≈0.05 → prec=e^6≈403 → effective_lambda≈201
        # Đây là mức tối đa hợp lý (không có loss phụ nào cần weight >200×L_CFM).
        # Khi l_reg/l_heading/l_calib rất nhỏ (model học tốt), optimizer muốn
        # kéo log_sigma → -∞ để total âm (gian lận) — clamp ngăn điều này.
        ...
        weighted_calib   = 1.0      * (0.5 * prec_calib   * l_calib   + self.log_sigma_calib.clamp(min=-3.0))
