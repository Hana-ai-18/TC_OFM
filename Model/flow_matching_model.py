...
FIXES vs v24:
  ...
  FIX-M28  [FREEZE BACKBONE] freeze_backbone() sửa để frozen đúng
           net.spatial_enc / net.enc_1d / net.env_enc thay vì dùng
           named_parameters() string-match vốn chỉ frozen 145 params.
           Phase-1 giờ thực sự frozen ~12M params, chỉ train
           ShortRangeHead + ctx_fc/transformer (~2.4M params).

  FIX-M29  [STEP_SCALE INIT] ShortRangeHead.step_scale init từ 0.3
           → 0.6 để early predictions có displacement lớn hơn ngay
           từ đầu, giảm số epoch cần để hội tụ.

  FIX-M30  [VEL_ENC FALLBACK] Xử lý robust khi obs_traj.shape[0] < 4:
           dùng available velocities + zero-padding thay vì chỉ v1.
           Không còn trường hợp mất motion prior hoàn toàn.

  FIX-M31  [SAMPLE SPREAD] sample() không còn override all_trajs[:,:4]
           với sr_pred deterministic (fix SSR=2.04). Thay vào đó chỉ
           override pred_mean[:4]. CRPS/SSR cho bước 1-4 vẫn được
           tính từ FM ensemble.

  FIX-M32  [CTX NOISE] ctx_noise_scale áp dụng mỗi ensemble member
           (không chỉ step=0) để tăng diversity trong FM ensemble.

Kept from v24:
  FIX-M23  ShortRangeHead GRU autoregressive
  FIX-M24  pred_mean[:4] từ ShortRangeHead
  FIX-M25  initial_sample_sigma=0.03
  FIX-M26  MAX_DELTA clamp built-in
  FIX-M27  short_range_loss weight=5.0
  FIX-M18  active_pred_len = pred_len
  FIX-M21  _physics_correct n_steps=5, lr=0.002
  FIX-M22  post-sampling clip [-3.0, 3.0]
"""
from __future__ import annotations
...
# ══════════════════════════════════════════════════════════════════════════════
#  ShortRangeHead  – FIX-M23 + FIX-M29 + FIX-M30
# ══════════════════════════════════════════════════════════════════════════════
...
class ShortRangeHead(nn.Module):
    ...
    FIX-M29: step_scale init 0.3 → 0.6 (faster convergence)
    FIX-M30: vel_enc fallback robust for short obs sequences
    """
    ...
    def __init__(self, raw_ctx_dim: int = 512, obs_len: int = 8):
        ...
        # ── GRU cell: input = ctx(256) + vel(128) + cur_pos(2) = 386 ─────
        self.gru_cell = nn.GRUCell(input_size=386, hidden_size=256)
        ...
        # FIX-M29: init=0.6 so early displacements are larger (~274 km/6h)
        self.step_scale = nn.Parameter(torch.ones(self.N_STEPS) * 0.6)
    ...
    def _build_vel_input(self, obs_traj: torch.Tensor) -> torch.Tensor:
        """
        FIX-M30: Robust velocity encoding for any obs length >= 1.
        Always returns [B, 6] concatenation of 3 velocity steps.
        Falls back to zero-padding for missing steps.
        """
        T = obs_traj.shape[0]
        B = obs_traj.shape[1]
        zeros = torch.zeros(B, 2, device=obs_traj.device, dtype=obs_traj.dtype)

        if T >= 4:
            v1 = obs_traj[-1] - obs_traj[-2]
            v2 = obs_traj[-2] - obs_traj[-3]
            v3 = obs_traj[-3] - obs_traj[-4]
        elif T == 3:
            v1 = obs_traj[-1] - obs_traj[-2]
            v2 = obs_traj[-2] - obs_traj[-3]
            v3 = zeros
        ...
        return torch.cat([v1, v2, v3], dim=-1)  # [B, 6]
    ...
    def forward(
        ...
        # FIX-M30: robust vel input
        vel_input = self._build_vel_input(obs_traj)       # [B, 6]
...
class VelocityField(nn.Module):
    ...
    def forward_short_range(
        ...
        """Deterministic short-range prediction [4, B, 2]."""
        return self.short_range_head(raw_ctx, obs_traj)


# ══════════════════════════════════════════════════════════════════════════════
#  Helper: freeze / unfreeze backbone  (FIX-M28)
# ══════════════════════════════════════════════════════════════════════════════

def freeze_backbone(model: "TCFlowMatching") -> int:
    """
    FIX-M28: Freeze net.spatial_enc, net.enc_1d, net.env_enc directly
    by accessing the module objects — not string matching on parameter names.
    Returns number of frozen parameters.
    """
    frozen_modules = [
        model.net.spatial_enc,
        model.net.enc_1d,
        model.net.env_enc,
    ]
    n_frozen = 0
    ...
    print(f"  [Phase-1] Frozen {n_frozen:,} params  "
...
def unfreeze_all(model: "TCFlowMatching") -> None:
    ...
    total = sum(p.numel() for p in model.parameters())
    print(f"  [Phase-2] Unfrozen all {total:,} params")
...
class TCFlowMatching(nn.Module):
    ...
    def get_loss_breakdown(self, batch_list: List,
        ...
        # Ensemble for AFCRPS — use n_train_ens members
        samples: List[torch.Tensor] = []
        for _ in range(self.n_train_ens):
            xt_s, ts, _, dens_s, _ = self._cfm_noisy(x1)
            pv_s  = self.net.forward_with_ctx(xt_s, ts, raw_ctx, noise_scale=0.0)
    ...
    def sample(
        ...
        """
        FIX-M24: pred_mean[:4] ← ShortRangeHead (deterministic).
                 pred_mean[4:] ← FM ensemble mean.
        FIX-M31: all_trajs NOT overridden for steps 1-4 — CRPS/SSR
                 computed correctly from FM ensemble.
        FIX-M32: ctx_noise applied per-member for diversity.
        ...
        for k in range(num_ensemble):
            ...
            for step in range(ddim_steps):
                t_b = torch.full((B,), step * dt, device=device)
                # FIX-M32: ctx_noise per-member (not just step=0)
                ns  = self.ctx_noise_scale
                vel = self.net.forward_with_ctx(x_t, t_b, raw_ctx, noise_scale=ns)
                x_t = x_t + dt * vel
        ...
        # FIX-M31: do NOT override all_trajs[:, :n_sr] — keep FM spread
        all_trajs = torch.stack(traj_s)    # [S, T, B, 2]
        all_me    = torch.stack(me_s)

        # FIX-M24: blend short-range into pred_mean only
        fm_mean   = all_trajs.mean(0)      # [T, B, 2]
        pred_mean = fm_mean.clone()
        pred_mean[:n_sr] = sr_pred         # Override mean steps 1-4
