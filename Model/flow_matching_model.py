...
"""
flow_matching_model_v70.py  ── TC-FlowMatching v70
══════════════════════════════════════════════════════════════════════════════
MỤC TIÊU: ADE giảm đều + giảm mạnh + stable như v59

ROOT CAUSES ĐÃ FIX (từ analysis v65/v68/v69):

  V68-BUG-1 [CRITICAL] LearnedStepWeights (nn.Parameter) drift:
    sw12: 8→11.28,  sw72: 10→7.12  sau 20 ep → lật ngược ratio
    Crossing tại ep9-10 → ADE spike 238→326 không phục hồi
    FIX: step_weights = FIXED v59 values, KHÔNG nn.Parameter, KHÔNG drift

  V69-BUG-2 [CRITICAL] mask_ns logic vẫn sai:
    v69: zeros→scatter(True at k_star)→masked_fill(mask_ks, inf)
    → fill inf TẠI k_star → min = dist to k_star = 0 → L_div = 0.40 LUÔN
    FIX: ones→scatter(False at k_star)→masked_fill(~mask_ns, inf)
    → fill inf TẠI k_star → min = dist to nearest OTHER mode > 0  ✅

  V69-BUG-3 [IMPORTANT] DPE weight quá nhỏ (0.5) → ADE giảm chậm:
    v69: LFM + 0.5*LDPE  (DPE yếu)
    v59: L_FM + 2.0*L_DPE (DPE mạnh, trực tiếp drive ADE!)
    FIX: Phase 1 dùng 2.0*LDPE như v59

  V69-BUG-4 [IMPORTANT] Curriculum step-function jump tại ep10:
    geo(α=0.8)→geo(α=1.5) đột ngột → loss landscape shift → potential spike
    FIX: KHÔNG curriculum, KHÔNG transition.
    Dùng đúng v59 fixed weights từ ep0: [2,8,3,5,2,2.5,3,4,5,6,7,10]

  V68-BUG-3 [IMPORTANT] Phase 2 fully frozen generator:
    modes không thay đổi → ADE stuck 294-305 km ep20-27
    FIX: Phase 2 vẫn update generator nhẹ với 0.2*LFM (như v69 đã có)

  V68-BUG-5 [MINOR] OT bật tại ep10 (trùng sw crossing):
    Double event → catastrophic
    FIX: OT từ ep>=20 (như v69 đã có)

THAM KHẢO V59 (ADE stable, train giảm đều):
  loss = L_FM + 2.0*L_DPE(STEP_WEIGHTS) + 0.80*L_ATE
  STEP_WEIGHTS = [2,8,3,5,2,2.5,3,4,5,6,7,10] → CỐ ĐỊNH, không parameter
  train: 2.67→1.43 đều đặn; ADE ep0=272, ep24=262; KHÔNG spike

V70 = V69 architecture + v59 loss philosophy:
  - Fixed v59 step_weights (không parameter → không drift)
  - DPE weight 2.0 (mạnh, trực tiếp drive ADE)
  - mask_ns đúng (như v65-stable)
  - ctx_sel luôn real context, detach (như v65-stable)
  - OT từ ep>=20
  - Phase 2 có generator nhẹ
"""
from __future__ import annotations
import math
import torch
...
# V59 fixed step weights — KHÔNG bao giờ thay đổi, KHÔNG parameter
# 6h=2, 12h=8, 18h=3, 24h=5, 30h=2, 36h=2.5, 42h=3, 48h=4, 54h=5, 60h=6, 66h=7, 72h=10
# 72h có weight cao nhất → model ưu tiên đúng metric ADE dài hạn
_V59_STEP_WEIGHTS = [2.0, 8.0, 3.0, 5.0, 2.0, 2.5, 3.0, 4.0, 5.0, 6.0, 7.0, 10.0]


# ── Step weights helper — FIXED, không gradient, không drift ──────────────────

def _get_step_weights(T: int, device) -> torch.Tensor:
    """
    Trả về v59 step weights [T], normalized để mean=1.
    KHÔNG phải nn.Parameter → không bao giờ drift như v68.
    sw72/sw12 = 10/8 = 1.25, cố định mọi epoch.
    """
    raw = torch.tensor(_V59_STEP_WEIGHTS[:T], dtype=torch.float32, device=device)
    if len(raw) < T:
        # Pad với giá trị cuối nếu T > 12
        raw = torch.cat([raw, raw[-1].expand(T - len(raw))])
    return raw * T / raw.sum()  # normalize: sum = T, mean = 1
...
# ── DPE loss — giống v59 compute_st_trans_loss ────────────────────────────────

def _dpe_loss(best_mode_rel, x1_rel, lp, step_w):
    """
    Per-step haversine với v59 fixed step_weights + Huber(d=200).
    step_w: [T] tensor, pre-computed, NO gradient.
    Giống l_dpe trong v59, trực tiếp drive ADE metric.
    """
    best_abs = _norm_to_deg(lp.unsqueeze(1) + best_mode_rel[:, :, :2])  # [B,T,2]
    gt_abs   = _norm_to_deg(lp.unsqueeze(1) + x1_rel[:, :, :2])
...
def _ot_cost(x0, x1, lp):
    ...
    sc=(s0.unsqueeze(1)-s1.unsqueeze(0)).abs()/20.
    def _mb(d):
        # BUG-FIX v68: dùng d[:,1:,:] không phải d[:,1,:]
        b=_fwd_az(d[:,:-1,:], d[:,1:,:])
        return torch.atan2(b.sin().mean(-1), b.cos().mean(-1))
    h0=_mb(d0); h1=_mb(d1)
    dh=(h0.unsqueeze(1)-h1.unsqueeze(0)+math.pi)%(2*math.pi)-math.pi
...
# ══════════════════════════════════════════════════════════════════════════════
#  TCFlowMatchingV70 — Main model
# ══════════════════════════════════════════════════════════════════════════════

class TCFlowMatchingV70(nn.Module):
    """
    V70: stable training với v59 loss philosophy + multi-mode generation.

    Key differences from v69:
    1. step_weights = FIXED v59 values (không parameter, không drift)
    2. mask_ns = ones→scatter False at k_star (BUG-2 fix từ v65-stable)
    3. DPE weight = 2.0 trong Phase 1 (như v59, mạnh hơn v69's 0.5)
    4. KHÔNG có curriculum transition → không có sudden loss landscape shift
    5. Phase 2 vẫn có 0.2*LFM (generator nhẹ không frozen hoàn toàn)
    6. ctx_sel LUÔN real context (như v65-stable)
    7. OT từ epoch>=20 (tránh coincide với bất kỳ event nào)
    """
    ...
    def __init__(self, pred_len=12, obs_len=8, ctx_dim=256, sigma_min=.02,
        ...
        self.selector=CompassSelector(ctx_dim,K,8)
        # KHÔNG có step_weights parameter — tính on-the-fly từ _V59_STEP_WEIGHTS
    ...
    def get_loss_breakdown(self, bl, epoch=0, **kw):
        ...
        # ── FIX 1: step weights FIXED v59, KHÔNG parameter, KHÔNG drift ──────
        # sw12=8.0, sw72=10.0 mọi epoch → ratio 1.25 cố định, không bao giờ cross
        step_w = _get_step_weights(self.pred_len, dev)  # [T], no_grad

        # ── Context ───────────────────────────────────────────────────────────
        raw=self.encoder.encode(bl)
        null=(torch.rand(1).item()<self.cfg_uncond_prob)
        ctx=self.encoder.apply_ctx_head(raw, use_null=null)
        # FIX 5: ctx_sel LUÔN real context, LUÔN detach
        ctx_sel=self.encoder.apply_ctx_head(raw, use_null=False).detach()
        Lc=_consist(self.encoder, raw)
        ...
        # FIX 4: OT từ epoch>=20, tránh coincide với bất kỳ event nào ─────────
        ...
        # ── FIX 2: Diversity — mask_ns ĐÚNG (như v65-stable) ─────────────────
        # mask_ns[b,k] = True nếu k != k_star[b] (candidates hợp lệ)
        # Cần fill inf TẠI k_star để loại nó khỏi min
        # → ones, scatter False at k_star → masked_fill(~mask_ns, inf) fills k_star
        ms=mng[bi,ks]  # oracle trajectory [B,T,4]
        da=torch.stack([((ms-mng[:,k])**2).mean(dim=[1,2]).sqrt() for k in range(self.K)], dim=1)
        mask_ns=torch.ones(B, self.K, device=dev, dtype=torch.bool)  # ones: all valid
        mask_ns.scatter_(1, ks.unsqueeze(1), False)                   # False at k_star
        # masked_fill(~mask_ns) = fill inf where ~mask_ns=True = where k_star
        # → min(dim=1) = min dist to nearest OTHER mode (NOT k_star) ✅
        md=da.masked_fill(~mask_ns, float('inf')).min(dim=1).values
        Ld=F.relu(.40-md)
        ...
        # ── DPE loss với fixed v59 weights ─────────────────────────────────
        og=mg[bi,ks]   # oracle mode với gradient
        ...
        # ── FIX 3: Phase loss — v59 philosophy, KHÔNG LearnedWeights ──────
        # DPE weight 2.0 như v59 → trực tiếp drive ADE metric
        # Loss LUÔN DƯƠNG: LFM>0, LDPE>0, Lrank>=0, Ldir>0
        if ph == 1:
            # ── Phase 1: Generator warm-up, giống v59 ──
            # L_FM + 2.0*L_DPE là core, như v59's "l_fm + 2.0*st_trans_loss"
            # DPE weight 2.0 mạnh hơn v69's 0.5 → ADE giảm nhanh hơn
            Lt = LFM + 2.0*LDPE + 0.02*Lc

        elif ph == 2:
            # ── Phase 2: Selector training ──
            # FIX: KHÔNG fully frozen generator (BUG-3 fix)
            # 0.2*LFM giữ generator warm → modes vẫn move → selector có signal
            Lt = 0.5*Lrank + 0.3*Ldir + 0.2*LFM

        else:
            # ── Phase 3: Joint fine-tuning ──
            # DPE weight 1.0 (giảm từ 2.0 để balance với selector losses)
            Lt = LFM + 1.0*LDPE + 0.3*Lrank + 0.2*Ldir + 0.05*Lcoh + 0.02*Lc
        ...
        return dict(
            ...
            L_coh=_s(Lcoh), L_consist=_s(Lc),
            delta_mean=_s(delta.mean()),
            # Step weights để log/verify (luôn cố định)
            sw_6h =_s(step_w[0]),
            sw_12h=_s(step_w[1]),
            sw_72h=_s(step_w[-1]),
            phase=ph,
...
# Backward compat aliases
TCFlowMatchingV69 = TCFlowMatchingV70
TCFlowMatchingV68 = TCFlowMatchingV70
TCFlowMatchingV67 = TCFlowMatchingV70
TCFlowMatchingV65 = TCFlowMatchingV70
TCFlowMatching    = TCFlowMatchingV70
TCDiffusion       = TCFlowMatchingV70