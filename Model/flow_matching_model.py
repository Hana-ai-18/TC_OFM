...
# """
# Model/flow_matching_model.py  —— TC-FlowMatching v2.2 (train from scratch + XAI)
# ═══════════════════════════════════════════════════════════════════════════════

# FIXES từ review code (BUG-1 → BUG-9) — đã confirmed đúng:
#   ✅ BUG-1: AR conditioning dùng stage_abs (absolute) thay vì x_rel
#   ✅ BUG-2: Bỏ diversity_score bias outlier
#   ✅ BUG-3: TTA anchor broadcast [1,B,2] thay vì [B,2]
#   ✅ BUG-4: Checkpoint ensemble track periodic ckpts riêng
#   ✅ BUG-5: ar_enc/ar_gate không bị freeze cùng backbone
#   ✅ BUG-6: Cache _encode_raw 1 lần, chỉ gọi ar_enc/ar_gate trong AR loop
#   ✅ BUG-8: _reg_loss dùng sigma_inference thay vì sigma schedule
#   ✅ BUG-9: ADE log dùng sigma_inference

# ADJUSTMENTS cho train từ đầu (không resume từ v2.1):
#   [SCRATCH-1] sigma_max=0.12 (tăng từ 0.08) cho exploration đủ rộng
#   [SCRATCH-2] L_reg ramp: ep<20 lam=0 (tăng từ 10) — encoder cần ổn định trước
#   [SCRATCH-3] AR freeze: ar_enc/ar_gate freeze đến ep15, sau đó unfreeze
#               Lý do: khi backbone chưa học được gì (ep0-15), AR conditioning
#               từ random init là pure noise → cản trở velocity learning.
#               ep15+ backbone đã có signal → ar_enc bắt đầu học được.
#   [SCRATCH-4] lambda_reg=0.3 (tăng từ 0.2) vì cần ADE signal mạnh hơn
#               khi train từ đầu để guide velocity field
#   [SCRATCH-5] n_inference_steps=1 (1-step Euler thay vì 8-step)
#               Lý do: 1-step là x0 + v(x0) — đây chính xác là cách _reg_loss
#               được train (t=0, predict x1 directly). Consistent hơn.
#               8-step Euler với sigma=0.04 (noise nhỏ) không thêm nhiều accuracy
#               nhưng tốn 8× thời gian inference.

# XAI COMPONENT — theo đề xuất của thầy:
#   [XAI-1] Feature Attribution Map: gradient-based attention
#     Tính gradient của ADE prediction w.r.t. obs_traj features
#     → Biết bước nào trong obs trajectory ảnh hưởng nhiều nhất đến pred
#     → "Tại sao model predict bão đi hướng này?"

#   [XAI-2] Hard Score Explanation
#     hard_score_from_obs đã là XAI tự nhiên — decompose thành:
#     curvature_contrib, speed_var_contrib, dir_change_contrib
#     → Biết storm khó vì lý do gì (curving? accelerating? zig-zag?)

#   [XAI-3] AR Stage Contribution
#     Gate values từ ar_gate per stage → biết stage nào được model "tin tưởng"
#     gate ≈ 1.0: model dùng pred từ stage trước nhiều
#     gate ≈ 0.0: model ignore stage trước, chỉ dùng obs

#   [XAI-4] Ensemble Uncertainty
#     Std deviation của K=20 predictions → uncertainty map theo lead time
#     High std at 72h → model không chắc chắn về 72h forecast
# """
# from __future__ import annotations
...
#     """
#     5-type augmentation. Chỉ augment obs + gt coords, không augment image/env.
#     Val KHÔNG gọi hàm này.
#     """
#     if not torch.is_tensor(batch_list[0]):
...
#     elif r < 0.70:
#         # Rotation ±15°
#         angle = (torch.rand(1).item() - 0.5) * (math.pi / 6)
...
#         B = obs.shape[1]
#         if B >= 4:
#             alpha = 0.15 + 0.20 * torch.rand(1).item()
#             idx = torch.randperm(B, device=device)
#             obs_new = obs.clone()
...
#     else:
#         obs_new = obs.clone()
#         obs_new[..., :2] = obs[..., :2] + torch.randn_like(obs[..., :2]) * 0.005
#         bl[0] = obs_new
...
# def _ot_match_noise_gt(x0_flat, x1_flat, epsilon=0.05):
#     B = x0_flat.shape[0]
...
# class VelocityTransformer(nn.Module):
#     def __init__(self, pred_len=12, d_model=256, nhead=8, num_layers=4,
#                  dim_ff=512, dropout=0.1, d_cond=256):
#         super().__init__()
#         self.pred_len = pred_len
#         self.d_model  = d_model
...
#     def forward(self, x_t, t, cond, step_offset: int = 0):
#         """
#         x_t: [B, T, 2]
#         step_offset: bắt đầu từ step nào trong sequence 12 steps.
#           Training: step_offset=0 (toàn bộ 12 steps).
#           AR inference stage 0: step_offset=0 (steps 0-3, tức 6h-24h)
#           AR inference stage 1: step_offset=4 (steps 4-7, tức 30h-48h)
#           AR inference stage 2: step_offset=8 (steps 8-11, tức 54h-72h)
#         Đảm bảo pos_emb và step_emb đúng với vị trí thực của steps trong sequence.
#         """
#         B, T, _ = x_t.shape
#         # [ISSUE-1 FIXED] step_idx bắt đầu từ step_offset
#         step_idx = (torch.arange(T, device=x_t.device) + step_offset).unsqueeze(0).expand(B, -1)
#         # Clamp để tránh out-of-bounds nếu step_offset + T > pred_len
#         step_idx = step_idx.clamp(0, self.pred_len - 1)
#         x_emb = (self.traj_embed(x_t) + self.pos_emb[:, step_idx[0]] + self.step_emb(step_idx))
...
#         return self.out_proj(out) * torch.sigmoid(self.out_scale[step_idx[0]]).unsqueeze(0)


# # ─────────────────────────────────────────────────────────────────────────────
# #  ContextEncoder với AR context fusion + XAI hooks
# # ─────────────────────────────────────────────────────────────────────────────
...
#         # AR modules — [BUG-1 FIXED] nhận absolute coords
#         # [SCRATCH-3] Freeze đến ep15 khi train từ đầu (control từ train loop)
#         self.ar_enc = nn.Sequential(
#             nn.Linear(self.ar_stage_len * 2, d_cond // 2),
#             nn.GELU(),
#             nn.LayerNorm(d_cond // 2),
#             nn.Linear(d_cond // 2, d_cond),
#         )
...
#             nn.Sigmoid(),
#         )
#         # Init ar_gate output ≈ 0 → AR conditioning bắt đầu tắt, tự học khi unfreeze
#         nn.init.zeros_(self.ar_gate[0].weight)
#         nn.init.constant_(self.ar_gate[0].bias, -3.0)  # sigmoid(-3) ≈ 0.05
...
#     def forward(self, batch_list,
#                 hard_score: Optional[torch.Tensor] = None,
#                 prev_stage_abs: Optional[torch.Tensor] = None,
#                 return_gate: bool = False):
#         """
#         prev_stage_abs: [B, ar_stage_len, 2] ABSOLUTE normalized coords (BUG-1 FIXED)
#         return_gate: nếu True, trả về (cond, gate_values) cho XAI-3
#         """
#         raw   = self._encode_raw(batch_list)
#         ctx   = self.ctx_ln2(self.ctx_fc2(self.ctx_drop(raw)))
#         kfeat = self._kinematic_feat(batch_list[0][:, :, :2])
...
#         gate_val = None
#         if prev_stage_abs is not None:
#             B = cond.shape[0]
#             sl = self.ar_stage_len
#             flat = prev_stage_abs.reshape(B, -1).to(cond.dtype)
#             if flat.shape[1] < sl * 2:
#                 flat = F.pad(flat, (0, sl*2 - flat.shape[1]))
#             else:
#                 flat = flat[:, :sl*2]
#             ar_feat  = self.ar_enc(flat)
#             gate_val = self.ar_gate(torch.cat([cond, ar_feat], dim=-1))  # [B, d_cond]
#             cond     = cond + gate_val * ar_feat

#         if return_gate:
#             return cond, gate_val
#         return cond


# # ─────────────────────────────────────────────────────────────────────────────
# #  Hard sample scoring + XAI-2 decomposition
# # ─────────────────────────────────────────────────────────────────────────────
...
#     """
#     [XAI-2] return_components=True → trả về dict với từng component
#     Giải thích tại sao 1 storm được coi là "hard":
#       curvature: bão đang xoay nhiều (recurvature)
#       speed_var: tốc độ bão thay đổi nhiều (acceleration/deceleration)
#       dir_change: có nhiều bước rẽ lớn (>20°)
#     """
#     T, B   = obs_traj_norm.shape[0], obs_traj_norm.shape[1]
#     device = obs_traj_norm.device
#     if T < 3:
...
#     score = (0.4*curvature + 0.3*speed_var + 0.3*dir_change).clamp(0.0, 1.0)

#     if return_components:
#         return score, {
#             "curvature":  curvature,   # bão đang recurve
#             "speed_var":  speed_var,   # tốc độ thay đổi
#             "dir_change": dir_change,  # rẽ đột ngột
#         }
#     return score
...
#                    obs_norm: torch.Tensor) -> torch.Tensor:
#     """
#     2-component: speed_continuity + heading_smoothness.
#     BUG-2 FIXED: bỏ diversity_score (bias outlier).
#     """
#     B, device = traj_norm.shape[1], traj_norm.device
#     traj_deg  = _norm_to_deg(traj_norm)
...
#         obs_deg  = _norm_to_deg(obs_norm)
#         obs_spd  = _step_speeds_kmh(obs_deg)
#         w_obs    = torch.exp(torch.linspace(0.0, 1.0, obs_spd.shape[0], device=device))
#         v_ref    = (obs_spd * w_obs.unsqueeze(1)).sum(0) / w_obs.sum()
#         pred_spd = _step_speeds_kmh(traj_deg)
#         v_sigma  = v_ref.clamp(min=5.0) * 0.6
#         speed_score = torch.exp(
#             -((pred_spd - v_ref.unsqueeze(0)) / v_sigma.unsqueeze(0)).pow(2).mean(0) * 0.5)
#     elif traj_deg.shape[0] >= 2:
...
#     if traj_deg.shape[0] >= 3:
#         vel  = traj_deg[1:] - traj_deg[:-1]
#         h1   = torch.atan2(vel[:-1,:,1], vel[:-1,:,0])
#         h2   = torch.atan2(vel[1:, :,1], vel[1:, :,0])
#         dh   = (h2 - h1).abs()
#         dh   = torch.where(dh > math.pi, 2*math.pi - dh, dh)
#         heading_score = torch.exp(-(dh - math.pi/4).clamp(min=0.0).mean(0) * 2.0)
#     else:
#         heading_score = torch.ones(B, device=device)

#     return (speed_score.pow(0.55) * heading_score.pow(0.45)).clamp(min=1e-6)


# # ─────────────────────────────────────────────────────────────────────────────
# #  [XAI-1] Gradient-based feature attribution
# # ─────────────────────────────────────────────────────────────────────────────

# def compute_obs_attribution(model, batch_list, device,
#                              target_step: int = 11) -> torch.Tensor:
#     """
#     [XAI-1] Tính gradient của ADE tại target_step w.r.t. obs_traj.

#     Output: attribution [T_obs, B] — mỗi obs timestep ảnh hưởng bao nhiêu
#     đến prediction tại target_step (mặc định step 11 = 72h).

#     Cách dùng:
#         attr = compute_obs_attribution(model, batch_list, device, target_step=11)
#         # attr[t, b] = importance của obs step t cho storm b tại 72h
#         # attr[-1, b] lớn: last obs ảnh hưởng nhiều (thường đúng)
#         # attr[0, b] lớn: obs 48h trước vẫn quan trọng (unusual, interesting)

#     Trong báo cáo XAI: visualize attr như heatmap để giải thích model decision.
#     """
#     raw_model = _unwrap_model(model)
#     raw_model.eval()

#     obs_traj = batch_list[0].detach().clone().requires_grad_(True)
#     bl_grad  = list(batch_list)
#     bl_grad[0] = obs_traj

#     with torch.enable_grad():
#         h_score  = hard_score_from_obs(obs_traj[:, :, :2].detach())
#         cond     = raw_model.encoder(bl_grad, hard_score=h_score, prev_stage_abs=None)
#         last_obs = obs_traj[-1, :, :2]

#         x0   = torch.randn(obs_traj.shape[1], raw_model.pred_len, 2,
#                            device=device) * raw_model.sigma_inference
#         t0   = torch.zeros(obs_traj.shape[1], device=device)
#         v    = raw_model.velocity(x0, t0, cond)
#         pred_rel = x0 + v   # [B, T, 2] relative coords

#         # [BUG-B FIXED] Loss = L2 displacement magnitude tại target_step.
#         # pred_rel[:, target_step, :] là displacement từ last_obs.
#         # Gradient w.r.t. obs_traj → influence của mỗi obs timestep
#         # lên magnitude của prediction displacement. Proxy đúng về mặt XAI.
#         displacement_mag = pred_rel[:, target_step, :].norm(dim=-1)  # [B]
#         loss_xai = displacement_mag.mean()
#         loss_xai.backward()

#     if obs_traj.grad is not None:
#         attr = obs_traj.grad[:, :, :2].norm(dim=-1)   # [T_obs, B]
#         attr = attr / (attr.sum(0, keepdim=True) + 1e-8)  # normalize per storm
#     else:
#         attr = torch.zeros(obs_traj.shape[0], obs_traj.shape[1], device=device)
...
#     """
#     [XAI-4] Tính uncertainty từ K ensemble predictions.

#     all_traj: [K, T, B, 2] absolute normalized coords

#     Returns dict:
#         std_per_step: [T, B] — std deviation theo km tại mỗi lead time
#         uncertainty_ratio: [B] — 72h std / 12h std (>1 = uncertainty tăng với time)
#         high_uncertainty_mask: [B] bool — storm nào có 72h uncertainty cao
#     """
#     all_deg = _norm_to_deg(all_traj)   # [K, T, B, 2]
#     K, T, B = all_deg.shape[:3]

#     mean_traj = all_deg.mean(0)        # [T, B, 2]
#     # Compute std in km using haversine
...
#     step_12h = min(1, T-1)   # step index for 12h
#     step_72h = min(11, T-1)  # step index for 72h

#     uncertainty_ratio = (std_km[step_72h] + 1e-3) / (std_km[step_12h] + 1e-3)
#     threshold = std_km[step_72h].median() * 1.5
#     high_uncertainty_mask = std_km[step_72h] > threshold

#     return {
#         "std_per_step":        std_km,              # [T, B] km
#         "uncertainty_ratio":   uncertainty_ratio,   # [B]
#         "high_uncertainty":    high_uncertainty_mask,  # [B] bool
...
# # ─────────────────────────────────────────────────────────────────────────────
# #  TCFlowMatching v2.2 — train from scratch
# # ─────────────────────────────────────────────────────────────────────────────

# class TCFlowMatching(nn.Module):

#     def __init__(self, pred_len=12, obs_len=8, unet_in_ch=13,
#                  d_cond=256, d_model=256, nhead=8, num_dec_layers=4,
#                  dim_ff=512, dropout=0.1,
#                  sigma_min=0.04,
#                  sigma_max=0.12,        # [SCRATCH-1] tăng từ 0.08 → 0.12
#                  lambda_reg=0.3,        # [SCRATCH-4] tăng từ 0.2 → 0.3
#                  use_ot=True, ot_epsilon=0.05,
#                  use_ema=True, ema_decay=0.995,
#                  n_inference_steps=1,   # [SCRATCH-5] 1-step Euler
#                  n_ensemble=20,
#                  sigma_inference=0.04,
#                  ar_inference=True, ar_stage_len=4,
#                  ar_freeze_until=15,    # [SCRATCH-3] freeze ar_enc/ar_gate đến ep này
#                  **kwargs):
#         super().__init__()
#         assert pred_len % ar_stage_len == 0
#         self.pred_len          = pred_len
...
#         self.n_ensemble        = n_ensemble
#         self.sigma_inference   = sigma_inference
#         self.ar_inference      = ar_inference
#         self.ar_stage_len      = ar_stage_len
#         self.n_stages          = pred_len // ar_stage_len
#         self.ar_freeze_until   = ar_freeze_until
...
#     def set_ar_requires_grad(self, requires_grad: bool):
#         """[SCRATCH-3] Control AR module freeze từ train loop."""
#         for p in list(self.encoder.ar_enc.parameters()) + \
#                  list(self.encoder.ar_gate.parameters()):
...
#         """
#         [SCRATCH-1] sigma_max=0.12 (train từ đầu cần exploration rộng hơn).
#         Cosine decay từ sigma_max → sigma_min trong ep5→ep50.
#         """
...
#     def _get_step_weights(self, T, device):
#         """Exponential weights: 72h weight ≈ 12× so với 12h."""
#         steps = torch.arange(T, device=device, dtype=torch.float32)
#         w     = torch.exp(2.5 * steps / T)
#         w     = w / w.mean()
#         return w.unsqueeze(1)  # [T, 1]

#     def _reg_loss(self, x1_matched, last_obs, cond, hard_score=None):
#         """
#         ADE signal tại t=0 với sigma_inference (BUG-8 FIXED).
#         Exponential step weights (FIX-M3).
#         """
#         B, T, _ = x1_matched.shape
#         device  = x1_matched.device

#         x0 = torch.randn_like(x1_matched) * self.sigma_inference
#         t0 = torch.zeros(B, device=device)
#         v  = self.velocity(x0, t0, cond)
#         x1_pred_abs = self._from_relative(x0 + v, last_obs)
#         x1_gt_abs   = self._from_relative(x1_matched, last_obs)

#         pred_deg = _norm_to_deg(x1_pred_abs.permute(1,0,2))
#         gt_deg   = _norm_to_deg(x1_gt_abs.permute(1,0,2))
#         dist     = _haversine_deg(pred_deg, gt_deg)   # [T, B]

#         sw = self._get_step_weights(dist.shape[0], device)
#         if hard_score is not None:
#             sample_w = (1.0 + hard_score.to(device).to(dist.dtype)).unsqueeze(0)
#         else:
#             sample_w = torch.ones(1, B, device=device, dtype=dist.dtype)
...
#         obs_traj = batch_list[0]; gt_traj = batch_list[1]
...
#         h_score = hard_score_from_obs(obs_traj[:, :, :2])
#         cond    = self.encoder(batch_list, hard_score=h_score, prev_stage_abs=None)
...
#         # L_reg — [SCRATCH-2] ramp bắt đầu ep20 thay vì ep10
#         if epoch < 20:
#             lam_reg = 0.0
#         elif epoch < 40:
#             lam_reg = self.lambda_reg * (epoch-20) / 20.0
#         else:
#             lam_reg = self.lambda_reg

#         if lam_reg > 0.0:
#             l_reg = self._reg_loss(x1_matched, last_obs, cond, hard_score=h_score)
#         else:
#             l_reg = x0.new_zeros(())
...
#         with torch.no_grad():
#             # [BUG-9 FIXED] ADE log dùng sigma_inference
#             x0_log = torch.randn_like(x1_rel) * self.sigma_inference
#             v_log  = self.velocity(x0_log, torch.zeros(B, device=device), cond)
#             x1_log = self._from_relative(x0_log + v_log, last_obs)
#             ade_log = _haversine_deg(_norm_to_deg(x1_log.permute(1,0,2)),
#                                      _norm_to_deg(x1_gt.permute(1,0,2))).mean().item()

#         return {
#             "total": total, "l_cfm": l_cfm.item(),
#             "l_reg": l_reg.item() if torch.is_tensor(l_reg) else 0.0,
#             "lam_reg": lam_reg, "sigma": sigma, "ade_1step": ade_log,
#             "hard_score_mean": float(h_score.mean()), "hard_score_max": float(h_score.max()),
#             "l_fm": l_cfm.item(), "dpe": 0.0, "heading": 0.0, "vel_reg": 0.0,
#             "speed": 0.0, "accel": 0.0, "fm_mse": l_cfm.item(),
#             "l_hard_total": 0.0, "n_hard": 0, "alpha_hard": 0.0,
#             "l_sel_total": 0.0, "speed_head_l": 0.0, "l_score": 0.0,
#         }
...
#                use_ar=None, return_xai=False, **kwargs):
#         """
#         AR inference với BUG-6 FIXED: cache _encode_raw 1 lần.

#         return_xai=True: trả về thêm dict XAI info cho visualization
#           - gate_values: [n_stages-1, B, d_cond] — AR gate per stage per storm
#           - uncertainty: dict từ compute_ensemble_uncertainty
#           - hard_components: dict từ hard_score_from_obs(return_components=True)
#         """
#         K      = num_ensemble or self.n_ensemble
#         N      = ddim_steps   or self.n_inference_steps
#         dt     = 1.0 / max(N, 1)
#         use_ar = self.ar_inference if use_ar is None else use_ar
...
#         cond_base = self.encoder(batch_list, hard_score=h_score, prev_stage_abs=None)
...
#                 for step in range(N):
#                     t_b   = torch.full((B,), step*dt, device=device)
#                     # step_offset=0: toàn bộ pred_len steps, pos_emb[0..pred_len-1]
#                     x_rel = (x_rel + dt * self.velocity(x_rel, t_b, cond_base, step_offset=0)).clamp(-3,3)
#                 all_traj.append(self._from_relative(x_rel, last_obs).permute(1,0,2))
#             scores    = torch.stack([_physics_score(t, obs_norm) for t in all_traj], 0)
#             all_t     = torch.stack(all_traj, 0)
#             pred_mean = self._weighted_top3(all_t, scores, B)
...
#         # AR path — [BUG-6 FIXED] cache _encode_raw 1 lần
#         # [ISSUE-3 FIXED] Tất cả stages dùng coordinate relative to last_obs
#         # để nhất quán với training (velocity được train với x_rel relative to last_obs).
#         # Stage predictions được concat trực tiếp trong relative space,
#         # không dùng anchor shifting (vốn tạo ra coordinate mismatch).
...
#         cond_base_ar = self.encoder.fuse(
#             torch.cat([base_ctx, kfeat, hfeat], dim=-1))

#         all_full_traj = []
#         all_gate_vals = []

#         for k in range(K):
#             stage_preds_rel = []   # [ISSUE-3 FIXED] relative to last_obs
#             gate_vals_k     = []
#             cond_k          = cond_base_ar

#             for stage in range(self.n_stages):
#                 sl = self.ar_stage_len
#                 step_offset = stage * sl   # [ISSUE-1 FIXED] đúng position trong 12 steps
#                 # Noise in space relative to last_obs — nhất quán với training
#                 x_rel = torch.randn(B, sl, 2, device=device) * self.sigma_inference
#                 for step in range(N):
#                     t_b   = torch.full((B,), step*dt, device=device)
#                     v     = self.velocity(x_rel, t_b, cond_k, step_offset=step_offset)
#                     x_rel = (x_rel + dt * v).clamp(-3.0, 3.0)

#                 # x_rel: displacement từ last_obs cho steps sl*stage → sl*(stage+1)
#                 stage_preds_rel.append(x_rel)

#                 # Convert sang absolute để truyền vào ar_enc (cần absolute position)
#                 stage_abs = x_rel + last_obs.unsqueeze(1)  # [B, sl, 2] absolute

#                 if stage < self.n_stages - 1:
#                     flat = stage_abs.detach().reshape(B, -1).to(cond_base_ar.dtype)
#                     if flat.shape[1] < sl * 2:
#                         flat = F.pad(flat, (0, sl*2 - flat.shape[1]))
#                     else:
#                         flat = flat[:, :sl*2]
#                     ar_feat  = self.encoder.ar_enc(flat)
#                     gate_val = self.encoder.ar_gate(
#                         torch.cat([cond_base_ar, ar_feat], dim=-1))
#                     cond_k   = cond_base_ar + gate_val * ar_feat
#                     gate_vals_k.append(gate_val.mean(-1))  # [B]

#             # Concat stages trong relative space → convert về absolute
#             full_rel = torch.cat(stage_preds_rel, dim=1)           # [B, pred_len, 2]
#             full_abs = self._from_relative(full_rel, last_obs)      # [B, pred_len, 2]
#             all_full_traj.append(full_abs.permute(1, 0, 2))         # [pred_len, B, 2]
#             if gate_vals_k:
#                 all_gate_vals.append(torch.stack(gate_vals_k, dim=0))

#         scores    = torch.stack([_physics_score(t, obs_norm) for t in all_full_traj], 0)
#         all_t     = torch.stack(all_full_traj, 0)
#         pred_mean = self._weighted_top3(all_t, scores, B)

#         if return_xai:
#             xai = compute_ensemble_uncertainty(all_t)
#             if all_gate_vals:
#                 # Average gate values across K samples
#                 gate_stack = torch.stack(all_gate_vals, dim=0)   # [K, n_stages-1, B]
#                 xai["gate_values"] = gate_stack.mean(0)          # [n_stages-1, B]
#                 xai["gate_mean"] = float(gate_stack.mean().item())
#             _, hard_comps = hard_score_from_obs(obs_norm, return_components=True)
#             xai["hard_components"] = hard_comps
#             return pred_mean, torch.zeros_like(pred_mean), all_t, xai
...
#     def _weighted_top3(self, all_t, scores, B):
#         K_actual  = all_t.shape[0]
#         top_k     = min(3, K_actual)
#         top_idx   = scores.topk(top_k, dim=0).indices
...
class ContextEncoder(nn.Module):
    ...
    def _kinematic_feat(self, obs_traj_norm: torch.Tensor) -> torch.Tensor:
        ...
        if T_obs >= 2:
            ...
            kine = obs_traj_norm.new_zeros(self.obs_len, B, 6)
        if kine.shape[0] < self.obs_len:
            kine = torch.cat([obs_traj_norm.new_zeros(self.obs_len-kine.shape[0], B, 6), kine], 0)
        else:
            kine = kine[-self.obs_len:]
        return self.vel_obs_enc(kine.permute(1,0,2).reshape(B,-1))
...
class TCFlowMatching(nn.Module):
    ...
    def _reg_loss(self, x1_rel: torch.Tensor, last_obs: torch.Tensor,
        ...
        # [NEW-4] Exp weights: w_i = exp(2.5 * i / T), normalized
        # i=0 (6h): w≈1.0, i=11 (72h): w≈e^2.5≈12.2, mean normalized → 72h ≈ 6×
        T_actual = dist.shape[0]
        steps = torch.arange(T_actual, device=device, dtype=dist.dtype)
        sw    = torch.exp(2.5 * steps / T_actual)
        sw    = (sw / sw.mean()).unsqueeze(1)  # [T, 1]
    ...
    def sample(self, batch_list, num_ensemble: Optional[int] = None,
        ...
        for _ in range(K):
            x_rel = torch.randn(B, self.pred_len, 2, device=device) * self.sigma_inference
            if N <= 1:
                # 1-shot — nhất quán với L_reg trong training
        ...
        if return_xai:
            ...
            # [XAI-3] Physics score components cho top-1 sample per storm
            # Giải thích: tại sao sample này được chọn (speed? smooth? heading?)
            top1_idx = scores.argmax(0)  # [B]
            speed_scores  = torch.zeros(B, device=device)
            smooth_scores = torch.zeros(B, device=device)
            head_scores   = torch.zeros(B, device=device)
            for b in range(B):
                k_best = int(top1_idx[b].item())
                traj_b = all_traj[k_best][:, b:b+1, :]  # [T, 1, 2]
                obs_b  = obs_norm[:, b:b+1, :]            # [T_obs, 1, 2]
                traj_deg_b = _norm_to_deg(traj_b)
                # Speed component
                obs_deg_b = _norm_to_deg(obs_b)
                obs_spd_b = _step_speeds_kmh(obs_deg_b)
                v_ref_b   = obs_spd_b.mean(0)
                pred_spd_b = _step_speeds_kmh(traj_deg_b)
                v_sigma_b  = v_ref_b.clamp(min=5.) * 0.5
                speed_scores[b]  = torch.exp(-((pred_spd_b - v_ref_b)/ v_sigma_b.clamp(1e-6)).pow(2).mean() * 0.5)
                # Smooth component
                if traj_deg_b.shape[0] >= 3:
                    vel_b = traj_deg_b[1:] - traj_deg_b[:-1]
                    smooth_scores[b] = torch.exp(-(vel_b[1:]-vel_b[:-1]).norm(dim=-1).mean() * 5.)
                else:
                    smooth_scores[b] = torch.ones(1, device=device)
                # Heading component
                if obs_b.shape[0] >= 2:
                    obs_v  = obs_b[-1, 0, :2] - obs_b[-2, 0, :2]
                    pred_v = traj_b[0, 0, :2] - obs_b[-1, 0, :2]
                    cos_s  = F.normalize(obs_v.unsqueeze(0), dim=-1, eps=1e-6) @ \
                             F.normalize(pred_v.unsqueeze(0), dim=-1, eps=1e-6).T
                    head_scores[b] = torch.exp((cos_s.squeeze().clamp(-1,1) - 1.) * 3.)

            xai["physics_components"] = {
                "speed":   speed_scores,   # [B] — score cao = tốc độ phù hợp
                "smooth":  smooth_scores,  # [B] — score cao = ít zigzag
                "heading": head_scores,    # [B] — score cao = heading nhất quán
            }
