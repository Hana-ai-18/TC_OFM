...
"""
Model/flow_matching_model.py — v35  CLEAN + 72H FOCUSED
═══════════════════════════════════════════════════════════════════

THAY ĐỔI SO VỚI v34fix:
  1. Sync với losses.py v35: bỏ velocity/recurv, thêm hard_72h/direct_mse
  2. Training log in hard_72h và direct_mse thay vì vel/recurv
  3. Sigma floor giữ 0.06 (không thay đổi — đã đúng từ v34fix)
  4. EMA unwrap _orig_mod giữ nguyên (đã fix từ v34fix)
  5. Ensemble diversity: initial noise σ × 2.5 giữ nguyên
  6. w_fm schedule: giảm chậm hơn (chia 80 thay vì 60) để FM loss
     không dominate quá sớm khi epoch còn nhỏ

KHÔNG thay đổi:
  - VelocityField architecture (FNO3D + Mamba + Transformer)
  - Sampling logic (ensemble + importance weighting)
  - Physics correction (β-drift + steering drift)
  - EMA / SWA / compile flow
"""
from __future__ import annotations
...
def _norm_to_deg_fn(t: torch.Tensor) -> torch.Tensor:
    """Normalized [lon, lat] → degrees."""
    out = t.clone()
    out[..., 0] = (t[..., 0] * 50.0 + 1800.0) / 10.0
    out[..., 1] = (t[..., 1] * 50.0) / 10.0
...
def _unwrap_model(model: nn.Module) -> nn.Module:
    """Unwrap torch.compile wrapper để lấy state_dict gốc."""
    if hasattr(model, '_orig_mod'):
        return model._orig_mod
    return model
...
# ══════════════════════════════════════════════════════════════════════════════
#  EMA wrapper — unwrap _orig_mod để tránh KeyError từ torch.compile
# ══════════════════════════════════════════════════════════════════════════════
...
class EMAModel:
    ...
    torch.compile() đổi key từ "net.pos_enc" → "_orig_mod.net.pos_enc".
    Fix: LUÔN unwrap _orig_mod trước khi đọc/ghi state_dict.
    """
    def __init__(self, model: nn.Module, decay: float = 0.999):
        self.decay = decay
        m = _unwrap_model(model)
...
# ══════════════════════════════════════════════════════════════════════════════
#  VelocityField — không thay đổi architecture
# ══════════════════════════════════════════════════════════════════════════════
...
class VelocityField(nn.Module):
        ...
        def _safe_get(key, default_val=0.0):
            ...
            if v.dim() >= 2:
                while v.dim() > 1:
                    v = v.mean(-1)
            if v.shape[0] != B:
                v = v.view(-1)[:B] if v.numel() >= B else \
                    torch.full((B,), default_val, device=device)
            return v
    ...
    def _steering_drift(self, x_t, env_data):
        """Inject 500hPa steering flow as drift velocity."""
        if env_data is None:
            return torch.zeros_like(x_t)
    ...
    def _decode(self, x_t, t, ctx, vel_obs_feat=None,
                steering_feat=None, env_data=None):
        B     = x_t.shape[0]
        t_emb = F.gelu(self.time_fc1(self._time_emb(t)))
        t_emb = self.time_fc2(t_emb)
...
class TCFlowMatching(nn.Module):
    ...
    def __init__(self, pred_len=12, obs_len=8, sigma_min=0.02,
        ...
        self.use_ema   = use_ema
        self.ema_decay = ema_decay
        self._ema      = None   # lazy init via init_ema()

    def init_ema(self):
        """Gọi một lần sau khi model đã lên device, TRƯỚC torch.compile."""
        if self.use_ema:
            self._ema = EMAModel(self, decay=self.ema_decay)
    ...
    def _mixup_aug(batch_list, alpha=0.2, p=0.2):
        ...
        lam = max(lam, 1 - lam)
        if torch.is_tensor(aug[0]) and aug[0].shape[1] >= 2:
            perm = torch.randperm(aug[0].shape[1], device=aug[0].device)
            for idx in [0, 1, 7, 8]:
                if torch.is_tensor(aug[idx]):
                    aug[idx] = lam * aug[idx] + (1 - lam) * aug[idx][:, perm]
        return aug
    ...
    def get_loss_breakdown(self, batch_list, step_weight_alpha=0.0, epoch=0):
        ...
        # ── Sigma schedule ─────────────────────────────────────────────────
        # floor 0.06 để ensemble diversity đủ cao khi inference
        if epoch < 15:
            current_sigma = 0.15
        elif epoch < 40:
            current_sigma = 0.15 - (epoch - 15) / 25.0 * 0.09   # 0.15→0.06
        else:
            current_sigma = 0.06   # floor: không giảm thêm
        ...
        # ── Geo losses (v35: bỏ velocity/recurv, thêm hard_72h/direct_mse)
        loss_dict = compute_total_loss(
            pred_deg=pred_deg,
            gt_deg=gt_deg,
            env_data=env_data,
        ...
        # w_fm giảm chậm hơn: chia 80 (thay vì 60) để FM loss đủ thời gian học
        w_fm  = max(0.3, 1.0 - epoch / 80.0)
        total = w_fm * l_fm_mse + loss_dict["total"]
        ...
        return dict(
            total       = total,
            fm_mse      = l_fm_mse.item(),
            # loss names khớp với logging trong trainer
            mse_hav     = loss_dict["mse_hav_horizon"],
            multi_scale = loss_dict["multi_scale"],
            endpoint    = loss_dict["endpoint"],
            hard_72h    = loss_dict["hard_72h"],      # MỚI
            direct_mse  = loss_dict["direct_mse"],    # MỚI
            shape       = loss_dict["shape"],
            heading     = loss_dict["heading"],
            steering    = loss_dict["steering"],
            ens_consist = l_ens.item() if torch.is_tensor(l_ens) else float(l_ens),
            sigma       = current_sigma,
            w_fm        = w_fm,
            # backward-compat stubs (trainer cũ có thể đọc)
            velocity=0.0, recurv=0.0, long_range=0.0, fde=0.0,
            w_lr=0.0, w_fde=0.0, fm=0.0, short_range=0.0,
            cont=0.0, pinn=0.0, spread=0.0, continuity=0.0,
            step=0.0, disp=0.0, recurv_ratio=0.0,
        )
    ...
    def sample(self, batch_list, num_ensemble=50, ddim_steps=20,
        ...
        traj_s, me_s, scores = [], [], []
        for ens_i in range(num_ensemble):
            # initial noise: sigma_min × 2.5 để ensemble diversity cao
            x_t = torch.randn(B, T, 4, device=device) * (self.sigma_min * 2.5)
            for step in range(ddim_steps):
                t_b = torch.full((B,), step * dt, device=device)
                ns  = self.ctx_noise_scale * 2.0 if step < 3 else 0.0
