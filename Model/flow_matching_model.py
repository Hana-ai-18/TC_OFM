...
"""
Model/flow_matching_model.py  ── v31 - LSTMCELL HIERARCHICAL
=============================================================
CHIẾN LƯỢC: SR (LSTMCell step 1-4) → LR (LSTMCell step 5-12)

KEY CHANGES từ v30:
  1. ShortRangeHead: GRU → multi-layer LSTMCell (paper Algorithm 3)
     - Explicit h_t, c_t init từ context (paper §2.5)
     - Truncated BPTT: detach states between timesteps
     - 3-layer LSTMCell với dropout=0.1
  2. LongRangeHead (thay VelocityField FM): GRUCell với dropout=0.2
     - Conditioned on SR step-4 endpoint (paper Algorithm 4 adapted)
     - Ensemble via MC-Dropout (inference với dropout ON)
     - Không còn ODE/CFM sampling
  3. TCFlowMatching: giữ hierarchical SR→LR
     - Sampling: dropout ensemble thay ODE
     - Loss: giữ CRPS/AFCRPS nhưng feed từ dropout ensemble
  4. VelocityField._context() GIỮ NGUYÊN hoàn toàn
     (FNO3D + Mamba + Env_net → raw_ctx [B, 512])

GIỮ NGUYÊN:
  - FNO3DEncoder, DataEncoder1D_Mamba, Env_net
  - _context() và toàn bộ encoder pipeline
  - losses.py interface (AFCRPS, continuity, SR loss)
  - train_flowmatching.py (không cần sửa)

PAPER REFERENCE:
  Rahman et al. (2025) Results in Engineering 26, 105009
  "Tropical cyclone track prediction harnessing deep learning algorithms"
  - Algorithm 3: LSTMCell forward pass
  - Algorithm 4: GRUCell forward pass with dropout
"""
from __future__ import annotations
...
# ══════════════════════════════════════════════════════════════════════════════
#  ShortRangeHead  ── v31: multi-layer LSTMCell (paper Algorithm 3)
# ══════════════════════════════════════════════════════════════════════════════

class ShortRangeHead(nn.Module):
    """
    LSTMCell-based short-range predictor (step 1-4, 6h-24h).

    Paper Algorithm 3 adaptation:
      - Explicit h_t, c_t initialisation từ context
      - Truncated BPTT: detach() sau mỗi timestep
      - 3-layer LSTMCell, hidden_dim=256
      - Stepwise feed-forward: output vị trí → input bước tiếp
    """
    N_STEPS   = 4
    MAX_DELTA = 0.48
    ...
    def __init__(
        ...
        # ── Khởi tạo hidden state từ context (paper: §2.5 init h_t=0, c_t=0
        #    nhưng chúng ta dùng context để warm-start layer 1)
        self.ctx_h_proj = nn.Sequential(
            nn.Linear(raw_ctx_dim, hidden_dim),
            nn.Tanh(),
        )
        self.ctx_c_proj = nn.Sequential(
            nn.Linear(raw_ctx_dim, hidden_dim),
            nn.Tanh(),
        )

        # ── Velocity encoder từ obs (giữ lại từ v30)
        self.vel_enc = nn.Sequential(
            nn.Linear(obs_len * 2, 128),
            nn.GELU(),
            nn.LayerNorm(128),
        )

        # ── LSTMCell stack (paper Algorithm 3)
        #    Input layer 1: pos(2) + vel(2) + vel_feat(128) + ctx_feat(hidden)
        input_dim_l1 = 2 + 2 + 128 + hidden_dim
        self.cells = nn.ModuleList()
        self.cells.append(nn.LSTMCell(input_dim_l1, hidden_dim))
        for _ in range(n_layers - 1):
            self.cells.append(nn.LSTMCell(hidden_dim, hidden_dim))

        self.dropout = nn.Dropout(dropout)

        # ── Readout layer (paper Algorithm 3 line 10: output = fc(h_t))
        ...
        self.step_scale = nn.Parameter(torch.ones(self.N_STEPS) * 0.25)
        self._init_weights()
    ...
    def forward(
        ...
        obs_traj: torch.Tensor,   # [T_obs, B, 2] normalised
    ) -> torch.Tensor:            # [4, B, 2] normalised
        B      = raw_ctx.shape[0]
        device = raw_ctx.device

        # ── Velocity feature từ obs
        T_obs = obs_traj.shape[0]
        if T_obs >= 2:
            vels = obs_traj[1:] - obs_traj[:-1]
        else:
            vels = torch.zeros(1, B, 2, device=device)

        target_v = self.obs_len
        if vels.shape[0] < target_v:
            pad  = torch.zeros(target_v - vels.shape[0], B, 2, device=device)
            vels = torch.cat([pad, vels], dim=0)
        else:
            vels = vels[-target_v:]

        vel_feat = self.vel_enc(vels.permute(1, 0, 2).reshape(B, -1))  # [B, 128]

        # ── Khởi tạo hidden và cell states (paper Algorithm 3 line 11-12)
        h_list = []
        c_list = []
        h_list.append(self.ctx_h_proj(raw_ctx))          # layer 1: từ context
        c_list.append(self.ctx_c_proj(raw_ctx))
        for _ in range(self.n_layers - 1):               # layer 2,3: zero init
            h_list.append(torch.zeros(B, self.hidden_dim, device=device))
            c_list.append(torch.zeros(B, self.hidden_dim, device=device))

        # Context feature (dùng lại cho mỗi bước)
        ctx_feat = h_list[0]
        ...
        for i in range(self.N_STEPS):
            ...
            # ── Input cho layer 1 (paper Algorithm 3 line 5: input_t)
            inp = torch.cat([cur_pos, last_vel, vel_feat, ctx_feat], dim=-1)

            # ── Layer 1 LSTMCell (paper Algorithm 3 line 9: h_t,c_t = lstm(input_t, ...))
            h_list[0], c_list[0] = self.cells[0](
                inp,
                (h_list[0].detach(), c_list[0].detach()),  # Truncated BPTT
            )
            ...
            for j in range(1, self.n_layers):
                h_list[j], c_list[j] = self.cells[j](
                    h,
                    (h_list[j].detach(), c_list[j].detach()),
                )
                h = self.dropout(h_list[j])

            # ── Readout (paper Algorithm 3 line 10: output = fc(h_t))
            scale    = torch.sigmoid(self.step_scale[i]) * self.MAX_DELTA
            raw_out  = self.fc(h)                        # [B, 2] delta
            delta    = (raw_out * scale).clamp(-self.MAX_DELTA, self.MAX_DELTA)
...
# ══════════════════════════════════════════════════════════════════════════════
#  LongRangeHead  ── v31: GRUCell với dropout (paper Algorithm 4 adapted)
# ══════════════════════════════════════════════════════════════════════════════

class LongRangeHead(nn.Module):
    """
    GRUCell-based long-range predictor (step 5-12, 30h-72h).

    Paper Algorithm 4 adaptation:
      - GRUCell với dropout=0.2 (paper §2.8: GRU dùng dropout rate 0.2)
      - Conditioned on SR step-4 endpoint + velocity (handoff info)
      - Truncated BPTT: detach() sau mỗi timestep
      - Ensemble via MC-Dropout: inference với dropout=ON
      - Feed-forward mode: output position → next step input

    Lý do chọn GRUCell cho long-range (paper §4):
      "GRU appears to perform well in early-stage cyclone tracking"
      → Phù hợp với FM zone (step 5-12) cần balance accuracy/diversity
    """
    ...
    def __init__(
        ...
        n_layers:    int   = 3,
        fm_pred_len: int   = 8,
        dropout:     float = 0.20,   # paper §2.8: dropout=0.2 cho GRU
    ):
        super().__init__()
        self.hidden_dim  = hidden_dim
        self.n_layers    = n_layers
        ...
        # ── Context → hidden state init
        self.ctx_h_proj = nn.Sequential(
            nn.Linear(raw_ctx_dim, hidden_dim),
            nn.Tanh(),
        )

        # ── SR anchor: encode SR step-4 pos + vel để conditioned FM
        #    (giống v30 compute_sr_anchor_emb nhưng inject vào h_0)
        self.sr_anchor_proj = nn.Sequential(
            nn.Linear(4, 128),      # sr_pos(2) + sr_vel(2)
            nn.GELU(),
            nn.Linear(128, hidden_dim),
            nn.Tanh(),
        )

        # ── GRUCell stack (paper Algorithm 4)
        #    Input: cur_pos(2) + sr_ref_pos(2) → concat với hidden
        input_dim_l1 = 2 + 2 + hidden_dim   # cur_pos + sr_ref + ctx_feat
        self.cells = nn.ModuleList()
        self.cells.append(nn.GRUCell(input_dim_l1, hidden_dim))
        for _ in range(n_layers - 1):
            self.cells.append(nn.GRUCell(hidden_dim, hidden_dim))

        # dropout=0.2 (paper Algorithm 4 line 12)
        self.dropout = nn.Dropout(dropout)

        # ── Readout (paper Algorithm 4 line 13)
        self.fc = nn.Sequential(
            nn.LayerNorm(hidden_dim),
            nn.Linear(hidden_dim, 64),
            nn.GELU(),
        ...
        self.step_scale = nn.Parameter(torch.ones(fm_pred_len) * 0.35)
        MAX_DELTA = 0.60
    ...
    def forward(
        self,
        raw_ctx:    torch.Tensor,           # [B, 512]
        sr_pred:    torch.Tensor,           # [4, B, 2] SR predictions normalised
        feed_forward: bool = True,          # paper Algorithm 4: feed-forward mode
    ) -> torch.Tensor:                      # [T_fm, B, 2] normalised
        B      = raw_ctx.shape[0]
        device = raw_ctx.device

        # ── SR handoff info
        sr_ref_pos = sr_pred[-1]                    # [B, 2] step-4 position
        sr_vel     = sr_pred[-1] - sr_pred[-2]      # [B, 2] step3→4 velocity
        anchor_inp = torch.cat([sr_ref_pos, sr_vel], dim=-1)  # [B, 4]
        sr_anchor  = self.sr_anchor_proj(anchor_inp)          # [B, hidden_dim]

        # ── Init hidden states (paper Algorithm 4 line 5)
        #    Layer 1: ctx + sr_anchor fusion
        h_list = []
        h0 = self.ctx_h_proj(raw_ctx) + sr_anchor   # conditioned init
        h0 = torch.tanh(h0)
        h_list.append(h0)
        for _ in range(self.n_layers - 1):
            h_list.append(torch.zeros(B, self.hidden_dim, device=device))

        ctx_feat = self.ctx_h_proj(raw_ctx)          # [B, hidden_dim]

        # ── Bắt đầu từ SR step-4 position
        cur_pos = sr_ref_pos.clone()                 # [B, 2]
        preds   = []

        for i in range(self.fm_pred_len):

            # ── Feed-forward input (paper Algorithm 4 line 8-10)
            inp = torch.cat([cur_pos, sr_ref_pos, ctx_feat], dim=-1)

            # ── Layer 1 GRUCell (paper Algorithm 4 line 11)
            h_list[0] = self.cells[0](inp, h_list[0].detach())   # Truncated BPTT
            h = self.dropout(h_list[0])                           # paper line 12
            ...
            # ── Readout (paper Algorithm 4 line 13)
            scale   = torch.sigmoid(self.step_scale[i]) * self.MAX_DELTA
            delta   = (self.fc(h) * scale).clamp(-self.MAX_DELTA, self.MAX_DELTA)

            cur_pos = cur_pos + delta
            preds.append(cur_pos)                                 # paper line 14

        return torch.stack(preds, dim=0)             # [T_fm, B, 2]


# ══════════════════════════════════════════════════════════════════════════════
#  VelocityField  ── v31: giữ encoder, thay prediction heads
# ══════════════════════════════════════════════════════════════════════════════

class VelocityField(nn.Module):
    """
    Encoder pipeline GIỮ NGUYÊN từ v30.
    Prediction heads thay bằng LSTMCell/GRUCell (paper).
    """
    RAW_CTX_DIM = 512
    ...
    def __init__(
        ...
        obs_len:     int   = 8,
        ctx_dim:     int   = 256,
        sigma_min:   float = 0.02,    # giữ cho tương thích
        unet_in_ch:  int   = 13,
        fm_pred_len: int   = 8,
        hidden_dim:  int   = 256,
        sr_n_layers: int   = 3,
        lr_n_layers: int   = 3,
        sr_dropout:  float = 0.10,
        lr_dropout:  float = 0.20,    # paper §2.8
    ):
        super().__init__()
        self.pred_len    = pred_len
        self.fm_pred_len = fm_pred_len
        ...
        # ════════════════════════════════════════════════════════
        #  PREDICTION HEADS  ── v31: LSTMCell / GRUCell (paper)
        # ════════════════════════════════════════════════════════
        ...
        self.short_range_head = ShortRangeHead(
            ...
            n_layers    = sr_n_layers,
            dropout     = sr_dropout,
        )

        self.long_range_head = LongRangeHead(
            raw_ctx_dim = self.RAW_CTX_DIM,
            hidden_dim  = hidden_dim,
            n_layers    = lr_n_layers,
            fm_pred_len = fm_pred_len,
            dropout     = lr_dropout,
        )

    # ── Context extraction: GIỮ NGUYÊN hoàn toàn từ v30 ─────────────────────
    ...
    def forward_long_range(
        self,
        raw_ctx:  torch.Tensor,   # [B, 512]
        sr_pred:  torch.Tensor,   # [4, B, 2]
    ) -> torch.Tensor:            # [8, B, 2]
        return self.long_range_head(raw_ctx, sr_pred)

    def compute_sr_anchor_emb(self, sr_pred: torch.Tensor) -> torch.Tensor:
        """Tương thích ngược với v30 interface (dùng trong losses nếu cần)."""
        sr_pos = sr_pred[-1]
        sr_vel = sr_pred[-1] - sr_pred[-2]
        return torch.cat([sr_pos, sr_vel], dim=-1)   # [B, 4]


# ══════════════════════════════════════════════════════════════════════════════
#  Beta-drift physics (giữ từ v30)
# ══════════════════════════════════════════════════════════════════════════════

def _beta_drift_velocity(x_t: torch.Tensor) -> torch.Tensor:
    OMEGA_val  = 7.2921e-5
    R_val      = 6.371e6
    DT         = 6 * 3600.0
    M_PER_NORM = 5.0 * 111.0 * 1000.0
...
# ══════════════════════════════════════════════════════════════════════════════
#  TCFlowMatching  ── v31 HIERARCHICAL với LSTMCell/GRUCell
# ══════════════════════════════════════════════════════════════════════════════

class TCFlowMatching(nn.Module):
    """
    TC trajectory prediction: SR LSTMCell (step 1-4) + LR GRUCell (step 5-12).

    Ensemble generation:
      - MC-Dropout: chạy num_ensemble lần với dropout=ON
      - Không cần ODE/CFM, không cần initial_sample_sigma
      - ctx_noise_scale: vẫn giữ để perturb context (tăng diversity)

    Paper §2.5-2.6:
      LSTM outperforms GRU ở short-range (3-gate memory)
      GRU phù hợp long-range với dropout regularisation
    """
    ...
    def __init__(
        ...
        pred_len:             int   = 12,
        obs_len:              int   = 8,
        sigma_min:            float = 0.02,    # giữ tương thích
        n_train_ens:          int   = 4,
        unet_in_ch:           int   = 13,
        ctx_noise_scale:      float = 0.002,
        initial_sample_sigma: float = 0.03,    # giữ tương thích (không dùng)
        hidden_dim:           int   = 256,
        **kwargs,
    ):
        super().__init__()
        ...
        self.active_pred_len      = pred_len
        self.ctx_noise_scale      = ctx_noise_scale
        self.initial_sample_sigma = initial_sample_sigma    # giữ tương thích
    ...
    # ── Context với optional noise (tăng diversity cho ensemble) ─────────────
    ...
    def _get_raw_ctx(
        ...
        raw = self.net._context(batch_list)
        if noise_scale > 0.0:
            raw = raw + torch.randn_like(raw) * noise_scale
        return raw

    # ── Forward một lần (train hoặc một ensemble member) ─────────────────────

    def _forward_once(
        self,
        raw_ctx: torch.Tensor,    # [B, 512]
        obs_t:   torch.Tensor,    # [T_obs, B, 2]
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Returns:
            sr_pred    : [4,    B, 2] normalised
            lr_pred    : [T_fm, B, 2] normalised
        """
        sr_pred = self.net.forward_short_range(obs_t, raw_ctx)
        lr_pred = self.net.forward_long_range(raw_ctx, sr_pred)
        return sr_pred, lr_pred

    # ── Loss ─────────────────────────────────────────────────────────────────

    def get_loss(self, batch_list: List, **kwargs) -> torch.Tensor:
        return self.get_loss_breakdown(batch_list, **kwargs)["total"]
    ...
    def get_loss_breakdown(
        ...
        """
        HIERARCHICAL loss (v31):
          1. SR LSTMCell → step 1-4
          2. LR GRUCell conditioned on SR → step 5-12
          3. Ensemble: n_train_ens lần forward với dropout=ON
          4. Loss: SR Huber + CRPS ensemble + continuity + physics
        """
        batch_list = self._lon_flip_aug(batch_list, p=0.3)
        ...
        raw_ctx     = self._get_raw_ctx(batch_list, noise_scale=0.0)
        intensity_w = self._intensity_weights(obs_Me)

        # ── SR prediction (deterministic, dropout=OFF bằng training mode)
        sr_pred = self.net.forward_short_range(obs_t, raw_ctx)  # [4, B, 2]

        n_sr = N_SR_STEPS   # 4
        l_sr = short_range_regression_loss(sr_pred, traj_gt[:n_sr], lp)

        # ── LR ensemble (MC-Dropout: training mode = dropout ON)
        #    paper: chạy n lần với noise perturbation context
        fm_gt   = traj_gt[n_sr:]   # [T_fm, B, 2]
        T_fm    = fm_gt.shape[0]
        ...
        if T_fm == 0:
            return dict(
                total=l_sr, fm=0.0, velocity=0.0, step=0.0, disp=0.0,
                heading=0.0, recurv=0.0, smooth=0.0, accel=0.0, jerk=0.0,
                pinn=0.0, spread=0.0, continuity=0.0,
                short_range=l_sr.item(), fm_physics=0.0, recurv_ratio=0.0,
            )

        lr_samples: List[torch.Tensor] = []
        for _ in range(self.n_train_ens):
            # Perturb context nhẹ (tăng diversity)
            raw_noisy = self._get_raw_ctx(batch_list, noise_scale=self.ctx_noise_scale)
            # training=True → dropout active → khác nhau mỗi run
            lr_one = self.net.forward_long_range(raw_noisy, sr_pred.detach())
            lr_samples.append(lr_one)

        # lr_pred_samples: [S, T_fm, B, 2] normalised
        lr_pred_samples = torch.stack(lr_samples)

        # Mean prediction
        lr_mean = lr_pred_samples.mean(0)                   # [T_fm, B, 2]

        # ── Full trajectory
        full_pred_norm = torch.cat([sr_pred, lr_mean], dim=0)  # [12, B, 2]

        full_pred_deg  = _denorm_to_deg(full_pred_norm)
        lr_mean_deg    = _denorm_to_deg(lr_mean)
        traj_gt_deg    = _denorm_to_deg(traj_gt)
        ref_deg        = _denorm_to_deg(lp)

        # ── Physics consistency (từ v30)
        l_fm_physics = fm_physics_consistency_loss(
            lr_pred_samples, gt_norm=fm_gt, last_pos=sr_pred[3].detach())
        ...
        breakdown = compute_total_loss(
            ...
            ref                = ref_deg,
            batch_list         = batch_list,
            pred_samples       = lr_pred_samples,
            gt_norm            = fm_gt,
            weights            = WEIGHTS,
            intensity_w        = intensity_w,
            env_data           = env_data,
            step_weight_alpha  = step_weight_alpha,
            all_trajs          = lr_pred_samples,
            epoch              = epoch,
            gt_abs_deg         = traj_gt_deg,
            sr_pred            = sr_pred,
            fm_pred_abs        = lr_mean_deg,
        )
    ...
    def sample(
        ...
        batch_list:   List,
        num_ensemble: int = 50,
        ddim_steps:   int = 20,        # giữ tương thích, không dùng cho LSTMCell
        predict_csv:  Optional[str] = None,
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        HIERARCHICAL sampling với MC-Dropout ensemble.

        Returns:
            pred_mean : [T=12, B, 2] normalised
            me_mean   : [T=12, B, 2]
            all_trajs : [S, T=12, B, 2]
        """
        obs_t  = batch_list[0]
        lp     = obs_t[-1]
        B      = lp.shape[0]
        ...
        # ── Step 1: SR (deterministic, eval mode)
        self.net.short_range_head.eval()
        sr_pred = self.net.forward_short_range(obs_t, raw_ctx)  # [4, B, 2]

        # ── Step 2: LR ensemble via MC-Dropout
        #    Bật dropout trong long_range_head (train mode = dropout ON)
        self.net.long_range_head.train()

        lr_traj_s: List[torch.Tensor] = []

        for k in range(num_ensemble):
            # Perturb context để tăng diversity
            ns = self.ctx_noise_scale if k < num_ensemble // 2 else 0.0
            raw_noisy = self._get_raw_ctx(batch_list, noise_scale=ns)
            lr_one    = self.net.forward_long_range(raw_noisy, sr_pred)
            lr_traj_s.append(lr_one)

        # Restore eval mode
        self.net.long_range_head.eval()

        lr_all  = torch.stack(lr_traj_s)              # [S, T_fm, B, 2]
        lr_mean = lr_all.mean(0)                      # [T_fm, B, 2]

        # ── Step 3: Concatenate SR + LR
        pred_mean = torch.cat([sr_pred, lr_mean], dim=0)     # [12, B, 2]

        sr_me  = torch.zeros(N_SR_STEPS, B, 2, device=device)
        me_mean = torch.cat([sr_me, torch.zeros(T_fm, B, 2, device=device)], dim=0)

        sr_expanded = sr_pred.unsqueeze(0).expand(num_ensemble, -1, -1, -1)
        all_trajs   = torch.cat([sr_expanded, lr_all], dim=1)  # [S, 12, B, 2]
    ...
    def _pinn_speed_constraint(pred_deg: torch.Tensor) -> torch.Tensor:
        ...
        dx_km   = dt_deg[:, :, 0] * cos_lat * 111.0
        dy_km   = dt_deg[:, :, 1] * 111.0
        return F.relu(torch.sqrt(dx_km ** 2 + dy_km ** 2) - 600.0).pow(2).mean()
    ...
    def _pinn_beta_plane_simplified(pred_deg: torch.Tensor) -> torch.Tensor:
        ...
        a_lon_km = a[:, :, 0] * cos_lat * 111.0
        a_lat_km = a[:, :, 1] * 111.0
        return F.relu(torch.sqrt(a_lon_km**2 + a_lat_km**2) - 50.0).pow(2).mean() * 0.1

    # ── CSV writer (giữ từ v30) ───────────────────────────────────────────────
