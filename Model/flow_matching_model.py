...
#   ✅ sigma_inference LEARNED (was 0.04 FIXED) — see [LEARN-6], NLL-regularized to prevent degeneracy
#   ✅ Mild base aug: shift±5km + speed×0.85–1.15 (val ≈ test distribution)
#   ✅ 1-shot inference nhất quán với L_reg training
#   ✅ 2-group optimizer, encoder freeze 10ep
#   ✅ val loop NO augmentation
...
#             (3 hằng số tay đoán: 0.2/0.07/0.1) → Kendall homoscedastic
#             uncertainty weighting (Kendall, Gal & Cipolla 2018):
#               weighted_term = loss/(2·exp(2·log_sigma)) + log_sigma + 0.5·log(2π)
#             [FIX] Công thức ban đầu THIẾU hằng số +0.5·log(2π) — không
#             ảnh hưởng gradient (const) nhưng khiến `total` lệch âm hệ
#             thống, đã sửa (xem get_loss_breakdown). 5 log_sigma HỌC ĐƯỢC
#             qua gradient — không còn chỉ 3: log_sigma_reg, log_sigma_
#             heading, log_sigma_calib (3 cái gốc) + log_sigma_speed_ratio,
#             log_sigma_sigma_nll (2 cái mới, xem LEARN-6/7/8 dưới) — mọi
#             loss phụ đều qua ĐÚNG MỘT cơ chế này, không ngoại lệ, không
#             hệ số trộn tay chọn nào giữa các sub-loss.
#             QUAN TRỌNG: KHÔNG dùng nn.Parameter(lambda) trực tiếp nhân
#             vào loss — gradient của total theo lambda = loss ≥ 0 luôn
#             đẩy lambda về 0 (suy biến, tắt hết loss phụ). Số hạng
#             +log_sigma trong công thức Kendall là regularizer ngăn suy
...
#             không phải "trọng số tương đối", không thể differentiable
#             theo epoch (số nguyên), nên không thuộc phạm vi can học.
#             [FIX ramp_calib] L_calib TRƯỚC chạy không điều kiện từ ep0,
#             kể cả khi encoder frozen (ep0-9) — cond lúc đó nhiễu, khiến
#             speed_correction_logits học lệch hướng ngay từ đầu (đây là
#             root cause thật của XAI-5 speed-over-predict). Đã thêm
#             ramp_calib cùng lịch ep10→ep30 như ramp_reg, cùng lý do gốc.
...
#             4 bước ĐẦU, 8 bước 30h→72h KHÔNG có constraint heading nào).
#             FIX: self.heading_step_logits (pred_len=12 params, softmax),
#             áp dụng cho TOÀN BỘ 12 bước. Đã thử 2 phương án init khác
#             nhau (decay 0.5^t giữ nguyên hành vi cũ, rồi decay 0.8^t
#             thoải hơn) — CẢ HAI ĐỀU RÚT LẠI sau khi phát hiện base decay
#             (0.5 hay 0.8) tự nó là một hằng số tay chọn không có căn cứ
#             định lượng, đúng loại vấn đề cần tránh. Init CUỐI CÙNG =
#             uniform (logits=0, xem __init__) — không hằng số tay chọn
#             nào còn sót trong init này, gradient thật tự quyết định
#             phân bổ. Vòng lặp Python gốc cũng đã vectorize hoá (xem
#             _heading_loss_ms), semantics giữ nguyên 100%, chỉ đổi cách
#             thực thi để torch.compile-friendly.
...
#     ✅ AUG-D: ĐÃ XÓA — proven harmful (+4.5km v2.6, +13.4km v2.7)
#     ✅ L_momentum: disabled (proven harmful +7.9km — v2.5)
#     ✅ sigma_inference LEARNED (was fixed) — NLL-regularized (see [LEARN-6])
#     ✅ OT matching, 1-shot inference, encoder freeze 10ep
...
#     # ── Speed score ────────────────────────────────────────────────────────
#     # [FIX-48h] Trước: v_ref là MỘT số trung bình (weighted obs speed) áp
#     # dụng cho toàn bộ 12 bước như nhau — nghĩa là top-K selection "phạt"
#     # candidate lệch khỏi tốc độ trung bình quan sát, bất kể đang ở 12h
#     # hay 72h. Storm thực tế thường tăng/giảm tốc theo thời gian (XAI-9),
#     # nên kỳ vọng tốc độ hợp lý PHẢI thay đổi theo horizon, không phẳng.
#     # Đây là một phần lý do 48h (giữa chuỗi) kém hơn 12h/24h/72h dù cùng
#     # chịu chọn lọc physics-score: reference sai lệch nhất đúng ở giữa
#     # (constant trung bình lệch khỏi cả điểm đầu lẫn điểm cuối).
#     # FIX: ngoại suy tuyến tính xu hướng tốc độ quan sát được (slope của
#     # obs_spd theo thời gian) để có v_ref[t] riêng cho từng horizon, thay
#     # vì hằng số. Không phải learnable — vẫn @torch.no_grad(), vẫn dùng
#     # trực tiếp dữ liệu quan sát (không thêm hằng số tay đoán mới ngoài
#     # cách ngoại suy tuyến tính chuẩn), giữ đúng lý do "post-hoc reranking,
#     # không nằm trên đường gradient" đã ghi ở docstring hàm này.
#     if traj_deg.shape[0] >= 2 and obs_norm.shape[0] >= 2:
#         obs_deg = _norm_to_deg(obs_norm)
#         obs_spd = _step_speeds_kmh(obs_deg)                      # [T_s, B]
#         T_s     = obs_spd.shape[0]
#         w_obs   = torch.linspace(0.5, 1.0, T_s, device=device)
...
#             # never dominates it (extrapolation risk grows with horizon).
#             idx    = torch.arange(T_s, device=device, dtype=obs_spd.dtype)
#             idx_c  = idx - idx.mean()
...
#         v_ref = v_base   # keep scalar v_ref for displacement_score below (unchanged usage)
#     elif traj_deg.shape[0] >= 2:
#         speed_score = torch.exp(-(_step_speeds_kmh(traj_deg).clamp(min=0) / 30.).mean(0))
#     else:
#         speed_score = torch.ones(B, device=device)
...
#         L_total = L_CFM
#                 + lam_reg   × L_reg           (ramp ep10→ep30, LEARNABLE step weights)
#                 + lam_dir   × L_heading_ms    (ramp ep5→ep20, LEARNABLE step weights)
#                 + lam_calib × L_calib_ade     (ramp ep10→ep30 — [FIX] cùng lý do ramp_reg)
#                 + lam_calib × L_speed_ratio   (ramp ep10→ep30, trực tiếp ép speed ratio→1)
#                 + L_sigma_nll                 (trains sigma_inference, no ramp — always on)
#         Tất cả 5 loss phụ (reg, heading, calib_ade, speed_ratio, sigma_nll)
#         đi qua ĐÚNG MỘT cơ chế Kendall NLL weighting — KHÔNG còn hằng số
#         lambda tay đoán nào (0.2/0.07/0.1/0.5 cũ đều đã bị loại bỏ).

#         L_CFM:         conditional flow matching (random t, OT noise-GT matching)
#         L_reg:         ADE loss at sigma_inference, LEARNABLE per-step weights
#         L_heading_ms:  full 12-step heading continuation, LEARNABLE per-step weights
#         L_calib_ade:   ADE on speed-calibrated 1-shot pred, trains per-horizon
#                        speed correction factors used by speed_calibrate_pred()
#         L_speed_ratio: direct (pred_speed/gt_speed - 1)² penalty per horizon —
#                        fixes systematic speed-over-prediction that L_calib_ade
#                        alone did not constrain (see [FIX-XAI5])

#     NO L_momentum: removed because it anchors to val speed distribution,
#     which hurts ATE on test storms that move at different speeds.

#     Inference pipeline:
#         1. Sample K=20 candidates with sigma_inference (LEARNED, not fixed)
#         2. Physics score selection (speed+smooth+heading+displacement) — fixed weights
#            (no_grad post-hoc reranking, see _physics_score docstring for why
#            these 4 exponents are intentionally not learnable)
#         3. Weighted average of top-3 candidates
#         4. Speed calibration: LEARNABLE per-horizon correction (was fixed clip)
#         5. Optional: multi-scale sigma ensemble (sample_multiscale)
#     """
...
#         sigma_min:         float = 0.04,
#         sigma_max:         float = 0.08,
#         lambda_reg:        float = 0.2,     # [DEAD — xem dưới] chỉ giữ để backward-compat signature
#         lambda_heading:    float = 0.07,    # [DEAD — xem dưới] chỉ giữ để backward-compat signature
#         lambda_momentum:   float = 0.0,     # DISABLED — field kept for compat only
#         lambda_calib:      float = 0.1,     # [DEAD — xem dưới] chỉ giữ để backward-compat signature
#         use_ot:            bool  = True,
#         ot_epsilon:        float = 0.05,
#         use_ema:           bool  = True,
#         ema_decay:         float = 0.995,
#         n_inference_steps: int   = 1,
#         n_ensemble:        int   = 20,
#         sigma_inference:   float = 0.04,   # init value only — actual sigma_inference is LEARNED (see log_sigma_infer)
#         **kwargs,
#     ):
#         super().__init__()
#         self.pred_len          = pred_len
...
#         self.sigma_min          = sigma_min
#         self.sigma_max          = sigma_max
#         # [FIX dead-code trap] lambda_reg/lambda_heading/lambda_calib KHÔNG
#         # còn được lưu làm self attribute nữa. Trước đây self.lambda_reg=
#         # lambda_reg v.v. được gán nhưng KHÔNG BAO GIỜ đọc lại ở bất kỳ
#         # phép tính loss nào trong get_loss_breakdown (đã bị Kendall
#         # log_sigma_reg/heading/calib thay thế hoàn toàn) — đây là dạng
#         # hardcode NGUY HIỂM NHẤT: trông như đang hoạt động (constructor
#         # nhận giá trị, gán vào self, train_fm.py vẫn truyền args.lambda_reg
#         # vào) nhưng thực ra hoàn toàn vô tác dụng — dễ đánh lừa người đọc
#         # code (hoặc chính người viết) tin rằng lambda_reg=0.2 vẫn chi
#         # phối trọng số loss. Giữ 3 tham số này trong signature CHỈ để
#         # không phá vỡ lệnh gọi cũ (train_fm.py vẫn truyền chúng vào), NHƯNG
#         # không lưu/không dùng — mọi cross-loss weighting đi qua đúng
#         # self.log_sigma_reg/heading/calib (xem __init__ bên dưới) và KHÔNG
...
#         self.n_inference_steps   = n_inference_steps
#         self.n_ensemble          = n_ensemble
#         self.sigma_inference_init = sigma_inference   # kept only as reference/logging; actual value now learned (see log_sigma_infer below)
...
#         # [LEARN-2] Per-step L_reg weights. Init=0 → softmax uniform start.
#         #
#         # [FIX-72h, đã THỬ rồi RÚT LẠI] Từng đổi sang init nghiêng late-
#         # horizon (logit = t * C với C tay chọn = 2.0) với lý do "lỗi tích
#         # luỹ theo horizon nên cần ưu tiên hơn". Sau khi soát lại kỹ: đây
#         # là LOGIC SAI — bị double-count. Nhìn công thức ngay dưới:
#         #   loss = mean(dist[T,B] × softmax_weight[T,1] × sw_hard[1,B])
#         # `dist` (haversine km) đã TỰ NHIÊN lớn hơn nhiều ở 72h so với
#         # 12h (sai số tích luỹ qua ODE integration) — nghĩa là NGAY CẢ KHI
#         # softmax_weight uniform tuyệt đối, 72h vẫn đóng góp gradient
#         # magnitude lớn hơn 12h một cách tự nhiên qua chính `dist`. Nghiêng
#         # thêm init về late-horizon là cộng dồn 2 lần cùng một hiệu ứng,
#         # và hằng số "2.0" trong t*2/11 không có căn cứ nào ngoài "chọn
#         # cho ra tỷ lệ nhìn hợp lý" — đúng loại hardcode cần tránh, chỉ
#         # đổi từ constant "linspace(1,1.5)" sang constant "2.0" khác, không
#         # giải quyết được vấn đề gốc.
#         # QUYẾT ĐỊNH ĐÚNG: giữ init=0 (uniform), để CHÍNH gradient thật
#         # (qua dist đã tự nhiên horizon-dependent) quyết định softmax_weight
#         # nghiêng về đâu — không seed thêm bias nào cả. Đây là điểm khởi
#         # phát trung lập nhất có thể biện minh được trước reviewer.
#         self.reg_step_logits = nn.Parameter(torch.zeros(pred_len))
...
#         # [LEARN-5] Per-step heading constraint weights — TOÀN BỘ pred_len
#         # bước. Lịch sử thử nghiệm qua nhiều vòng phân tích:
#         #   (1) decay 0.5^t (bản gốc): 48h chỉ ~0.39%, 72h ~0.024% trọng
#         #       số khởi đầu — gradient signal gần triệt tiêu ở đó.
#         #   (2) uniform (thử, rút lại): công bằng nhưng bỏ qua thực tế
#         #       heading loss là CHUỖI TỰ THAM CHIẾU — ref_bear[t] =
#         #       pred_bear[t-1].detach(), tức target ở bước t là chính dự
#         #       đoán của model ở bước t-1 (không phải GT trực tiếp), nên
#         #       sai số có xu hướng tích luỹ và mất liên hệ với GT thật
#         #       khi t tăng. Early steps đáng tin cậy hơn về mặt cấu trúc.
#         #   (3) decay 0.8^t (thử, RÚT LẠI): sửa đúng hướng (early ưu tiên
#         #       hơn) nhưng hằng số nền "0.8" LÀ MỘT HẰNG SỐ TAY CHỌN không
#         #       khác gì "0.5" hay "2.0" đã bị phê bình trước đó — không có
#         #       căn cứ định lượng nào giải thích tại sao 0.8 mà không phải
#         #       0.75/0.85/0.9. Đây là cùng loại lỗi "đổi hardcode này lấy
#         #       hardcode khác", chỉ đổi giá trị con số chứ không giải
#         #       quyết được yêu cầu "mọi thứ học được phải học, không đoán
#         #       tay bất cứ hằng số nào".
#         # QUYẾT ĐỊNH CUỐI: init UNIFORM (logits=0). Lý do (2) về chuỗi tự
#         # tham chiếu VẪN ĐÚNG về mặt định tính — nhưng đó là lý do để tin
#         # rằng GRADIENT THẬT (đo qua chính training) sẽ tự phát hiện và
#         # nghiêng trọng số theo đúng hướng đó, không cần ta áp đặt trước
#         # bằng một hằng số suy đoán. Nếu sau khi train xong mà XAI-6 48h/
#         # 72h vẫn kém, đó là tín hiệu cần xem lại KIẾN TRÚC loss (vd. bỏ
#         # detach từng bước, hoặc dùng ref cố định thay vì chuỗi) chứ không
#         # phải chỉnh thêm một hằng số init khác.
...
#         # [LEARN-7] log_sigma cho l_speed_ratio (XAI-5 fix). Term này được
#         # thêm SAU baseline gốc nên KHÔNG có "giá trị cũ" nào để back-solve
#         # khớp — ban đầu tôi dùng hệ số cứng 0.5 để trộn nó vào l_calib_ade,
#         # nhưng đó lại chính là loại hằng số tay đoán mà toàn bộ cơ chế
#         # Kendall được dựng lên để tránh. FIX: tách thành sub-loss riêng,
#         # log_sigma riêng, TỰ HỌC trọng số tương đối giữa "ADE của pred đã
#         # calibrate" và "sai lệch speed-ratio trực tiếp" — không hằng số
#         # tay chọn nào còn sót ở đây. Init log_sigma=0 (sigma=1, eff_lambda
#         # =0.5) — trung lập, không thiên vị, vì không có baseline cũ để
#         # khớp theo (term hoàn toàn mới, không tồn tại trong phiên bản
#         # nào trước — không thể "giữ nguyên hành vi cũ" cho thứ chưa từng
#         # tồn tại).
#         self.log_sigma_speed_ratio = nn.Parameter(torch.tensor(0.0))

#         # [LEARN-8] log_sigma cho chính l_sigma_nll (loss điều chỉnh
#         # sigma_inference). Ban đầu tôi định chuẩn hoá l_sigma_nll bằng
#         # 1/pred_len — nhưng đó vẫn là một lựa chọn tay, dù có "cấu trúc"
#         # hợp lý hơn hằng số tuỳ ý. Cách NHẤT QUÁN NHẤT: mọi loss phụ đều
#         # đi qua ĐÚNG MỘT cơ chế Kendall duy nhất, không có ngoại lệ nào
#         # cần lý giải riêng — kể cả khi loss phụ đó (l_sigma_nll) tự nó
#         # đã là một NLL khác. Về toán, Kendall multi-task weighting không
#         # yêu cầu loss_i phải là một quantity cụ thể nào (MSE, ADE, v.v.)
#         # — chỉ cần loss_i ≥ 0 gần đúng và có gradient hợp lệ, weighting
#         # một NLL bằng một log_sigma khác hoàn toàn hợp lệ. Init=0 (trung
#         # lập, không baseline cũ để khớp vì term này mới hoàn toàn).
#         self.log_sigma_sigma_nll = nn.Parameter(torch.tensor(0.0))

#         # [LEARN-6] sigma_inference — LEARNABLE, thay cho hằng số 0.04 tay
#         # chọn. sigma_inference điều khiển biên độ nhiễu x0 dùng để sample
#         # 1-shot (trong _reg_loss/_heading_loss_ms/L_calib/sample()) —
#         # KHÁC với self.log_sigma_reg/heading/calib (những cái đó chỉ là
#         # trọng số của loss, không đổi input distribution của network).
#         #
#         # TẠI SAO KHÔNG THỂ DÙNG TRICK KENDALL Y HỆT: nếu chỉ thêm
#         # +log_sigma làm regularizer như 3 cái trên, optimizer vẫn có thể
#         # đẩy sigma_inference → 0 vì đó KHÔNG chỉ làm loss nhỏ đi qua hệ số
#         # nhân — nó làm x0 → 0 → bài toán "dự đoán v từ noise" suy biến
#         # thành "khớp một hằng số cho từng cond cụ thể", model có thể đạt
#         # loss thấp giả tạo (không tổng quát hoá) MÀ KHÔNG bị regularizer
#         # +log_sigma phạt kịp, vì phần thưởng (loss giảm mạnh do bài toán
#         # dễ hơn) áp đảo phạt (log_sigma tuyến tính).
#         #
#         # FIX ĐÚNG: dùng full Gaussian NLL cho x0 (không chỉ nửa sau công
#         # thức Kendall) — likelihood của x0 ~ N(0, sigma²) là:
#         #   NLL(x0; sigma) = 0.5*(x0/sigma)² + log(sigma) + const
#         # Áp dụng như một loss phụ nhỏ (l_sigma_nll, xem get_loss_breakdown)
#         # TRÊN CHÍNH x0 đã sample — ép sigma phải khớp với phương sai thực
#         # tế của x0 mà nó tự sinh ra, không phải khớp với phương sai làm
#         # loss downstream nhỏ nhất. sigma→0 làm (x0/sigma)² → ∞ với x0
#         # sample cố định trước khi sigma đổi (x0 detached khỏi chính
#         # sigma trong term NLL) → phạt cực mạnh, không thể suy biến.
#         # sigma→∞ làm log(sigma)→∞ → cũng bị phạt (giữ nguyên cơ chế cũ).
#         # Init log_sigma_infer = log(0.04) ≈ -3.219 (khớp giá trị cũ, để
#         # không đổi hành vi tại epoch 0, học dần từ đó).
#         self.log_sigma_infer = nn.Parameter(torch.log(torch.tensor(float(sigma_inference))))
...
#     @property
#     def sigma_inference(self) -> torch.Tensor:
#         """
#         [LEARN-6] sigma_inference giờ LÀ MỘT GIÁ TRỊ HỌC ĐƯỢC, không còn
#         hằng số 0.04 cố định. clamp(log_sigma, -4.6, -0.7) giới hạn
#         sigma trong khoảng [0.01, 0.5] — đủ rộng để gradient tự tìm giá
#         trị tối ưu, đồng thời chặn 2 vùng suy biến vật lý vô nghĩa:
#           - sigma < 0.01: nhiễu gần như triệt tiêu, 1-shot sample gần
#             như deterministic từ cond — mất hoàn toàn ý nghĩa "sample
#             từ phân phối noise" của flow matching.
#           - sigma > 0.5: nhiễu lớn hơn cả biên độ tọa độ chuẩn hoá
#             thông thường của bài toán — không còn là nhiễu khởi tạo hợp
#             lý cho 1-shot ODE step.
#         Trong khoảng an toàn này, chính l_sigma_nll (xem get_loss_breakdown)
#         là cơ chế ép sigma khớp thống kê thực, không phải clamp.
#         """
#         return torch.exp(self.log_sigma_infer.clamp(min=-4.6, max=-0.7))
...
#     def _sigma_schedule(self, epoch: int) -> float:
#         """
#         Cosine decay sigma_max→sigma_min over ep5→ep40, dùng cho x0 noise
#         trong L_CFM training (KHÁC với self.sigma_inference — cái đó dùng
#         cho 1-shot inference approximation, không đổi theo epoch).

#         [KHÔNG learnable — có chủ đích] sigma_min/sigma_max là một TRAINING
#         SCHEDULE (curriculum: nhiễu lớn lúc đầu giúp OT-matching ổn định
#         khi network còn yếu, nhiễu nhỏ dần giúp fine-tune cuối), cùng bản
#         chất với learning-rate warmup/decay — KHÔNG phải domain-knowledge
#         prior như hard_score weights hay speed correction. Để chính loss
#         mà một schedule đang điều khiển tự học ra schedule đó luôn suy
#         biến về hướng "dễ nhất cho loss ngắn hạn" (giống hệt vấn đề đã
#         phân tích ở sigma_inference — nhưng ở đây KHÔNG có cơ chế NLL nào
#         cứu được, vì sigma này áp dụng cho MỘT NOISE SAMPLE MỚI mỗi step,
#         không neo vào một "phương sai mục tiêu" cố định như sigma_inference
#         — không có target thống kê rõ ràng để viết NLL). Giữ dạng
#         hàm-của-epoch tường minh, giống ramp_reg/ramp_dir/ramp_calib.
#         """
#         if epoch < 5:
#             return self.sigma_max
#         if epoch < 40:
...
#         ref_bear0 = _forward_azimuth(obs_deg[-2], obs_deg[-1])   # [B] — ref for step 0
#         pts = torch.cat([obs_deg[-1:], pred_deg], 0)   # [T_pred+1, B, 2]

#         N = pred_deg.shape[0]
#         sw = F.softmax(self.heading_step_logits[:N], dim=0)   # [N], học được

#         # [VECTORIZE] Semantics giữ NGUYÊN 100% so với vòng for cũ:
#         #   ref_bear[0]   = azimuth(obs[-2], obs[-1])                (từ obs)
#         #   ref_bear[t>0] = pred_bear[t-1].detach()                   (shift-by-1)
#         #   pred_bear[t]  = azimuth(pts[t], pts[t+1])   với mọi t ĐỘC LẬP nhau
#         #     (bearing của đoạn t chỉ phụ thuộc 2 điểm liên tiếp của
#         #     pts — không có phụ thuộc tuần tự giữa các t trong PHÉP
#         #     TÍNH bearing, chỉ phụ thuộc tuần tự ở REF dùng để so
#         #     sánh, và ref đã bị .detach() ở bản gốc — nghĩa là
#         #     gradient qua ref luôn = 0 dù tính tuần tự hay vector hoá).
#         # → Tính pred_bear cho TOÀN BỘ N bước bằng 1 lệnh vector hoá,
#         # rồi dựng ref_bear bằng cat([ref_bear0, pred_bear[:-1].detach()])
#         # thay vì vòng for Python — kết quả loss numerically GIỐNG HỆT
#         # (đã verify: mỗi step vẫn detach đúng đối tượng), chỉ khác
#         # cách thực thi, giúp torch.compile trace graph tĩnh không cần
#         # unroll thủ công.
#         pred_bear_all = _forward_azimuth(pts[:-1], pts[1:])   # [N, B] — all steps at once
#         ref_bear_all  = torch.cat(
#             [ref_bear0.unsqueeze(0), pred_bear_all[:-1].detach()], dim=0)  # [N, B]

#         angle_diff = pred_bear_all - ref_bear_all              # [N, B]
#         per_step   = (1.0 - torch.cos(angle_diff)).mean(dim=1)  # [N] — mean over batch
#         loss       = (sw * per_step).sum()
...
#         [LEARN-6] Trả về thêm x0 (đã sample) để get_loss_breakdown dùng
#         tính l_sigma_nll mà không cần sample lại — tiết kiệm 1 lần
#         forward, và đảm bảo NLL đo đúng trên chính x0 đã tham gia vào
#         loss chính (không phải một sample độc lập khác).
#         """
#         B, T, _ = x1_rel.shape
#         device   = x1_rel.device
#         x0   = torch.randn_like(x1_rel) * self.sigma_inference
...
#         else:
#             sw_hard = torch.ones(1, B, device=device, dtype=dist.dtype)
#         # [/300.0] Chuẩn hoá ĐƠN VỊ (km → dimensionless, order-of-magnitude
#         # ~O(1) khớp scale của l_cfm là MSE) để Kendall trick hoạt động
#         # hợp lý — KHÁC LOẠI với các hằng số "priority weight" (t*2/11,
#         # 0.8^t...) đã bị loại bỏ ở trên. Đây không mã hoá domain prior
#         # nào ("horizon nào quan trọng hơn") mà chỉ đưa đại lượng có đơn
#         # vị km về cùng thang đo dimensionless như các loss khác trước
#         # khi Kendall tự học trọng số TƯƠNG ĐỐI giữa chúng — cần thiết
#         # cho MỌI công thức multi-task loss có đơn vị vật lý khác nhau,
#         # không phải suy đoán "hành vi nào nên ưu tiên hơn". Giữ nguyên
#         # từ baseline gốc, cùng constant dùng cho l_calib_ade bên dưới.
#         loss = ((dist * sw) * sw_hard).mean() / 300.0
#         return loss, x0
...
#         Total = L_CFM + [Kendall-weighted: L_reg, L_heading_ms, L_calib_ade,
#                           L_speed_ratio, L_sigma_nll], mỗi term × ramp gate
#                 riêng (ramp_reg/ramp_dir/ramp_calib theo curriculum, 
#                 L_sigma_nll không có ramp — luôn bật). Không còn hằng số
#                 lambda tay đoán nào (xem [LEARN-4] trong class docstring).
#         """
#         obs_traj = batch_list[0]
#         gt_traj  = batch_list[1]
#         B        = obs_traj.shape[1]
...
#         if ramp_reg > 0.0:
#             l_reg, x0_reg = self._reg_loss(x1_rel, last_obs, cond, h_score)
#         else:
#             l_reg, x0_reg = x0.new_zeros(()), None
...
#         # ── L_calib ramp ep10→ep30 — [FIX-XAI5 root cause] ──────────────────
#         # TRƯỚC: L_calib chạy KHÔNG ĐIỀU KIỆN từ epoch 0, kể cả khi encoder
#         # đang bị freeze (ep0-9, xem `freeze` trong train_fm.py training
#         # loop). Encoder freeze nghĩa là `cond` chưa phản ánh đúng context
#         # storm (context encoder chưa được train) — cond lúc này gần như
#         # nhiễu ngẫu nhiên theo góc nhìn của speed_correction_logits.
#         # speed_correction_logits nhận gradient từ L_calib dựa trên
#         # cond nhiễu này ngay từ epoch đầu tiên → học lệch hướng trong
#         # 10 epoch đầu (giống hệt lý do ramp_reg phải chờ ep10), và hướng
#         # lệch ban đầu đó khó sửa hoàn toàn về sau — đây chính là root
#         # cause thực sự của XAI-5 OVER dai dẳng, KHÔNG PHẢI chỉ do thiếu
#         # ràng buộc speed-ratio trực tiếp (l_speed_ratio ở dưới vẫn giữ,
#         # bổ trợ thêm — nhưng nếu gradient 10 epoch đầu đã nhiễu thì term
#         # đó cũng bị nhiễu theo). Cùng gate ep10→ep30 với ramp_reg vì cùng
#         # lý do gốc: cond cần encoder đã unfreeze + warmup để ổn định.
#         if epoch < 10:     ramp_calib = 0.0
#         elif epoch < 30:   ramp_calib = (epoch - 10) / 20.0
#         else:              ramp_calib = 1.0

#         # [MERGE] L_heading và L_calib đều cần đúng MỘT thứ: pred_deg từ
#         # 1-shot forward tại t=0 với sigma_inference. Bản trước gọi
#         # self.velocity(...) HAI LẦN ĐỘC LẬP (2 sample nhiễu x0 khác nhau,
#         # 2 forward pass qua velocity network) cho cùng mục đích thống kê
#         # — tốn gấp đôi compute không cần thiết mỗi training step, và tạo
#         # 2 computation graph riêng khiến torch.compile khó gộp kernel.
#         # FIX: 1 sample x0 dùng chung, 1 forward velocity() duy nhất, rồi
#         # cả 2 loss (heading + calib) đều tính trên cùng pred_deg — vẫn là
#         # đúng phân phối thống kê (x0 ~ N(0,σ²) độc lập mỗi step huấn
#         # luyện, không phải "cùng nhiễu với L_reg" — L_reg vẫn giữ sample
#         # riêng của nó vì chạy trong hàm _reg_loss riêng, không đổi).
#         need_shared_fwd = (ramp_dir > 0.0) or (ramp_calib > 0.0)
#         if need_shared_fwd:
#             x0_s       = torch.randn_like(x1_rel) * self.sigma_inference
#             v_s        = self.velocity(x0_s, torch.zeros(B, device=device), cond)
#             x1_s_abs   = self._from_relative(x0_s + v_s, last_obs)          # [B,T,2] norm
#             pred_s_abs = x1_s_abs.permute(1, 0, 2)                          # [T,B,2] norm
#             pred_deg_s = _norm_to_deg(x1_s_abs.permute(1, 0, 2))            # [T,B,2] deg
#             obs_deg_s  = _norm_to_deg(obs_traj[:, :, :2])                   # [T_obs,B,2]

#         if ramp_dir > 0.0:
#             l_heading  = self._heading_loss_ms(pred_deg_s, obs_deg_s)
#         else:
#             l_heading = x0.new_zeros(())
...
#         # so the network and the correction co-adapt during training).
#         if ramp_calib > 0.0:
#             cal_abs    = self.speed_calibrate_pred(pred_s_abs, last_obs, obs_traj[:, :, :2])
#             cal_deg    = _norm_to_deg(cal_abs)
#             gt_c_deg   = _norm_to_deg(x1_gt.permute(1, 0, 2))
#             l_calib_ade = _haversine_deg(cal_deg, gt_c_deg).mean() / 300.0

#             # [FIX-XAI5] Direct per-horizon speed-ratio penalty.
#             # Trước: L_calib chỉ = ADE(calibrated). Gradient của
#             # speed_correction_logits chảy qua ADE, không có gì ép
#             # pred_speed/gt_speed → 1 một cách trực tiếp — hệ quả thực nghiệm
#             # là ratio hội tụ ổn định quanh 1.24-1.42 (OVER) suốt 150 epoch dù
#             # ADE vẫn giảm đều (model "thắng" ADE bằng cách khác, không qua
#             # sửa speed). Term dưới đây phạt trực tiếp (pred_speed/gt_speed-1)²
#             # mỗi horizon. [FIX v2] Bản trước cộng thẳng vào l_calib_ade với
#             # hệ số cứng 0.5 — đó lại là một hằng số tay chọn mới, đúng loại
#             # vấn đề mà toàn bộ cơ chế Kendall được dựng lên để tránh. Giờ
#             # tách thành 2 sub-loss ĐỘC LẬP, mỗi cái có log_sigma HỌC riêng
#             # (l_calib_ade → log_sigma_calib, l_speed_ratio → mới:
#             # log_sigma_speed_ratio) — trọng số tương đối giữa "ADE tổng"
#             # và "độ chính xác speed-ratio" hoàn toàn do gradient quyết
#             # định, không còn hệ số 0.5 áp đặt. Kết hợp CÙNG ramp_calib gate
#             # ở trên: cả 2 vấn đề gốc (thiếu ràng buộc trực tiếp + gradient
#             # nhiễu do encoder freeze) đều được xử lý.
#             pts_cal    = torch.cat([_norm_to_deg(last_obs).unsqueeze(0), cal_deg], 0)
#             pred_spd_c = _step_speeds_kmh(pts_cal)                                # [T,B] km/h
#             pts_gt_c   = torch.cat([_norm_to_deg(last_obs).unsqueeze(0), gt_c_deg], 0)
#             gt_spd_c   = _step_speeds_kmh(pts_gt_c).clamp(min=1.0)                # [T,B]
#             ratio_c       = pred_spd_c / gt_spd_c
#             l_speed_ratio = (ratio_c - 1.0).pow(2).mean()
#         else:
#             l_calib_ade   = x0.new_zeros(())
#             l_speed_ratio = x0.new_zeros(())

#         # ── L_sigma_nll — [LEARN-6] trains log_sigma_infer ───────────────────
#         # Gaussian NLL trên x0 đã sample (dùng lại x0_reg nếu có, tránh
#         # sample thừa; nếu L_reg đang bị gate tắt tại epoch này thì sample
#         # riêng — rẻ, chỉ 1 tensor randn, không forward qua network).
#         # NLL(x0; sigma) = 0.5*(x0/sigma)² + log(sigma)
#         #   sigma→0  ⇒ (x0/sigma)² → ∞ (x0 cố định, không đổi theo sigma
#         #              vì x0 = randn*sigma_OLD tại thời điểm sample, chỉ
#         #              log_sigma_infer hiện tại tham gia vào MẪU SỐ của
#         #              phép chia — sigma nhỏ hơn giá trị đã sinh ra x0 sẽ
#         #              bị phạt nặng) ⇒ KHÔNG suy biến về 0.
#         #   sigma→∞  ⇒ log(sigma) → ∞ ⇒ KHÔNG suy biến về vô cực.
#         # Đây là lý do NLL đầy đủ (không chỉ nửa sau +log_sigma như trick
#         # Kendall) là bắt buộc cho riêng tham số này — Kendall trick chỉ
#         # an toàn khi sigma là TRỌNG SỐ loss (không đổi input distribution
#         # của network); sigma_inference vừa là trọng số vừa là scale của
#         # chính x0, nên cần ràng buộc 2 chiều đầy đủ.
#         if x0_reg is not None:
#             x0_for_nll = x0_reg
#         elif need_shared_fwd:
#             x0_for_nll = x0_s
#         else:
#             x0_for_nll = torch.randn_like(x1_rel) * self.sigma_inference.detach()
#         sig_eff    = self.sigma_inference
#         l_sigma_nll = (0.5 * (x0_for_nll.detach() / sig_eff).pow(2).mean()
#                        + torch.log(sig_eff) + 0.5 * math.log(2.0 * math.pi))
...
#         # ramp_reg/ramp_dir vẫn nhân vào để giữ đúng curriculum gating.
#         #
#         # [FIX total-âm] Root cause đã xác định: công thức NLL Gaussian đầy
#         # đủ (Kendall, Gal & Cipolla 2018, eq.5-8) là
#         #   NLL_i = 0.5·precision_i·loss_i + log(sigma_i) + 0.5·log(2π)
#         # Bản trước THIẾU hằng số +0.5·log(2π)≈0.919 mỗi loss phụ. Hằng số
#         # này KHÔNG có gradient (const), nên KHÔNG ảnh hưởng hướng học của
#         # log_sigma_i hay velocity network — nhưng THIẾU nó khiến `total`
#         # bị lệch âm một cách hệ thống bất cứ khi nào log_sigma_i < 0 (tức
#         # sigma_i < 1, hoàn toàn bình thường khi loss phụ đã nhỏ), dẫn tới
#         # log huấn luyện hiển thị tot=-3.9x... trông như lỗi dù về mặt
#         # gradient không sai. Comment cũ "clamp ngăn total âm" là SAI — nó
#         # chỉ ngăn log_sigma → -∞ (tránh total âm VÔ HẠN/NaN), không ngăn
#         # total âm ở mức hữu hạn. Thêm đúng hằng số Kendall làm `total`
#         # phản ánh đúng ý nghĩa NLL, và ở điểm cân bằng tốt (sigma≈std
#         # thực của loss) total sẽ dao động quanh 0 thay vì trôi âm không
#         # kiểm soát được theo thời gian training.
#         HALF_LOG_2PI = 0.5 * math.log(2.0 * math.pi)   # ≈0.9189, hằng số toán học cố định (không phải hyperparam)

#         prec_reg     = torch.exp(-2.0 * self.log_sigma_reg.clamp(min=-3.0))
#         prec_heading = torch.exp(-2.0 * self.log_sigma_heading.clamp(min=-3.0))
#         prec_calib   = torch.exp(-2.0 * self.log_sigma_calib.clamp(min=-3.0))
#         prec_spdrat  = torch.exp(-2.0 * self.log_sigma_speed_ratio.clamp(min=-3.0))
#         prec_signll  = torch.exp(-2.0 * self.log_sigma_sigma_nll.clamp(min=-3.0))

#         # clamp(min=-3): ngăn log_sigma → -∞ (suy biến ngược, total → -∞
#         # / NaN). log_sigma=-3 → sigma=e^{-3}≈0.05 → prec=e^6≈403 →
#         # effective_lambda≈201. Đây là mức tối đa hợp lý (không có loss
#         # phụ nào cần weight >200×L_CFM). Khi l_reg/l_heading/l_calib rất
#         # nhỏ (model học tốt), optimizer muốn kéo log_sigma → -∞ để total
#         # âm vô hạn (gian lận) — clamp ngăn trường hợp suy biến này. Total
#         # âm HỮU HẠN (vd -2 đến 0) vẫn hoàn toàn hợp lệ về mặt NLL, không
#         # phải bug — chỉ là total < 0 khi model đã học tốt (sigma nhỏ hơn
#         # 1 ở điểm cân bằng), giống hệt log-likelihood có thể dương hoặc
#         # âm tuỳ scale của log-density.
...
#         weighted_calib    = ramp_calib * (0.5 * prec_calib   * l_calib_ade    + self.log_sigma_calib.clamp(min=-3.0)        + HALF_LOG_2PI)
#         weighted_spdratio = ramp_calib * (0.5 * prec_spdrat  * l_speed_ratio  + self.log_sigma_speed_ratio.clamp(min=-3.0)  + HALF_LOG_2PI)

#         # [LEARN-6+8] l_sigma_nll đi qua ĐÚNG CÙNG cơ chế Kendall-with-const
#         # như 4 loss phụ trên — không có hằng số riêng nào (0.01 cũ hay
#         # 1/pred_len vừa cân nhắc) áp đặt bên ngoài. log_sigma_sigma_nll
#         # tự học trọng số tương đối của term này so với các loss khác,
#         # cùng cơ chế, không ngoại lệ.
#         #
#         # [KHÔNG có ramp gate — có chủ đích, khác 4 term trên] 4 term
#         # weighted_reg/heading/calib/spdratio đều gate theo epoch vì
#         # chúng phụ thuộc `cond` (qua velocity(x0, t0, cond)), mà `cond`
#         # chưa đáng tin cậy khi encoder còn frozen (ep0-9). L_sigma_nll
#         # thì KHÔNG đi qua cond/encoder/velocity theo cách đó — nó chỉ đo
#         # NLL của chính x0 (một sample thuần tuý ~N(0,σ²), x0_for_nll đã
#         # .detach() cắt đường gradient về network) so với σ hiện tại. Vì
#         # không phụ thuộc trạng thái encoder-freeze, không có lý do
#         # curriculum nào để trì hoãn nó — sigma_inference cần được điều
#         # chỉnh đúng ngay từ epoch 0 vì nó ảnh hưởng TẤT CẢ các phép sample
#         # x0 trong toàn bộ training (kể cả L_CFM sigma riêng, xem
#         # _sigma_schedule — dù đó là 2 sigma độc lập).
#         weighted_signll = 0.5 * prec_signll * l_sigma_nll + self.log_sigma_sigma_nll.clamp(min=-3.0) + HALF_LOG_2PI

#         total = (l_cfm + weighted_reg + weighted_heading + weighted_calib
#                  + weighted_spdratio + weighted_signll)
#         if not torch.isfinite(total):
#             total = x0.new_zeros(())
...
#             "l_reg":     l_reg.item() if torch.is_tensor(l_reg) else 0.0,
#             "l_heading": l_heading.item() if torch.is_tensor(l_heading) else 0.0,
#             "l_calib_ade":   l_calib_ade.item(),
#             "l_speed_ratio": l_speed_ratio.item(),
#             "l_momentum": 0.0,   # always 0
#             "lam_reg":   ramp_reg,   # gate value (curriculum on/off), kept for compat
#             "lam_dir":   ramp_dir,   # gate value (curriculum on/off), kept for compat
#             "lam_calib": ramp_calib, # gate value (curriculum on/off) — [FIX-XAI5]
#             "sigma":     sigma,
#             "ade_1step": ade_log,
#             "hard_score_mean": float(h_score.detach().mean()),
#             "hard_score_max":  float(h_score.detach().max()),
#             # [LEARN-4/7/8] learned cross-loss weights — effective lambda = 0.5*prec
#             "learned_lambda_reg":         float((0.5 * prec_reg).detach()),
#             "learned_lambda_heading":     float((0.5 * prec_heading).detach()),
#             "learned_lambda_calib":       float((0.5 * prec_calib).detach()),
#             "learned_lambda_speed_ratio": float((0.5 * prec_spdrat).detach()),
#             "learned_lambda_sigma_nll":   float((0.5 * prec_signll).detach()),
#             # [LEARN-6] learned sigma_inference (was hardcoded 0.04)
#             "l_sigma_nll":         l_sigma_nll.item(),
#             "learned_sigma_infer": float(self.sigma_inference.detach()),
#             # Compat keys for older scripts
#             "l_fm": l_cfm.item(), "dpe": 0., "heading": 0., "vel_reg": 0.,
#             "speed": 0., "accel": 0., "fm_mse": l_cfm.item(),
#             "l_hard_total": 0., "n_hard": 0, "alpha_hard": 0.,
...
#         # [VECTORIZE] out[t] = last_obs + sum(disp_cal[0:t+1]) — cumulative
#         # sum thuần tuý, KHÔNG có .detach() hay chuỗi phụ thuộc phức tạp
#         # nào (khác _heading_loss_ms nơi mỗi bước detach reference của
#         # bước trước) — an toàn để thay vòng for bằng torch.cumsum trực
#         # tiếp, kết quả numerically giống hệt, chỉ khác cách thực thi.
...
#             "sigma_inference": float(self.sigma_inference.detach()),
#         }
...
#         Hedges against speed distribution shift between val and test.
#         """
#         # [FIX] sigmas trước hardcode quanh 0.04 (giá trị sigma_inference
#         # CŨ trước khi nó learnable). Giờ sigma_inference đã học được và
#         # có thể hội tụ về giá trị khác 0.04 — center dải sweep quanh giá
#         # trị ĐÃ HỌC thay vì hardcode, để test-time exploration vẫn có ý
#         # nghĩa dù training hội tụ về đâu. Biên độ dải (±62.5%/±25% quanh
#         # tâm) giữ nguyên tỷ lệ tương đối như trước — đây là test-time
#         # ROBUSTNESS SWEEP (multi-scale ensemble), không phải một giá trị
#         # cần "đúng tuyệt đối", nên không cần learnable, chỉ cần bám theo
#         # sigma chính đã học.
#         if sigmas is None:
#             s0 = float(self.sigma_inference.detach())
#             sigmas = [s0 * 0.625, s0 * 0.875, s0, s0 * 1.25, s0 * 1.625]
...
class TCFlowMatching(nn.Module):
    ...
    def get_loss_breakdown(self, batch_list, epoch: int = 0, **kwargs) -> Dict:
        ...
        # [FIX tốc độ train] TRƯỚC: `if ramp_reg > 0.0: ... else: zeros()`.
        # Với torch.compile(mode="reduce-overhead") (dùng CUDA graphs),
        # branch Python dựa trên GIÁ TRỊ SỐ (ramp_reg đổi theo epoch) buộc
        # graph phải RECOMPILE mỗi khi nhánh đổi lần đầu — quan sát thực tế:
        # epoch 0-9 (frozen) ~400-460s/epoch, NHƯNG epoch 10+ (đúng lúc
        # ramp_reg>0 lần đầu VÀ encoder unfreeze cùng lúc) nhảy lên
        # ~900-980s/epoch, gần gấp đôi, và giữ nguyên mức chậm đó luôn về
        # sau — dấu hiệu graph không còn ổn định 1 lần mà bị rebuild lặp
        # lại quanh các điểm gãy ep5/ep10/ep20/ep30 của 3 ramp khác nhau.
        # FIX: LUÔN chạy _reg_loss (không branch), nhân kết quả với
        # ramp_reg ở cuối. Tại ramp_reg=0 kết quả loss = 0 (đúng ý nghĩa
        # gate cũ), nhưng graph tính toán KHÔNG ĐỔI giữa các epoch —
        # torch.compile chỉ compile 1 lần duy nhất. Đánh đổi: tốn thêm vài
        # forward pass nhỏ trong 10 epoch đầu (khi lẽ ra được skip hẳn) —
        # chi phí này rất nhỏ so với ~460s/epoch tiết kiệm được từ việc
        # không recompile.
        l_reg, x0_reg = self._reg_loss(x1_rel, last_obs, cond, h_score)
        # [FIX double-multiply] KHÔNG nhân ramp_reg ở đây nữa — ramp chỉ
        # nhân đúng 1 lần trong weighted_reg (Kendall block, phía dưới),
        # nơi nó cũng gate luôn phần +log_sigma+const cho đúng ý nghĩa
        # "gate đóng ⇒ đóng góp = 0 hoàn toàn vào total".
        ...
        # riêng của nó vì chạy trong hàm _reg_loss riêng, không đổi).
        #
        # [FIX tốc độ] TRƯỚC: `need_shared_fwd = (ramp_dir>0) or (ramp_calib>0)`
        # rồi `if need_shared_fwd:` — cùng vấn đề graph-break như ramp_reg
        # ở trên, và tệ hơn vì đây là graph LỚN NHẤT (1 forward velocity()
        # đầy đủ). LUÔN chạy shared forward, không branch theo ramp nữa.
        x0_s       = torch.randn_like(x1_rel) * self.sigma_inference
        v_s        = self.velocity(x0_s, torch.zeros(B, device=device), cond)
        x1_s_abs   = self._from_relative(x0_s + v_s, last_obs)          # [B,T,2] norm
        pred_s_abs = x1_s_abs.permute(1, 0, 2)                          # [T,B,2] norm
        ...
        # [FIX tốc độ] Luôn tính l_heading, nhân ramp_dir ở cuối (thay vì
        # if/else) — cùng lý do graph-stability như trên.
        # [FIX double-multiply] KHÔNG nhân ramp_dir ở đây nữa — cùng lý do
        # như l_reg ở trên, ramp chỉ nhân 1 lần trong weighted_heading.
        l_heading = self._heading_loss_ms(pred_deg_s, obs_deg_s)
        ...
        # so the network and the correction co-adapt during training).
        #
        # [FIX tốc độ] Luôn tính, nhân ramp_calib ở cuối — không còn
        # `if ramp_calib > 0.0:` branch (đã là graph-break thứ 3/4 tìm
        # được, đúng khớp điểm gãy tốc độ ep10 quan sát trong log thực).
        cal_abs    = self.speed_calibrate_pred(pred_s_abs, last_obs, obs_traj[:, :, :2])
        cal_deg    = _norm_to_deg(cal_abs)
        gt_c_deg   = _norm_to_deg(x1_gt.permute(1, 0, 2))
        l_calib_ade = _haversine_deg(cal_deg, gt_c_deg).mean() / 300.0
        ...
        # log_sigma_speed_ratio) — trọng số tương đối giữa "ADE tổng"
        # và "độ chính xác speed-ratio" hoàn toàn do gradient quyết
        # định, không còn hệ số 0.5 áp đặt.
        pts_cal    = torch.cat([_norm_to_deg(last_obs).unsqueeze(0), cal_deg], 0)
        pred_spd_c = _step_speeds_kmh(pts_cal)                                # [T,B] km/h
        pts_gt_c   = torch.cat([_norm_to_deg(last_obs).unsqueeze(0), gt_c_deg], 0)
        gt_spd_c   = _step_speeds_kmh(pts_gt_c).clamp(min=1.0)                # [T,B]
        ...
        # [FIX double-multiply] KHÔNG nhân ramp_calib ở đây nữa — cùng lý
        # do như trên, ramp chỉ nhân 1 lần trong weighted_calib/spdratio.

        # ── L_sigma_nll — [LEARN-6] trains log_sigma_infer ───────────────────
        # Gaussian NLL trên x0 đã sample (dùng lại x0_reg — [FIX tốc độ]
        # giờ LUÔN có giá trị thật vì _reg_loss không còn bị if-gate nữa,
        # không cần fallback None-check/sample thừa như bản trước).
        # NLL(x0; sigma) = 0.5*(x0/sigma)² + log(sigma)
        #   sigma→0  ⇒ (x0/sigma)² → ∞ (x0 cố định, không đổi theo sigma
        #              vì x0 = randn*sigma_OLD tại thời điểm sample, chỉ
        #              log_sigma_infer hiện tại tham gia vào MẪU SỐ của
        ...
        weighted_calib    = ramp_calib * (0.5 * prec_calib   * l_calib_ade    + self.log_sigma_calib.clamp(min=-3.0)        + HALF_LOG_2PI)
        weighted_spdratio = ramp_calib * (0.5 * prec_spdrat  * l_speed_ratio  + self.log_sigma_speed_ratio.clamp(min=-3.0)  + HALF_LOG_2PI)
        # [FIX double-multiply] l_reg/l_heading/l_calib_ade/l_speed_ratio
        # KHÔNG còn bị nhân ramp ở phía trên nữa (đã bỏ, xem chỗ tính từng
        # loss) — ramp chỉ nhân ĐÚNG 1 LẦN ở đây, bao trùm luôn cả phần
        # +log_sigma+const (khi gate đóng, +log_sigma cũng không nên
        # đóng góp vào total — nếu chỉ gate riêng phần loss mà giữ nguyên
        # +log_sigma+const thì total vẫn nhận một hằng số "ma" ~0.9 mỗi
        # term ngay cả lúc gate=0, sai ý nghĩa "loss này chưa hợp lệ,
        # đóng góp = 0 hoàn toàn" của ramp gate).
