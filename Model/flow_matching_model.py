...
"""
Model/flow_matching_model.py — TC-FlowMatching v53
═══════════════════════════════════════════════════════════════════════════════

FIXES vs v52 (3 bugs patched):

  [FIX-B] heading loss → km-space với cos_lat correction
    v52 bug: pv = pred_deg[1:T] - pred_deg[:T-1]  (degree-space)
             normalize(Δlon, Δlat) bị bias vì 1°lon≠1°lat (~14% error tại lat15°)
    v53 fix: convert sang km trước khi normalize:
             pv_km = [Δlon * cos_lat * 111, Δlat * 111]
             normalize(pv_km)
    Tác động: heading supervision đúng với NW/SW (SCS most common) → CTE −5–10%

  [FIX-C] OT matching: x1_batch→x1_batch → x0_batch→x1_batch
    v52 bug: _geodesic_ot_cost(x1_batch, x1_batch, lp)
             cost x1→x1 giống nhau → OT vô nghĩa, random sampling
    v53 fix: _geodesic_ot_cost(x0_batch, x1_batch, lp)
    Tác động: noise-trajectory alignment đúng → training converge nhanh hơn

  [FIX-D] steering_enc: Linear(4→7), thêm steering_speed/sin/cos
    v52: chỉ [u500_mean, v500_mean, u500_center, v500_center] (4 dims)
    v53: thêm [steering_speed, steering_dir_sin, steering_dir_cos] (7 dims)
         u/v500 normalized /30.0 trước khi feed
    Tác động: sử dụng pre-computed steering features từ ENV_fixed v13 → CTE −3–5%
    ⚠  steering_enc layer shape thay đổi → checkpoint v52 load với strict=False
       steering_enc sẽ reinit random, cần ~5 epoch để ổn định

Kept from v52:
  _signed_ate_loss: km-space [FIX-P1-A]
  _signed_cte_loss: mirror [FIX-P2-B]
  vel_reg: no ratio, clamp 5.0 [FIX-P1-B]
  sph_ate_cte: ate_w=2.0, cte_w=2.5 [FIX-P2-A]
  heading weight: 0.20 [FIX-P2-C]
  EMA decay 0.995, sigma schedule, use_slerp=False

TARGETS: ATE<79.94 | CTE<93.58 | ADE<136.41 | 72h<297

MONITOR sau ep16-20:
  signed_ate↓ và signed_cte↓ đồng thời → balanced
  ate_mean_km < 130km @ ep18, < 100km @ ep22
  vel_reg > 0.03 (active), heading > 0.05
"""
from __future__ import annotations
...
# ══════════════════════════════════════════════════════════════════════════════
#  [FIX-P2-A] Spherical ATE/CTE loss — REBALANCED
# ══════════════════════════════════════════════════════════════════════════════

def _spherical_ate_cte_loss(pred_deg, gt_deg, ate_weight=2.0, cte_weight=2.5):
    """
    [FIX-P2-A] Rebalanced: ate_weight 5→2, cte_weight 1→2.5
    Tránh trade-off: cả ATE và CTE đều có supervision budget tương đương.
    """
    T = min(pred_deg.shape[0], gt_deg.shape[0])
    if T < 2:
        zero = pred_deg.new_zeros(())
...
# ══════════════════════════════════════════════════════════════════════════════
#  [FIX-P1-B] Velocity regression loss — bỏ ratio, giữ abs+vec
# ══════════════════════════════════════════════════════════════════════════════

def _velocity_regression_loss(pred_deg, gt_deg, speed_stats=None):
    """
    [FIX-P1-B] Bỏ l_ratio (bias toward faster), giữ l_abs + l_vec
    clamp gt_spd min 5.0 (SCS realistic minimum, không phải 2.0)

    l_abs: |pred_spd - gt_spd|² / v_sigma²  → magnitude supervision
    l_vec: ||pred_vel_km - gt_vel_km||²      → direction + magnitude
    """
    sp      = speed_stats or _SPEED_PRIOR
    v_sigma = sp.get("v_sigma", 10.0)
    T = min(pred_deg.shape[0], gt_deg.shape[0])
    ...
    # FIX-P1-B: clamp min=5.0 (không phải 2.0), bỏ ratio loss
    gt_spd_clamped = gt_spd.clamp(min=5.0)
    l_abs = ((pred_spd - gt_spd_clamped).pow(2) / v_sigma**2
             * w.unsqueeze(1)).mean()

    # Vector supervision (direction + magnitude combined)
    plon = pred_deg[1:T, :, 0] - pred_deg[:T-1, :, 0]
    plat = pred_deg[1:T, :, 1] - pred_deg[:T-1, :, 1]
    glon = gt_deg[1:T, :, 0]   - gt_deg[:T-1, :, 0]
    glat = gt_deg[1:T, :, 1]   - gt_deg[:T-1, :, 1]
    ...
    # Chỉ dùng abs + vec (bỏ ratio)
    return (0.5*l_abs + 0.5*l_vec).clamp(0.0, 20.0)


# ══════════════════════════════════════════════════════════════════════════════
#  [FIX-P1-A] Signed ATE loss — km-space đúng
# ══════════════════════════════════════════════════════════════════════════════

def _signed_ate_loss(pred_deg, gt_deg):
    """
    [FIX-P1-A] Direct along-track error penalization.
    FIX: tính gt_dir trong km-space từ đầu, tránh double cos_lat.

    v51 bug: gt_dir_n = normalize(delta_degrees) → rồi nhân cos_lat
             → double-counting với hướng chéo NW/SW
    v52 fix: dx_km = delta_lon * cos_lat * 111
             dy_km = delta_lat * 111
    ...
    # FIX-P1-A: tính along-track direction trong km-space
    cos_lat = torch.cos(torch.deg2rad(gt_deg[1:T, :, 1])).clamp(1e-4)

    # Ground truth direction trong km (đúng)
    dx_km = (gt_deg[1:T, :, 0] - gt_deg[:T-1, :, 0]) * cos_lat * DEG2KM
    dy_km = (gt_deg[1:T, :, 1] - gt_deg[:T-1, :, 1]) * DEG2KM
    gt_dir_km_n = F.normalize(
        torch.stack([dx_km, dy_km], dim=-1), dim=-1, eps=1e-6)  # [T-1, B, 2]

    # Error vector trong km
    err_lon_km = (pred_deg[1:T, :, 0] - gt_deg[1:T, :, 0]) * cos_lat * DEG2KM
    err_lat_km = (pred_deg[1:T, :, 1] - gt_deg[1:T, :, 1]) * DEG2KM
    err_km = torch.stack([err_lon_km, err_lat_km], dim=-1)  # [T-1, B, 2]

    # ATE = projection lên along-track direction (km)
    ate_km = (err_km * gt_dir_km_n).sum(-1)   # [T-1, B]
...
# ══════════════════════════════════════════════════════════════════════════════
#  [FIX-P2-B] Signed CTE loss — mirror của signed_ate
# ══════════════════════════════════════════════════════════════════════════════

def _signed_cte_loss(pred_deg, gt_deg):
    """
    [FIX-P2-B] Direct cross-track error penalization.
    Mirror của _signed_ate_loss nhưng lấy perpendicular direction.
    Cross-track = projection của error lên hướng vuông góc với track.

    Tại sao cần: _spherical_ate_cte_loss dùng bearing difference
                 → indirect gradient cho CTE
    Hàm này: direct projection → mạnh hơn khi error lớn
    """
    T = min(pred_deg.shape[0], gt_deg.shape[0])
    if T < 3:
        return pred_deg.new_zeros(())
    ...
    # Along-track direction trong km
    dx_km = (gt_deg[1:T, :, 0] - gt_deg[:T-1, :, 0]) * cos_lat * DEG2KM
    dy_km = (gt_deg[1:T, :, 1] - gt_deg[:T-1, :, 1]) * DEG2KM
    gt_dir_km_n = F.normalize(
        torch.stack([dx_km, dy_km], dim=-1), dim=-1, eps=1e-6)

    # Cross-track direction = perpendicular (rotate 90°)
    # n = (-dy, dx) normalized
    ct_dir_km_n = torch.stack([-gt_dir_km_n[..., 1],
                                 gt_dir_km_n[..., 0]], dim=-1)  # [T-1, B, 2]
    ...
    # CTE = projection lên cross-track direction
    cte_km = (err_km * ct_dir_km_n).sum(-1)   # [T-1, B]

    w = pred_deg.new_tensor(STEP_WEIGHTS[1:T]); w = w / w.sum()

    cte_loss = F.huber_loss(
        cte_km, torch.zeros_like(cte_km),
        delta=80.0, reduction="none"
    )
    return (cte_loss * w.unsqueeze(1)).mean() / 100.0


# ══════════════════════════════════════════════════════════════════════════════
#  Direct 72h/48h endpoint loss — scheduled
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
#  [FIX-P2] compute_st_trans_loss — v52 BALANCED
# ══════════════════════════════════════════════════════════════════════════════

def compute_st_trans_loss(pred_deg, gt_deg, epoch=0, speed_stats=None):
    """
    v52 loss — Balanced ATE/CTE:

    THAY ĐỔI SO VỚI v51:
      vel_reg    : bỏ ratio, giữ abs+vec; clamp min=5.0    [FIX-P1-B]
      heading    : 0.08 → 0.20 (CTE supervision)          [FIX-P2-C]
      sph_ate_cte: ate_w=2.0, cte_w=2.5                   [FIX-P2-A]
      signed_cte : MỚI × 0.25 (mirror of signed_ate)      [FIX-P2-B]
      signed_ate : giữ × 0.40 (đã sửa gradient P1-A)      [FIX-P1-A]

    Lý thuyết:
      ATE (speed error)     → vel_reg + signed_ate
      CTE (direction error) → heading + signed_cte + sph_ate(cte_w=2.5)
      Cả hai share         → DPE + endpoint + direct_ep

    Budget (rough):
      ATE:  0.60(vel) + 0.40(sig_ate) + 2/4.5(sph) ≈ 1.44
      CTE:  0.20(head) + 0.25(sig_cte) + 2.5/4.5(sph) ≈ 1.00
      Ratio: ~1.4:1 (vs 30:1 trước) → balanced
    """
    sp         = speed_stats or _SPEED_PRIOR
    v_opt      = sp.get("v_opt",      15.0)
    v_sigma    = sp.get("v_sigma",    10.0)
    ...
    # ── [FIX-P2-C] Heading × 0.20 (CTE supervision) ───────────────────────
    # FIX-BUG2: tính trong km-space (cos_lat correction) tránh bias NW/SW
    if T >= 3:
    ...
    # ── [FIX-P1-B] Velocity regression (abs+vec, no ratio) ─────────────────
    l_vel_reg = _velocity_regression_loss(pred_deg, gt_deg, speed_stats)

    # ── [FIX-P2-A] Spherical ATE/CTE (rebalanced) ─────────────────────────
    ate_d     = _spherical_ate_cte_loss(pred_deg[:T], gt_deg[:T],
                                        ate_weight=2.0, cte_weight=2.5)
    l_sph_ate = ate_d["ate_cte_total"]
    ...
    # ── [FIX-P1-A] Signed ATE (km-space đúng) × 0.40 ─────────────────────
    l_signed_ate = _signed_ate_loss(pred_deg, gt_deg)

    # ── [FIX-P2-B] Signed CTE (NEW mirror) × 0.25 ────────────────────────
    ...
    total = (0.60 * l_dpe
             ...
             + 0.05 * l_speed
             + 0.01 * l_accel
             + 0.20 * l_heading          # [FIX-P2-C] 0.08→0.20
             + 0.60 * l_vel_reg          # [FIX-P1-B] no ratio
             + 0.25 * l_sph_ate          # [FIX-P2-A] rebalanced weights
             + 0.15 * l_endpoint
             + 0.40 * l_signed_ate       # [FIX-P1-A] km-space đúng
             + 0.25 * l_signed_cte       # [FIX-P2-B] NEW
             + l_direct_ep)              # scheduled
    ...
    return dict(
        ...
        sph_ate     = _s(l_sph_ate),
        endpoint    = _s(l_endpoint),
        signed_ate  = _s(l_signed_ate),
        signed_cte  = _s(l_signed_cte),
        direct_ep   = _s(l_direct_ep),
        ate_mean_km = ate_d.get("ate_mean_km", 0.0),
        cte_mean_km = ate_d.get("cte_mean_km", 0.0),
        speed_match=0.0, acc_kmh2=0.0, aux_fno=0.0, sigma=0.0,
...
def _spherical_ot_matching(x0_batch, x1_batch, lp, epsilon=0.05):
    ...
    if B < 4:
        return x0_batch, x1_batch
    try:
        cost = _geodesic_ot_cost(x0_batch, x1_batch, lp)  # FIX-BUG1: x0→x1
        with torch.no_grad():
            pi = _sinkhorn_log(cost, epsilon=epsilon)
        flat = pi.reshape(-1).clamp(0.0)
        s    = flat.sum()
...
# ══════════════════════════════════════════════════════════════════════════════
#  VelocityField (giữ nguyên từ v51)
# ══════════════════════════════════════════════════════════════════════════════
...
class VelocityField(nn.Module):
    ...
    def _get_steering_feat(self, env_data, B, device):
        ...
        def _safe(k, norm=1.0):
            ...
            return (v / norm).clamp(-3.0, 3.0)  # FIX-BUG3: normalize raw m/s
        # FIX-BUG3: /30.0 để normalize raw m/s → [-1,1] range
        # GAP-3: thêm steering_speed/sin/cos (đã có trong env_data từ v13)
        return self.steering_enc(torch.stack([
            _safe("u500_mean",    30.0),   # raw m/s → normalized
            _safe("v500_mean",    30.0),
            _safe("u500_center",  30.0),
            _safe("v500_center",  30.0),
            _safe("steering_speed",  1.0), # đã normalized /20 trong builder
...
# ══════════════════════════════════════════════════════════════════════════════
#  Inference helpers (giữ nguyên từ v51)
# ══════════════════════════════════════════════════════════════════════════════
