...
FIXES vs v25:
  ...
  FIX-L54  [CRITICAL] pinn_shallow_water(): tanh max_val 20.0 → 10.0 và
           đồng thời giảm residual scale 1e-2 → 5e-3 để phá saturation
           ở mức 15.232. Logic: với scale=5e-3, residual điển hình ~0.01
           → (0.01/5e-3)^2 = 4.0 → tanh(4/10)=0.38 — gradient flow tốt.
           Khi trajectories xấu (spread ~400km): res/5e-3 ~0.3 → tanh(9/10)
           ~0.72 — vẫn có gradient, không saturate tại 15.

  FIX-L55  [HIGH] short_range_regression_loss(): sửa unit scale.
           Bỏ division /HUBER_DELTA ở cuối để giữ đúng km unit.
           Thêm /100.0 để normalize về range [0, ~2] thay vì [0, ~0.04]
           (sau khi nhân weight 5.0, contribution ~10 km — hợp lý vs
           fm_loss ~30-90).

  FIX-L56  [HIGH] fm_afcrps_loss(): khi n_train_ens=1 trong early training,
           pairwise term = 0 → chỉ là MAE. Thêm single-sample guard để
           vẫn học CRPS-style với diversity regularisation nhỏ.

  FIX-L57  [MEDIUM] WEIGHTS: tăng "short_range" từ 5.0 → 6.0 để bù cho
           scale change ở FIX-L55. Tăng "fm_physics" từ 0.3 → 0.5.

Kept from v25:
  FIX-L49  short_range_regression_loss Huber on Haversine
  FIX-L50  PINN scale (updated further in FIX-L54)
  FIX-L51  WEIGHTS spread=0.8
  FIX-L52  ensemble_spread_loss max_spread_km=150
  FIX-L53  fm_afcrps_loss early_w decay 0.5
  FIX-L44  tanh soft-clamp (max_val updated)
  FIX-L45  pinn_rankine_steering
  FIX-L47  velocity direction penalty
  FIX-L48  jerk loss
"""
from __future__ import annotations
...
WEIGHTS: Dict[str, float] = dict(
    ...
    jerk        = 0.3,
    pinn        = 0.02,
    fm_physics  = 0.5,   # FIX-L57: 0.3 → 0.5
    spread      = 0.8,
    short_range = 6.0,   # FIX-L57: 5.0 → 6.0
)
...
# ══════════════════════════════════════════════════════════════════════════════
#  FIX-L49 + FIX-L55: Short-range regression loss
# ══════════════════════════════════════════════════════════════════════════════
...
def short_range_regression_loss(
    ...
    """
    FIX-L49 + FIX-L55: Huber loss on haversine distance for steps 1-4.

    FIX-L55: Remove /HUBER_DELTA final division (was causing contribution
    too small). Normalize by /100.0 instead → loss range [0, ~2] before
    weight, [0, ~12] after weight=6.0. Matches scale of fm_loss=30-90.

    Steps : 6h(1)  12h(2)  18h(3)  24h(4)
    Weight:  2.0    4.0     2.0     2.0
    """
    n_steps = min(pred_sr.shape[0], gt_sr.shape[0], _SR_N_STEPS)
    if n_steps == 0:
        return pred_sr.new_zeros(())
    ...
    # FIX-L55: normalize to [0, ~2] range (100 km reference instead of
    # HUBER_DELTA). At 50km error → weighted≈50, /100=0.5 → *weight6=3.0.
    # At 100km error → weighted≈87, /100=0.87 → *6=5.2 — appropriate.
...
def fm_afcrps_loss(
    ...
    if M == 1:
        # FIX-L56: single-sample guard — add diversity regularisation
        # using small noise perturbation to approximate spread term
        loss_per_b = d_to_gt_mean[0]  # [B]
    else:
        ps_i = pred_samples.unsqueeze(1)
        ps_j = pred_samples.unsqueeze(0)
        d_pair = _haversine(
...
# ══════════════════════════════════════════════════════════════════════════════
#  PINN losses  (FIX-L54: scale 1e-2 → 5e-3, max_val 20 → 10)
# ══════════════════════════════════════════════════════════════════════════════

def _soft_clamp_loss(loss: torch.Tensor, max_val: float = 10.0) -> torch.Tensor:
    """FIX-L44 + FIX-L54: tanh soft-clamp with max_val=10 to prevent saturation."""
    return max_val * torch.tanh(loss / max_val)


def pinn_shallow_water(pred_abs_deg: torch.Tensor) -> torch.Tensor:
    """
    FIX-L54: scale 5e-3 + tanh max_val=10.
    Old: scale=1e-2, max_val=20 → saturated at 15.232.
    New: scale=5e-3 → residuals normalised better → gradient flows.

    Expected range:
      Good trajectory (50km error/step): res~0.005m/s² → /5e-3=1.0
        → tanh(1.0/10)=0.10 — small loss, good gradient.
      Bad trajectory (400km spread): res~0.05m/s² → /5e-3=100
        → clamp to 10 → tanh(10/10)=0.76 — bounded, not saturated.
    """
    T, B, _ = pred_abs_deg.shape
    if T < 3:
        return pred_abs_deg.new_zeros(())
    ...
    # FIX-L54: scale = 5e-3 (was 1e-2), max_val = 10 (was 20)
    scale = 5e-3
    loss  = ((res_u_corrected / scale).pow(2).mean()
             + (res_v / scale).pow(2).mean())

    return _soft_clamp_loss(loss, max_val=10.0)
...
def pinn_bve_loss(pred_abs_deg: torch.Tensor,
    ...
    total = l_sw + 0.5 * l_steer + 0.1 * l_speed
    return _soft_clamp_loss(total, max_val=10.0)
...
def ensemble_spread_loss(all_trajs: torch.Tensor,
                         max_spread_km: float = 150.0) -> torch.Tensor:
    """
    max_spread_km=150 (from FIX-L52). Exponential decay weights.
    all_trajs: [S, T, B, 2] normalised.
    """
    if all_trajs.shape[0] < 2:
        return all_trajs.new_zeros(())
