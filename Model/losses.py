...
FIXES vs v17:
  ...
  FIX-L24  [CRITICAL] compute_total_loss signature mới: nhận gt_norm (normalized)
           cho AFCRPS và pred_abs_deg (degrees) cho directional losses.
           v17 dùng cùng một tensor cho tất cả losses — sai vì:
           - AFCRPS cần normalized coords với unit_01deg=True
           - Velocity/heading/recurv/pinn cần degrees để Haversine có ý nghĩa
           
  FIX-L25  pinn_bve_loss giờ nhận pred_abs_deg (degrees) → lat_rad đúng.
           Trước đây lat_rad = normalized_lat * 5/180*pi ≈ 0 → beta_n ≈ max
           dẫn đến PINN loss vô nghĩa.

  FIX-L26  velocity_loss_per_sample với degrees: step displacement ~15-50 km
           (trong degrees ~0.1-0.5) thay vì ~0.01-0.05 (normalized). Scale
           nhân 30.0 không còn cần thiết — giảm xuống 1.0 để balance.

  FIX-L27  smooth_loss và acceleration_loss với degrees: second difference
           của degrees ~ 0.01-0.1, nhân 10.0 hợp lý.

  FIX-L28  Thêm fallback: nếu gt_norm không được pass (backward compat),
           dùng pred_abs làm proxy cho AFCRPS với unit_01deg=False.

Kept from v17:
  FIX-L21..L23 (recurvature detection/weighting, turning point loss)
  FIX-L17..L20 (velocity, acceleration, disp, CRPS)
  FIX-L13..L16 (pinn clamp, NaN guard, eps floor)
"""
from __future__ import annotations
...
R_EARTH      = 6.371e6
DT_6H        = 6 * 3600
# FIX-L25: NORM_TO_DEG không còn dùng trong PINN vì input đã là degrees
NORM_TO_DEG  = 1.0   # identity khi input đã là degrees
NORM_TO_M    = 111_000.0  # 1 degree ≈ 111 km

PINN_SCALE   = 1.0
WEIGHTS: Dict[str, float] = dict(
    fm=2.0,        # Vị trí (AFCRPS km)
    velocity=2.0,  # Tốc độ
    heading=1.5,   # Độ cong
    recurv=1.5,    # Turning point
    step=0.5,      # Hướng từng bước
...
# ── Haversine (dùng cho AFCRPS với normalized coords) ────────────────────────
...
# ── Haversine (dùng cho directional losses với degree coords) ─────────────────

def _haversine_deg(p1: torch.Tensor, p2: torch.Tensor) -> torch.Tensor:
    """Input đã là degrees (lon, lat)."""
    return _haversine(p1, p2, unit_01deg=False)


# ── FIX-L21: Recurvature detection ───────────────────────────────────────────

def _total_rotation_angle_batch(gt: torch.Tensor) -> torch.Tensor:
    """
    gt: [T, B, 2] — degrees (lon, lat)
    Returns: [B] tensor, đơn vị degrees
    """
    T, B, _ = gt.shape
    if T < 3:
    ...
    lats_rad = torch.deg2rad(gt[:, :, 1])    # [T, B]
    cos_lat  = torch.cos(lats_rad[:-1])       # [T-1, B]
    dlat = gt[1:, :, 1] - gt[:-1, :, 1]      # [T-1, B] in degrees
    dlon = (gt[1:, :, 0] - gt[:-1, :, 0]) * cos_lat  # [T-1, B]
    v = torch.stack([dlon, dlat], dim=-1)     # [T-1, B, 2]
    ...
    total_deg = torch.rad2deg(angles_rad).sum(0)
    return total_deg
...
def recurvature_loss(pred: torch.Tensor, gt: torch.Tensor) -> torch.Tensor:
    ...
    cos_sim = (p_hat * g_hat).sum(-1)
    dir_loss = (1.0 - cos_sim)

    mask = sign_change.float()
    if mask.sum() < 1:
        return pred.new_zeros(())
    weighted = (dir_loss * mask).sum() / mask.sum().clamp(min=1)
    return weighted


# ── AFCRPS loss (dùng normalized coords, unit_01deg=True) ─────────────────────
...
def fm_afcrps_loss(
    ...
    """
    AFCRPS với time-weighted CRPS.
    pred_samples: [M, T, B, 2] normalized coords
    gt: [T, B, 2] normalized coords
    unit_01deg=True: dùng denorm formula để convert sang degrees cho Haversine
    """
    M, T, B, _ = pred_samples.shape
    eps = 1e-3
...
# ── PINN BVE (FIX-L25: input đã là degrees) ──────────────────────────────────

def _pinn_simplified(pred_abs_deg: torch.Tensor) -> torch.Tensor:
    """
    FIX-L25: pred_abs_deg: [T, B, 2] in degrees (lon, lat)
    DT = 6h per step, displacement in degrees → convert sang meters.
    """
    T = pred_abs_deg.shape[0]
    if T < 5:
        return pred_abs_deg.new_zeros(())

    # Step velocity in degrees per 6h
    v   = pred_abs_deg[1:] - pred_abs_deg[:-1]   # [T-1, B, 2] degrees/step
    # Convert to m/s for BVE: 1 degree lat ≈ 111 km, 1 step = 6h
    DEG_TO_M_PER_S = 111_000.0 / DT_6H            # ~5.08 m/s per degree/step

    vx = v[..., 0] * DEG_TO_M_PER_S  # [T-1, B] u-component (m/s)
    vy = v[..., 1] * DEG_TO_M_PER_S  # [T-1, B] v-component (m/s)

    zeta    = vx[1:] * vy[:-1] - vy[1:] * vx[:-1]  # [T-2, B]
    dzeta   = zeta[1:] - zeta[:-1]                   # [T-3, B]

    # Latitude of midpoint in radians (degrees input)
    lat_deg = pred_abs_deg[2:T-1, :, 1]              # [T-3, B]
    lat_rad = torch.deg2rad(lat_deg)

    beta_n  = (2 * OMEGA / R_EARTH) * torch.cos(lat_rad)  # [T-3, B]
...
# ── Per-sample losses (degrees) ───────────────────────────────────────────────

def velocity_loss_per_sample(pred: torch.Tensor, gt: torch.Tensor) -> torch.Tensor:
    """
    pred, gt: [T, B, 2] degrees
    Returns [B] — loss in degree^2 units.
    FIX-L26: không cần scale 30x vì degrees tự nhiên lớn hơn normalized.
    """
    if pred.shape[0] < 2:
        return pred.new_zeros(pred.shape[1])
    ...
    # Speed loss: L2 trên step displacement (degrees/step)
    s_pred = torch.norm(v_pred, dim=-1)
    s_gt   = torch.norm(v_gt,   dim=-1)
    l_speed = (s_pred - s_gt).pow(2).mean(0)

    gt_v_unit = v_gt / torch.norm(v_gt, dim=-1, keepdim=True).clamp(min=1e-6)
    ate_errors = ((v_pred - v_gt) * gt_v_unit).sum(-1)
    l_ate = ate_errors.pow(2).mean(0)

    return l_speed + 0.5 * l_ate


def disp_loss_per_sample(pred: torch.Tensor, gt: torch.Tensor) -> torch.Tensor:
    """pred, gt: [T, B, 2] degrees. Returns [B]."""
    if pred.shape[0] < 2:
        return pred.new_zeros(pred.shape[1])
    pd = (pred[1:] - pred[:-1]).norm(dim=-1).mean(0)
    gd = (gt[1:]   - gt[:-1]).norm(dim=-1).mean(0)
...
def compute_total_loss(
    pred_abs,           # [T, B, 2] DEGREES — cho directional losses
    gt,                 # [T, B, 2] DEGREES — cho directional losses
    ref,                # [B, 2] DEGREES
    batch_list,
    pred_samples=None,  # [S, T, B, 2] NORMALIZED — cho AFCRPS
    gt_norm=None,       # [T, B, 2] NORMALIZED — cho AFCRPS (FIX-L28)
    weights=WEIGHTS,
    intensity_w: Optional[torch.Tensor] = None,
) -> Dict:
    """
    FIX-L24: pred_abs và gt đã là DEGREES (từ flow_matching_model FIX-M8).
    pred_samples và gt_norm vẫn là NORMALIZED cho AFCRPS với unit_01deg=True.

    FIX-L28: Fallback nếu gt_norm=None — dùng gt trực tiếp với unit_01deg=False.
    """
    # 1. TRỌNG SỐ MẪU
    recurv_w = _recurvature_weights(gt, w_recurv=2.5)  # [B], gt là degrees
    sample_w = recurv_w * (intensity_w.to(gt.device) if intensity_w is not None else 1.0)
    sample_w = sample_w / sample_w.mean().clamp(min=1e-6)

    # 2. AFCRPS — dùng normalized coords (FIX-L24)
    if pred_samples is not None:
        if gt_norm is not None:
            # FIX-L28: dùng gt_norm (normalized) với unit_01deg=True
            l_fm = fm_afcrps_loss(pred_samples, gt_norm,
                                  intensity_w=sample_w, unit_01deg=True)
        else:
            # Fallback: convert pred_samples từ normalized về degrees inline
            # pred_samples shape [S, T, B, 2] normalized
        ...
        # Không có ensemble — dùng mean pred với Haversine degrees
        l_fm = _haversine_deg(pred_abs, gt).mean()

    # 3. CÁC THÀNH PHẦN LOSS (degrees input) — FIX-L26: scale giảm
    l_vel  = (velocity_loss_per_sample(pred_abs, gt) * sample_w).mean()
    l_disp = (disp_loss_per_sample(pred_abs, gt) * sample_w).mean()
    l_step = (step_dir_loss_per_sample(pred_abs, gt) * sample_w).mean()
    ...
    # 4. TỔNG HỢP
    # FIX-L26: scale velocity/disp giảm vì degrees tự nhiên lớn hơn normalized
    # velocity: degree/step ~ 0.1-0.5, vs normalized ~ 0.01-0.05 → scale ~10x
    # Không còn cần nhân 30x — giảm về 3x để vẫn có weight tương đối với AFCRPS
    total = (
        weights['fm']       * l_fm +
        weights['velocity'] * l_vel   * 3.0 +   # FIX-L26: 30→3
        weights['disp']     * l_disp  * 3.0 +   # FIX-L26: 30→3
        weights['step']     * l_step  * 1.0 +   # FIX-L26: 10→1
        weights['heading']  * l_heading * 1.0 + # FIX-L26: 10→1
        weights['recurv']   * l_recurv  * 1.0 + # FIX-L26: 10→1
        weights['dir']      * l_dir_final * 0.5 +
        weights['smooth']   * l_smooth * 1.0 +
        weights['accel']    * l_accel  * 1.0 +
        weights['pinn']     * l_pinn
    )
