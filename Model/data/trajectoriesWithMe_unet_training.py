...
"""
Model/data/trajectoriesWithMe_unet_training.py  ── v15
======================================================
TC trajectory dataset — TRAINING VERSION.

FIXES vs v14:

  FIX-DATA-8  DATA3D_MEAN/STD ch0 (gph500_mean) updated for new unit.
              Dataset now stores gph500 as raw_geopotential / 9.80665
              (meters of geopotential height, ~1268 m at 500hPa).
              Old:  mean=12439.46, std=91.59  (raw geopotential m²/s²)
              New:  mean=1287.66,  std=144.31  (meters after /g)
              DATA3D sentinel guard also updated to match new unit range.

  FIX-DATA-9  env_data_processing() sentinel guard for gph500 updated
              to match new /g unit: [1100, 2500] (was [10, 100]).

Kept from v14:
  FIX-DATA-5: ch3 u500_center mean=5930.27 std=1025.26 (was 752/28)
  FIX-DATA-5: ch4 v500_center mean=1622.27 std=1600.32 (was 480/24)
...
# Full history of fixes across versions:
#
#  Ch  Name             v13 (wrong)       v14 (fixed)       v15 (this)
#  0   gph500_mean      12439.46/91.59    12439.46/91.59    1287.66/144.31 ← FIX-DATA-8
#  1   u500_mean        5843.14/50.55     5843.14/50.55     unchanged ✓
#  2   v500_mean        1482.47/29.42     1482.47/29.42     unchanged ✓
#  3   u500_center       752.80/28.49     5930.27/1025.26   unchanged ✓
#  4   v500_center       480.31/24.17     1622.27/1600.32   unchanged ✓
#  5-11 (wind fields, small magnitude)   unchanged          unchanged
#  12  SST              300.95/3.05       unchanged          unchanged

DATA3D_MEAN = np.array([
    1287.66,    # ch0  gph500_mean  FIX-DATA-8: raw/9.80665 → meters (was 12439.46)
    ...
    1.34,       # ch10 unchanged
    0.94,       # ch11 unchanged
    300.95,     # ch12 SST         unchanged
], dtype=np.float32)

DATA3D_STD = np.array([
    144.31,     # ch0  gph500_mean  FIX-DATA-8 (was 91.59)
    50.55,      # ch1  u500_mean    ✓
    29.42,      # ch2  v500_mean    ✓
...
# FIX-DATA-8: ch0 valid range after /g: [1100, 2500] m
# Note: zero-sentinel still triggers for ch0 before large-sentinel check,
# but _DATA3D_SENTINEL_LARGE=20000 handles the pre-division raw values
# if any files haven't been updated yet (backward compatible).
_DATA3D_GPH_VALID_MIN = 1100.0   # m after /g  (was implied by MEAN-3*STD)
_DATA3D_GPH_VALID_MAX = 2500.0   # m after /g
...
def env_data_processing(env_dict: dict) -> dict:
    """
    Clean env_npy dictionary.
    FIX-DATA-9: gph500 sentinel updated to [1100, 2500] for /g unit.
    """
    if not isinstance(env_dict, dict):
        return {}
    cleaned = {}
    ...
    # GPH500 sentinel (FIX-DATA-9: updated range for /g unit)
    for gph_key in ("gph500_mean", "gph500_center"):
        if gph_key in cleaned:
            val = cleaned[gph_key]
            if val is not None and isinstance(val, (int, float)):
                if val < 1100.0 or val > 2500.0:
                    cleaned[gph_key] = None
...
class TrajectoryDataset(Dataset):
    ...
    def _normalize_data3d(self, arr: np.ndarray) -> np.ndarray:
        """
        FIX-DATA-8: ch0 mean/std updated for gph500/g unit.
        All other fixes from v14 retained.
        """
        arr = arr.copy()
        for c in range(DATA3D_CH):
            ch = arr[:, :, c]
