...
===================================================================
FIXES vs v20:
  FIX-ENV-22  [P0] uv500_already_normed: build_env_features_one_step
              đọc flag từ env_npy dict đúng cách.
              .npy cũ pipeline: giá trị /U500_NORM=30, range [-1,1].
              CSV pipeline: giá trị raw m²/s², cần _normalize_uv500.
              Flag survive qua env_data_processing (fix ở traj file).

Kept from v20:
  FIX-ENV-20  actual steering flow keys
  FIX-ENV-19  gph500 already_normed handling
  FIX-ENV-18  _GPH500_MEAN/STD
  FIX-ENV-14  wind /150.0
"""
from __future__ import annotations
...
# U/V500 normalization (CSV pipeline: raw m²/s²)
_U500_MEAN  = 5844.9
_U500_STD   =   46.4
_V500_MEAN  = 1482.4
_V500_STD   =   28.7
_UV500_VALID_MIN =  5000.0
_UV500_VALID_MAX = 10000.0
...
def _normalize_uv500_raw(raw_val, mean: float, std: float) -> float:
    """Normalize raw m²/s² value. Returns 0.0 for missing/sentinel."""
    if raw_val is None:
        return 0.0
    try:
        v = float(raw_val)
...
def build_env_features_one_step(
    ...
    u/v500 sources:
      - .npy cũ (build_env_data_scs_v10): uv500_already_normed=True
        keys: "u500_raw_mean" etc. chứa giá trị /30, range [-1,1]
        → dùng trực tiếp, chỉ clip [-5,5]
      - CSV pipeline: uv500_already_normed=False
        keys: "u500_raw_mean" etc. chứa raw m²/s²
        → _normalize_uv500_raw()
    """
    lon_deg = (lon_norm * 50.0 + 1800.0) / 10.0
    lat_deg = (lat_norm * 50.0) / 10.0
    wind_kt = wind_norm * 25.0 + 40.0
    ...
    # ── GPH500 ────────────────────────────────────────────────────────────────
    already_normed_gph = (isinstance(env_npy, dict)
                          and bool(env_npy.get("gph500_already_normed", False)))
        ...
        if isinstance(env_npy, dict) and k in env_npy:
            ...
            if raw is None or math.isnan(raw):
                ...
                if raw < _GPH500_SENTINEL_LO or raw > _GPH500_SENTINEL_HI:
                    ...
                    val = float(np.clip((raw - mean_val) / (std_val + 1e-8), -5.0, 5.0))
        feat[k] = [val]

    # ── U500 / V500 ───────────────────────────────────────────────────────────
    # FIX-ENV-22: phân biệt .npy cũ (already_normed) vs CSV (raw)
    already_normed_uv = (isinstance(env_npy, dict)
                         and bool(env_npy.get("uv500_already_normed", False)))
            ...
            for rk in raw_keys:
                if rk in env_npy:
                    ...
                    if already_normed_uv:
                        # .npy cũ: đã /30, range [-1,1] → dùng trực tiếp
                        try:
                            val = float(np.clip(float(raw_v), -5.0, 5.0))
                        except (TypeError, ValueError):
                            val = 0.0
                    else:
                        # CSV: raw m²/s²
                        val = _normalize_uv500_raw(raw_v, mean_val, std_val)
                    break
        feat[feat_key] = [val]
...
class Env_net(nn.Module):
    ...
    def forward(self, env_data, gph: torch.Tensor) -> tuple:
        if gph.dim() == 4:
            gph = gph.unsqueeze(1)
        if gph.dim() == 5 and gph.shape[-1] <= 13:
            # [B, T, H, W, C] format
            gph = gph.permute(0, 4, 1, 2, 3)
        B, C, T, H, W = gph.shape
        device = gph.device

        # Clamp T để match pos_enc_env
        T_enc = min(T, self.obs_len)

        feat    = build_env_vector(env_data, B, T_enc, device)
        feat_1d = feat[:, :, :ENV_1D_DIM]
        feat_3d = feat[:, :, ENV_1D_DIM:]
        ...
        t_actual = min(T_enc, self.pos_enc_env.shape[1])
        e_env    = e_env[:, :t_actual, :] + self.pos_enc_env[:, :t_actual, :]
        e_env_time = self.transformer_env(e_env)
        ctx = self.out_proj(e_env_time[:, -1, :])
        return ctx, 0, 0